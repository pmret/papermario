...
try:
    ...
    ahocorasick_rs = None
    MATCHKIND_LEFTMOST_LONGEST = None
...
EXTRA_SOURCE_TEXT_FILES = (
    ROOT_DIR / "ver" / "current" / "symbol_addrs.txt",
    ROOT_DIR / "ver" / "current" / "splat.yaml",
)
SOURCE_TEXT_SUFFIXES = {".c", ".h", ".inc", ".s", ".yaml"}
...
def is_source_text_path(path: Path) -> bool:
    return path.suffix in SOURCE_TEXT_SUFFIXES or path.name in {
        "splat.yaml",
        "symbol_addrs.txt",
    }
...
def iter_source_text_files() -> Iterable[Path]:
    ...
    for root in SOURCE_TEXT_ROOTS:
        ...
        for dirpath, dirnames, filenames in os.walk(root):
            ...
            for filename in filenames:
                path = Path(dirpath) / filename
                if not is_source_text_path(path):
                    continue
                if path in seen:
                    continue
                seen.add(path)
    ...
    for path in EXTRA_SOURCE_TEXT_FILES:
...
def is_word_edge_char(ch: str) -> bool:
    return ch == "_" or "0" <= ch <= "9" or "A" <= ch <= "Z" or "a" <= ch <= "z"


def has_identifier_edges(text: str, start: int, end: int) -> bool:
    if start > 0 and is_word_edge_char(text[start - 1]):
        return False
    if end < len(text) and is_word_edge_char(text[end]):
        return False
    return True


def build_aho_replacement_plan(replacements: dict[str, str]) -> AhoReplacementPlan | None:
    replacements = {old: new for old, new in replacements.items() if old != new}
    if not replacements:
        return None

    if ahocorasick_rs is None or MATCHKIND_LEFTMOST_LONGEST is None:
        raise RuntimeError(
            "ahocorasick_rs is required for source text replacement; "
            "install it or use the older regex-based replace_identifiers path"
        )
    ...
    return AhoReplacementPlan(patterns, replacements, automaton)


def replace_identifiers_aho(text: str, plan: AhoReplacementPlan | None) -> str:
    if plan is None:
        return text

    matches = plan.automaton.find_matches_as_indexes(text)
    if not matches:
        return text
    ...
    for pattern_index, start, end in matches:
        if not has_identifier_edges(text, start, end):
            continue

        old = plan.patterns[pattern_index]
        parts.append(text[pos:start])
        parts.append(plan.replacements[old])
        pos = end
        changed = True
...
def replace_text_files(replacements: dict[str, str], dry_run: bool) -> int:
    plan = build_aho_replacement_plan(replacements)
    if plan is None:
        return 0
    ...
    for path in iter_source_text_files():
        ...
        new_text = replace_identifiers_aho(text, plan)
        if new_text == text:
            continue
        changed += 1
        if not dry_run:
