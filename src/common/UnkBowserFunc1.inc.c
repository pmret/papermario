...
API_CALLABLE(N(UnkBowserFunc1)) {
    ...
    if (isInitialCall) {
        ...
        for (i = 0; i < 10; i++) {
            func_802DE780(part->spriteInstanceID, i, IMGFX_UPD_ALLOC_COLOR_BUF, 20, 0, 0, 255, 0);
        }
    }
    ...
    if (part->eventFlags & BOWSER_FLAG_TOCHECK) {
        ...
        for (j = 0; j < 20; j++) {
            ...
            for (i = 0; i < 10; i++) {
                func_802DE780(part->spriteInstanceID, i, IMGFX_UPD_COLOR_BUF_SET_C, j, colorRGBA, 0, 255, 0);
            }
        }
        ...
        for (j = 0; j < 20; j++) {
            ...
            for (i = 0; i < 10; i++) {
                ...
                func_802DE780(part->spriteInstanceID, i, IMGFX_UPD_COLOR_BUF_SET_C, j, colorRGBA, 0, opacity & 0xFFFF, 0);
            }
        }
        script->functionTemp[1] = 0;
        script->functionTemp[2] = 0;
