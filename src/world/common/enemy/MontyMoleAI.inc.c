...
static void N(MontyMoleAI_Wander)(Evt* script, NpcAISettings* aiSettings, EnemyDetectVolume* territory) {
    ...
    if (aiSettings->playerSearchInterval >= 0) {
        if (script->functionTemp[1] <= 0) {
            ...
            if (N(MontyMoleAI_CanAttack)(script, territory, aiSettings->alertRadius, aiSettings->alertOffsetDist.f)) {
                npc->duration = 0;
...
static void N(MontyMoleAI_DrawRock)(Evt* script, NpcAISettings* aiSettings, EnemyDetectVolume* territory) {
    ...
    npc->duration--;
    if ((npc->duration) <= 0) {
        if (!N(MontyMoleAI_CanAttack)(script, territory, aiSettings->alertRadius * 1.1, aiSettings->alertOffsetDist.f)) {
