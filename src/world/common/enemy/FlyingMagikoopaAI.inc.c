...
void N(FlyingMagikoopaAI_11)(Evt* script, NpcAISettings* arg1, EnemyDetectVolume* arg2) {
    Enemy* enemy = script->owner1.enemy;
    Npc* npc = get_npc_unsafe(enemy->npcID);    
    ...
    if (N(MagikoopaAI_CanShootSpell)(script, arg1->chaseRadius, arg1->unkChase, arg2) == 1) {
...
void N(FlyingMagikoopaAI_21)(Evt* script, NpcAISettings* aiSettings, EnemyDetectVolume* territory) {
    ...
    } else if ((N(MagikoopaAI_CanShootSpell)(script, aiSettings->chaseRadius, aiSettings->unkChase, territory) == 1) && (npc->turnAroundYawAdjustment == 0)) {
...
void N(FlyingMagikoopaAI_23)(Evt* script, NpcAISettings* aiSettings, EnemyDetectVolume* territory) {
    ...
    npc->duration--;
    if (npc->duration <= 0) {
        temp_s1 = N(MagikoopaAI_CanShootSpell)(script, aiSettings->chaseRadius, aiSettings->unkChase, territory);
        if (temp_s1 != 1) {
            fx_emote(2, npc, 0.0f, npc->collisionHeight, 1.0f, 2.0f, -20.0f, 12, &emoteTemp);
            npc->currentAnim.w = enemy->animList[0];
            npc->duration = 15;
