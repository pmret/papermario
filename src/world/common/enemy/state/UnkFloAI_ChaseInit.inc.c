...
void N(UnkFloAI_ChaseInit)(Evt* script, NpcAISettings* npcAISettings, EnemyTerritoryThing* territory) {
    ...
    if (npcAISettings->unk_1C.s < fabsf(deltaYaw)) {
        yawFwd = npc->yaw;
        if (deltaYaw < 0.0f) {
            yawFwd += -npcAISettings->unk_1C.s;
        } else {
            yawFwd += npcAISettings->unk_1C.s;
        }
    }
