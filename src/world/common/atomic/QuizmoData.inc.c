...
EvtScript N(EVS_Quizmo_SetQuizCamera) = {
    ...
    EVT_CALL(GetCamType, 0, LocalVar(1), LocalVar(2))
    EVT_CALL(SetCamType, 0, LocalVar(1), 0)
    EVT_CALL(GetCamDistance, 0, LocalVar(0))
    EVT_IF_GT(LocalVar(0), 0)
        EVT_SETF(LocalVar(0), 370)
    EVT_ELSE
        EVT_SETF(LocalVar(0), -370)
    EVT_END_IF
...
EvtScript N(EVS_Quizmo_OtherCamScript) = {
    EVT_CALL(GetNpcPos, CHUCK_QUIZMO_NPC_ID, LocalVar(0), LocalVar(1), LocalVar(2))
...
EvtScript N(EVS_Quizmo_MovePlayerToPodium) = {
    EVT_WAIT(20)
    EVT_CALL(N(Quizmo_AddViewRelativeOffset), ArrayVar(1), ArrayVar(3), 83, LocalVar(0), LocalVar(1))
    EVT_THREAD
        EVT_SETF(LocalVar(2), 0)
        EVT_LOOP(60)
            EVT_SETF(LocalVar(3), LocalVar(0))
            EVT_SETF(LocalVar(4), LocalVar(1))
            EVT_MULF(LocalVar(3), LocalVar(2))
            EVT_MULF(LocalVar(4), LocalVar(2))
            EVT_DIVF(LocalVar(3), 60)
            EVT_DIVF(LocalVar(4), 60)
            EVT_ADDF(LocalVar(3), ArrayVar(1))
            EVT_ADDF(LocalVar(4), ArrayVar(3))
            EVT_CALL(SetPlayerPos, LocalVar(3), ArrayVar(2), LocalVar(4))
...
EvtScript N(EVS_Quizmo_PlayerReaction_RightAnswer) = {
    ...
    EVT_END_LOOP
    EVT_CALL(SetPlayerAnimation, ANIM_10002)
    EVT_CALL(SetPlayerPos, LocalVar(0), ArrayVar(2), LocalVar(2))
    EVT_WAIT(1)
    EVT_RETURN
    EVT_END
};
...
EvtScript N(EVS_Quizmo_ReturnPlayerToOriginalPos) = {
    EVT_THREAD
        EVT_CALL(N(Quizmo_AddViewRelativeOffset), ArrayVar(1), ArrayVar(3), 25, LocalVar(0), LocalVar(1))
        EVT_SETF(LocalVar(2), ArrayVar(1))
        EVT_ADDF(LocalVar(2), LocalVar(0))
        EVT_SETF(LocalVar(3), ArrayVar(3))
        EVT_ADDF(LocalVar(3), LocalVar(1))
        EVT_CALL(SetNpcAnimation, NPC_PARTNER, PARTNER_ANIM_JUMP)
        EVT_CALL(NpcMoveTo, NPC_PARTNER, LocalVar(2), LocalVar(3), 40)
        EVT_CALL(SetNpcAnimation, NPC_PARTNER, PARTNER_ANIM_RUN)
    EVT_END_THREAD
...
EvtScript N(EVS_Quizmo_RightAnswer) = {
    EVT_EXEC_GET_TID(N(EVS_Quizmo_PlayerReaction_RightAnswer), LocalVar(1))
    EVT_WAIT(60)
    EVT_KILL_THREAD(LocalVar(1))
    EVT_LOOP(5)
        EVT_CALL(GetPlayerPos, LocalVar(0), LocalVar(1), LocalVar(2))
        EVT_ADD(LocalVar(1), -1)
        EVT_CALL(SetPlayerPos, LocalVar(0), LocalVar(1), LocalVar(2))
...
EvtScript N(EVS_Quizmo_WrongAnswer) = {
    EVT_EXEC_GET_TID(N(EVS_Quizmo_PlayerReaction_WrongAnswer), LocalVar(1))
    EVT_WAIT(60)
    EVT_LOOP(5)
        EVT_CALL(GetPlayerPos, LocalVar(0), LocalVar(1), LocalVar(2))
        EVT_ADD(LocalVar(1), -1)
...
EvtScript N(EVS_Quizmo_QuizMain) = {
    ...
    EVT_CALL(SetNpcAnimation, NPC_PARTNER, PARTNER_ANIM_RUN)
    EVT_EXEC_GET_TID(N(EVS_Quizmo_SetCharacterPositons), LocalVar(1))
    ...
    EVT_LOOP(0)
        EVT_IS_THREAD_RUNNING(LocalVar(1), LocalVar(0))
    ...
    EVT_LOOP(5)
        ...
        EVT_WAIT(2)
    EVT_END_LOOP
    EVT_SET(LocalVar(0), MESSAGE_ID(0x2C, 0x00))
    EVT_ADD(LocalVar(0), GB_CompletedQuizzes)
    EVT_CALL(SpeakToPlayer, NPC_SELF, NPC_ANIM_chuck_quizmo_Palette_00_Anim_5, NPC_ANIM_chuck_quizmo_Palette_00_Anim_6, 0, LocalVar(0))
    EVT_CALL(SetPlayerAnimation, ANIM_QUESTION)
    EVT_SET(LocalVar(0), MESSAGE_ID(0x2D, 0x00))
    EVT_ADD(LocalVar(0), GB_CompletedQuizzes)
    EVT_CALL(PlaySound, 142)
    EVT_CALL(ShowChoice, LocalVar(0))
    EVT_KILL_THREAD(LocalVar(1))
    EVT_CALL(StopSound, 142)
    EVT_EXEC(N(EVS_Quizmo_PlayerHitBuzzer))
    EVT_WAIT(15)
    EVT_CALL(PlaySound, 141)
    EVT_CALL(N(Quizmo_UnkStageEffectMode), LocalVar(0))
    ...
    EVT_IF_EQ(LocalVar(0), 1)
        ...
        EVT_THREAD
            ...
            EVT_CALL(N(Quizmo_AddViewRelativeOffset), 0, 0, 83, LocalVar(0), LocalVar(2))
            EVT_CALL(PlayEffect, 0x7, 2, LocalVar(0), LocalVar(1), LocalVar(2), 0, 0, 0, 0, 0, 0, 0, 0, 0)
            EVT_CALL(PlayEffect, 0x44, 4, LocalVar(0), LocalVar(1), LocalVar(2), 1, 60, 0, 0, 0, 0, 0, 0, 0)
            EVT_WAIT(15)
            EVT_ADD(LocalVar(1), -3)
            EVT_CALL(N(Quizmo_AddViewRelativeOffset), 0, 0, 58, LocalVar(0), LocalVar(2))
            EVT_CALL(PlayEffect, 0x7, 2, LocalVar(0), LocalVar(1), LocalVar(2), 0, 0, 0, 0, 0, 0, 0, 0, 0)
            EVT_CALL(PlayEffect, 0x44, 4, LocalVar(0), LocalVar(1), LocalVar(2), 1, 60, 0, 0, 0, 0, 0, 0, 0)
            EVT_WAIT(15)
            EVT_ADD(LocalVar(1), 30)
            EVT_CALL(N(Quizmo_AddViewRelativeOffset), 0, 0, 93, LocalVar(0), LocalVar(2))
            EVT_CALL(PlayEffect, 0x7, 2, LocalVar(0), LocalVar(1), LocalVar(2), 0, 0, 0, 0, 0, 0, 0, 0, 0)
            EVT_CALL(PlayEffect, 0x44, 4, LocalVar(0), LocalVar(1), LocalVar(2), 1, 60, 0, 0, 0, 0, 0, 0, 0)
            EVT_WAIT(15)
        EVT_END_THREAD
        EVT_WAIT(20)
        EVT_EXEC_GET_TID(N(EVS_Quizmo_RightAnswer), LocalVar(1))
        ...
        EVT_IF_GT(GB_CompletedQuizzes, 63)
            ...
            EVT_SET(LocalVar(0), ITEM_STAR_PIECE)
    ...
    EVT_ELSE
        ...
        EVT_EXEC_GET_TID(N(EVS_Quizmo_WrongAnswer), LocalVar(1))
        EVT_CALL(GetPlayerPos, LocalVar(2), LocalVar(3), LocalVar(4))
        EVT_CALL(PlayEffect, 0x2B, 0, LocalVar(2), LocalVar(3), LocalVar(4), 0, 0, 0, 0, 0, 0, 0, 0, 0)
