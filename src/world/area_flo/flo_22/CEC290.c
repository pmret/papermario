...
EvtScript N(80241F6C) = {
    EVT_SET(LocalVar(0), EVT_ADDR(N(tree1)))
    EVT_BIND_TRIGGER(N(shakeTree), TRIGGER_WALL_HAMMER, 14, 1, 0)
    EVT_BIND_TRIGGER(N(shakeTree), TRIGGER_POINT_BOMB, EVT_ADDR(N(triggerCoord_80241F28)), 1, 0)
    EVT_SET(LocalVar(0), EVT_ADDR(N(tree2)))
    EVT_BIND_TRIGGER(N(shakeTree), TRIGGER_WALL_HAMMER, 16, 1, 0)
    EVT_BIND_TRIGGER(N(shakeTree), TRIGGER_POINT_BOMB, EVT_ADDR(N(triggerCoord_80241F5C)), 1, 0)
    EVT_RETURN
    EVT_END
};
