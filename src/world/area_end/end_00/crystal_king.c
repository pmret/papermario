...
EvtScript N(EVS_ParadePhase_CrystalKing) = {
    ...
    Thread
        Call(SetNpcAnimation, NPC_FakeCrystalKing, ANIM_CrystalKing_LeanForward)
        Wait(60 * DT)
        Call(SetNpcAnimation, NPC_FakeCrystalKing, ANIM_CrystalKing_Idle)
    EndThread
    Thread
        Wait(10 * DT)
        Loop(4)
            Call(SetNpcAnimation, NPC_Duplighost1, ANIM_Duplighost_Still)
            Wait(10 * DT)
            Call(SetNpcAnimation, NPC_Duplighost1, ANIM_Duplighost_Crouch)
            Wait(10 * DT)
        EndLoop
        Call(SetNpcAnimation, NPC_Duplighost1, ANIM_Duplighost_Idle)
    EndThread
    ...
    Wait(10 * DT)
    Call(SetNpcAnimation, NPC_FakeCrystalKing, ANIM_CrystalKing_Hurt)
    Wait(10 * DT)
    KillThread(LVarB)
    Call(GetNpcPos, NPC_FakeCrystalKing, LVar0, LVar1, LVar2)
    PlayEffect(EFFECT_BIG_SMOKE_PUFF, LVar0, LVar1, LVar2, 1, 1, 1, 1)
    ...
    Thread
        Loop(6)
            Call(SetNpcAnimation, NPC_TrueCrystalKing, ANIM_CrystalKing_LeanForward)
            Wait(10 * DT)
            Call(SetNpcAnimation, NPC_TrueCrystalKing, ANIM_CrystalKing_FlailLoop)
            Wait(10 * DT)
        EndLoop
    EndThread
    Thread
        ...
        Loop(4)
            Call(SetNpcAnimation, NPC_Duplighost1, ANIM_Duplighost_Still)
            Call(SetNpcAnimation, NPC_Duplighost2, ANIM_Duplighost_Still)
            Wait(10 * DT)
            Call(SetNpcAnimation, NPC_Duplighost1, ANIM_Duplighost_Crouch)
            Call(SetNpcAnimation, NPC_Duplighost2, ANIM_Duplighost_Crouch)
            Wait(10 * DT)
        EndLoop
        Call(SetNpcAnimation, NPC_Duplighost1, ANIM_Duplighost_Idle)
        Call(SetNpcAnimation, NPC_Duplighost2, ANIM_Duplighost_Idle)
    ...
    EndLoop
    Wait(30 * DT)
    Call(SetNpcAnimation, NPC_Duplighost1, ANIM_Duplighost_Walk)
    Call(SetNpcAnimation, NPC_Duplighost2, ANIM_Duplighost_Walk)
    KillThread(LVarA)
    KillThread(LVarB)
    Return
    End
