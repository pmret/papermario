...
ApiStatus N(func_80241F10_802790)(ScriptInstance* script, s32 isInitialCall) {
    ...
    if (N(D_80248C30_8094B0) == NULL) {
        ...
        for (i = 0; i < 16; i++) {
            N(D_80248C30_8094B0)[i] = script->varTable[i].s;
        }
    } else {
        for (i = 0; i < 16; i++) {
            script->varTable[i].s = N(D_80248C30_8094B0)[i];
        }
...
ApiStatus func_80245440_805CC0(ScriptInstance* script, s32 isInitialCall) {
    if (gPlayerData.currentPartner == script->varTable[10].s) {
        script->varTable[1].s = 0;
        return ApiStatus_DONE2;
    }

    func_800EB168(script->varTable[10].s);
    script->varTable[1].s = 1;
    return ApiStatus_DONE2;
}
