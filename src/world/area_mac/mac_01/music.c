...
EvtScript N(EVS_SetupMusic) = {
    Switch(GB_StoryProgress)
        ...
        CaseEq(STORY_EPILOGUE)
            Call(SetMusicTrack, 0, SONG_PEACHS_CASTLE_PARTY, 0, VOL_LEVEL_FULL)
...
EvtScript N(EVS_PlaySpellcastSong) = {
    Call(SetMusicTrack, 0, SONG_MERLEE_SPELL, BGM_VARIATION_1, VOL_LEVEL_FULL)
    Return
    End
};
...
EvtScript N(EVS_PlayFlowerGateSong) = {
    Call(SetMusicTrack, 0, SONG_FLOWER_GATE_APPEARS, 0, VOL_LEVEL_FULL)
    Return
    End
};
