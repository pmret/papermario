...
EvtScript N(EVS_PlaySpellcastSong) = {
    Call(SetMusicTrack, 0, SONG_MERLEE_SPELL, 1, VOL_LEVEL_8)
    Return
    End
};
...
EvtScript N(EVS_ResetMusicAfterFortune) = {
    Switch(GB_StoryProgress)
        CaseRange(STORY_CH3_STAR_SPRIT_DEPARTED, STORY_CH4_STAR_SPIRIT_RESCUED)
            Call(SetMusicTrack, 0, SONG_SHY_GUY_INVASION, 0, VOL_LEVEL_8)
        CaseDefault
            Call(SetMusicTrack, 0, SONG_TOAD_TOWN, 0, VOL_LEVEL_8)
            Wait(20)
...
EvtScript N(EVS_PlayFlowerGateSong) = {
    Call(SetMusicTrack, 0, SONG_FLOWER_GATE_APPEARS, 0, VOL_LEVEL_8)
    Return
    End
};
