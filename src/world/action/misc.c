...
void action_update_raise_arms(void) {
    ...
    if (playerStatus->flags & PS_FLAG_ACTION_STATE_CHANGED) {
        ...
        playerStatus->currentSpeed = 0.0f;
        playerStatus->pitch = 0.0f;
        suggest_player_anim_setUnkFlag(ANIM_Mario1_UsePower);
    }
...
void action_update_pushing_block(void) {
    ...
    if (playerStatus->flags & PS_FLAG_ACTION_STATE_CHANGED) {
        ...
        if (((angle >= 45.0f) && (angle <= 135.0f)) || ((angle >= 225.0f) && (angle <= 315.0f))) {
            suggest_player_anim_setUnkFlag(ANIM_MarioW2_PushSide);
        } else {
            suggest_player_anim_clearUnkFlag(ANIM_MarioW1_PushToward);
        }
    }
...
void action_update_talk(void) {
    ...
    if (playerStatus->flags & PS_FLAG_ACTION_STATE_CHANGED) {
        ...
        if (!(playerStatus->animFlags & PA_FLAG_USING_PEACH_PHYSICS)) {
            suggest_player_anim_setUnkFlag(ANIM_MarioW2_SpeakUp);
