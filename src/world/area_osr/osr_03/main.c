...
EvtScript N(EVS_Main) = {
    ...
    Switch(GB_StoryProgress)
        ...
        CaseOrEq(STORY_CH3_STAR_SPIRIT_RESCUED)
            ...
            Call(SetMusicTrack, 0, SONG_PRISONER_PEACH_THEME, 0, VOL_LEVEL_FULL)
        EndCaseGroup
        CaseEq(STORY_CH2_STAR_SPIRIT_RESCUED)
            Call(ClearAmbientSounds, 250)
            Call(SetMusicTrack, 0, SONG_BOWSER_THEME, 0, VOL_LEVEL_FULL)
        CaseOrEq(STORY_CH4_STAR_SPIRIT_RESCUED)
        CaseOrEq(STORY_CH5_OPENED_ESCAPE_ROUTE)
        CaseOrEq(STORY_CH6_STAR_SPIRIT_RESCUED)
            Call(ClearAmbientSounds, 250)
            Call(SetMusicTrack, 0, SONG_PEACH_MISSION, 0, VOL_LEVEL_FULL)
        EndCaseGroup
        ...
        CaseDefault
            ...
            Else
                Call(SetMusicTrack, 0, SONG_BOWSERS_CASTLE_FALLS, 0, VOL_LEVEL_FULL)
            EndIf
    EndSwitch
    Exec(N(EVS_EnterMap))
    Call(GetEntryID, LVar0)
