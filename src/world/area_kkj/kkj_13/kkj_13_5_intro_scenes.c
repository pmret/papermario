...
API_CALLABLE(N(UpdatePropellerSoundPos_Intro)) {
    ...
    sfx_adjust_env_sound_pos(SOUND_LRAW_BOWSER_PROPELLER, SOUND_SPACE_MODE_0, x, y, z);
    script->varTable[0] = y;
...
EvtScript N(EVS_Scene_BowserAttacks) = {
    ...
    EVT_CALL(PlaySoundAtNpc, NPC_Bowser_Body, SOUND_2121, SOUND_SPACE_MODE_0)
    EVT_PLAY_EFFECT(EFFECT_GATHER_ENERGY_PINK, 0, 780, 95, 27, 1, 30)
    EVT_WAIT(40 * DT)
    EVT_THREAD
        EVT_CALL(ShakeCam, CAM_DEFAULT, 0, 10 * DT, EVT_FLOAT(4.0))
    EVT_END_THREAD
    EVT_CALL(PlaySoundAtPlayer, SOUND_BOWSER_LIGHTNING, SOUND_SPACE_MODE_0)
    EVT_CALL(GetPlayerPos, LVar0, LVar1, LVar2)
    EVT_ADD(LVar2, 5)
    EVT_PLAY_EFFECT(EFFECT_LIGHTNING, 4, LVar0, LVar1, LVar2, 0, 0)
    EVT_THREAD
        EVT_CALL(PlaySoundAtPlayer, SOUND_0390, SOUND_SPACE_MODE_0)
