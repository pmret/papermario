...
extern EvtScript EVS_AdvancedDoorOpen;
extern EvtScript EVS_AdvancedDoorClose;
...
API_CALLABLE(MakeDoorAdvanced) {
    ...
    bind_trigger_1(&EVS_AdvancedDoorOpen, 0x100, colliderID, 0, (s32) door, 3);
    if (visGroups & 0x800) {
        bind_trigger_1(&EVS_AdvancedDoorClose, 0x40, triggerColliderID, 1, (s32) door, 3);
    } else {
        bind_trigger_1(&EVS_AdvancedDoorClose, 0x100, triggerColliderID, 1, (s32) door, 3);
    }
    return ApiStatus_DONE2;
}

API_CALLABLE(func_802822A8) {
    Bytecode* args = script->ptrReadPos;
...
API_CALLABLE(SetNewItemVisGroup) {
    AdvancedDoor* door = script->functionTempPtr[1];
...
EvtScript EVS_802843D4 = {
	...
	EVT_SWITCH(LVar0)
		EVT_CASE_EQ(0)
			...
			EVT_CALL(func_802822A8, SOUND_A0000000)
		EVT_CASE_EQ(1)
	...
	EVT_SWITCH(LVar3)
		EVT_CASE_EQ(1)
			EVT_CALL(func_802822A8, SOUND_A0000001)
		EVT_CASE_EQ(3)
			EVT_CALL(func_802822A8, SOUND_A0000001)
	EVT_END_SWITCH
	EVT_RETURN
...
EvtScript EVS_AdvancedDoorOpen = {
	...
	EVT_IF_NE(LVarC, 0)
		...
		EVT_ELSE
			EVT_EXEC_GET_TID(EVS_80284584, LVar5)
		EVT_END_IF
	EVT_END_IF
	EVT_IF_NE(LVarC, 0)
		EVT_CALL(func_802823B0, 0, LVar5)
	...
	EVT_ELSE
		EVT_EXEC_GET_TID(EVS_802843D4, LVar5)
	EVT_END_IF
	...
	EVT_CALL(func_802823B0, 3, LVar5)
	EVT_SET(LVar0, 0)
	EVT_BITWISE_AND_CONST(LVar2, 0x400)
	EVT_IF_NE(LVarD, 0)
		EVT_CALL(func_8028236C, 3)
		EVT_IF_NE(LVar2, 0)
			EVT_EXEC_GET_TID(LVarD, LVar5)
		EVT_ELSE
			EVT_EXEC_GET_TID(EVS_802846AC, LVar5)
		EVT_END_IF
	EVT_END_IF
	EVT_IF_NE(LVarD, 0)
...
EvtScript EVS_AdvancedDoorClose = {
	EVT_SET_GROUP(EVT_GROUP_00)
	EVT_CALL(GetPlayerActionState, LVar3)
	EVT_SET(LVar4, 0)
	EVT_IF_EQ(LVar3, ACTION_STATE_IDLE)
	...
	EVT_CALL(GetDoorData)
	EVT_SET(LVar0, 3)
	EVT_BITWISE_AND_CONST(LVar2, 0x100)
	EVT_IF_NE(LVar2, 0)
		EVT_EXEC_WAIT(LVarB)
	EVT_ELSE
		EVT_EXEC_WAIT(EVS_802843D4)
	EVT_END_IF
	EVT_CALL(GetDoorData)
	EVT_SET(LVar0, 3)
	EVT_IF_NE(LVarE, 0)
