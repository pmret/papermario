...
EvtScript N(EVS_UnlockPrompt_PrisonCell) = {
    ...
    EVT_SET(GF_KPA95_UnlockedDoor, TRUE)
...
EvtScript N(EVS_MakeEntities) = {
    EVT_IF_EQ(GF_KPA95_UnlockedDoor, FALSE)
    ...
    EVT_ELSE
        EVT_BIND_TRIGGER(EVT_PTR(N(EVS_OpenCellDoor)), TRIGGER_WALL_PRESS_A, COLLIDER_deilittn, 1, 0)
    EVT_END_IF
    EVT_IF_EQ(GF_KPA95_Defeated_Guard, TRUE)
        EVT_CALL(MakeItemEntity, ITEM_PRISON_KEY1, -70, 0, 55, ITEM_SPAWN_MODE_FIXED_NEVER_VANISH, GF_KPA95_Item_PrisonKey1)
    EVT_END_IF
    EVT_RETURN
    EVT_END
};
