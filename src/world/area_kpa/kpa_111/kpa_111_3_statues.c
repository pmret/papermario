...
EvtScript N(EVS_PushRightStatue_Impl) = {
    ...
    EVT_CALL(PlaySoundAtCollider, LVar9, SOUND_LOOP_MOVE_STATUE, SOUND_SPACE_MODE_0)
    EVT_LOOP(0)
        EVT_CALL(UpdateLerp)
        EVT_CALL(TranslateGroup, MODEL_g21, LVar0, 0, 0)
...
EvtScript N(EVS_PushRightStatue_FromLeft) = {
    EVT_IF_EQ(GB_KPA111_StatuePosition, 2)
        EVT_RETURN
    EVT_END_IF
    EVT_IF_EQ(GB_KPA111_StatuePosition, 0)
        EVT_SET(LVar6, 0)
        EVT_SET(LVar7, 50)
        EVT_SET(LVar8, 2)
    EVT_ELSE
    ...
    EVT_IF_NE(LVar8, -1)
        EVT_SET(GB_KPA111_StatuePosition, LVar8)
    EVT_END_IF
    EVT_RETURN
    EVT_END
};
...
EvtScript N(EVS_PushRightStatue_FromRight) = {
    EVT_IF_EQ(GB_KPA111_StatuePosition, 1)
        EVT_RETURN
    EVT_END_IF
    EVT_IF_EQ(GB_KPA111_StatuePosition, 0)
        EVT_SET(LVar6, 0)
...
EvtScript N(EVS_SetupStatues) = {
    EVT_SWITCH(GB_KPA111_StatuePosition)
        EVT_CASE_EQ(1)
            EVT_CALL(TranslateGroup, MODEL_g21, -50, 0, 0)
        EVT_CASE_EQ(2)
            EVT_CALL(TranslateGroup, MODEL_g21, 50, 0, 0)
    EVT_END_SWITCH
    EVT_CALL(ParentColliderToModel, COLLIDER_kpa_c, MODEL_o105)
    EVT_CALL(ParentColliderToModel, COLLIDER_kpa_l, MODEL_o105)
    EVT_CALL(ParentColliderToModel, COLLIDER_kpa_r, MODEL_o105)
    EVT_BIND_TRIGGER(EVT_PTR(N(EVS_PushRightStatue_FromLeft)), TRIGGER_WALL_PUSH, COLLIDER_kpa_l, 1, 0)
    EVT_BIND_TRIGGER(EVT_PTR(N(EVS_PushRightStatue_FromRight)), TRIGGER_WALL_PUSH, COLLIDER_kpa_r, 1, 0)
    EVT_RETURN
    EVT_END
};
