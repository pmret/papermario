...
EvtScript N(EVS_OpenAirshipDockDoor) = {
    EVT_CALL(PlaySoundAtCollider, COLLIDER_deilitts, SOUND_01DF, SOUND_SPACE_DEFAULT)
...
EvtScript N(EVS_CloseAirshipDockDoor) = {
    ...
    EVT_CALL(PlaySoundAtCollider, COLLIDER_deilitts, SOUND_01E0, SOUND_SPACE_DEFAULT)
    EVT_RETURN
    EVT_END
};
...
EvtScript N(EVS_ExitWalk_kpa_1X_Upper) = {
    EVT_SET_GROUP(EVT_GROUP_1B)
    EVT_CALL(UseExitHeading, 60, kpa_60_ENTRY_1)
    EVT_EXEC(ExitWalk)
...
EvtScript N(EVS_ExitWalk_kpa_1X_Lower) = {
    EVT_SET_GROUP(EVT_GROUP_1B)
    EVT_CALL(UseExitHeading, 60, kpa_60_ENTRY_2)
    EVT_EXEC(ExitWalk)
    EVT_IF_EQ(GF_KPA16_ShutOffLava, FALSE)
...
EvtScript N(EVS_ExitDoor_kpa_63_0) = {
    ...
    EVT_WAIT(15)
    EVT_CALL(UseExitHeading, 60, kpa_60_ENTRY_3)
    EVT_EXEC(ExitWalk)
...
EvtScript N(EVS_HideDistalGeometry) = {
    EVT_CALL(EnableGroup, MODEL_g390, FALSE)
    EVT_CALL(EnableGroup, MODEL_g391, FALSE)
    EVT_CALL(EnableGroup, MODEL_g402, FALSE)
    EVT_RETURN
    EVT_END
};
...
EvtScript N(EVS_EnterMap) = {
    ...
    EVT_SWITCH(LVar0)
        EVT_CASE_EQ(kpa_60_ENTRY_0)
            EVT_EXEC_WAIT(N(EVS_HideDistalGeometry))
            EVT_SET(LVar2, MODEL_o1647)
            EVT_SET(LVar3, MODEL_o1646)
            EVT_EXEC_WAIT(EnterDoubleDoor)
            EVT_EXEC(N(EVS_BindExitTriggers))
        EVT_CASE_EQ(kpa_60_ENTRY_3)
            EVT_EXEC_WAIT(N(EVS_HideDistalGeometry))
            EVT_THREAD
                EVT_EXEC_WAIT(N(EVS_EnterFromAirshipDock))
                EVT_EXEC(N(EVS_BindExitTriggers))
            EVT_END_THREAD
        EVT_CASE_EQ(kpa_60_ENTRY_4)
            EVT_EXEC(N(EVS_Scene_Arrival))
        EVT_CASE_EQ(kpa_60_ENTRY_5)
            EVT_EXEC(N(EVS_Scene_Departure))
        EVT_CASE_DEFAULT
            EVT_EXEC_WAIT(N(EVS_HideDistalGeometry))
