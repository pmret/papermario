...
EvtScript N(EVS_PlaySongForPlayerPos) = {
    ...
    Label(0)
        ...
        Switch(GB_StoryProgress)
            CaseLt(STORY_CH6_DESTROYED_PUFF_PUFF_MACHINE)
                Call(SetMusicTrack, 0, SONG_SUN_TOWER_CLOUDY, 0, VOL_LEVEL_8)
            CaseDefault
                Call(SetMusicTrack, 0, SONG_SUN_TOWER_SUNNY, 0, VOL_LEVEL_8)
        EndSwitch
        Loop(0)
            Call(GetPlayerPos, LVar0, LVar1, LVar2)
            IfGt(LVar0, -500)
...
EvtScript N(EVS_PushPartnerSong) = {
    Call(SetMusicTrack, 0, SONG_NEW_PARTNER, 0, VOL_LEVEL_8)
    Return
    End
};
