...
EvtScript N(EVS_LogAnim_RiseUp) = {
    EVT_CALL(PlaySoundAtCollider, COLLIDER_o11, SOUND_SMASH_GOOMNUT_TREE, 0)
...
EvtScript N(EVS_LogAnim_FallDown) = {
    ...
    EVT_CALL(PlaySoundAtCollider, COLLIDER_o11, SOUND_209D, 0)
...
EvtScript N(EVS_LogAnim_Split) = {
    EVT_CALL(PlaySoundAtCollider, COLLIDER_o11, SOUND_209E, 0)
    EVT_CALL(EnableModel, MODEL_o79, TRUE)
    EVT_CALL(EnableModel, MODEL_o76, TRUE)
...
EvtScript N(EVS_SetupLogs) = {
    EVT_IF_EQ(GF_JAN10_CreateLogBridge, FALSE)
        EVT_BIND_TRIGGER(EVT_PTR(N(EVS_SmashWall_BuriedLog)), TRIGGER_WALL_HAMMER, COLLIDER_o11, 1, 0)
    EVT_END_IF
    EVT_EXEC(N(EVS_SetupLogObjects))
    EVT_RETURN
    EVT_END
