...
EvtScript N(EVS_RoomListener_House) = {
    EVT_SWITCH(LVar0)
        EVT_CASE_EQ(ROOM_UPDATE_ENTER_BEGIN)
            EVT_CALL(SetGroupVisibility, MODEL_g29, 1)
            EVT_CALL(SetGroupVisibility, MODEL_g81, 1)
            EVT_CALL(SetGroupVisibility, MODEL_g51, 1)
        EVT_CASE_EQ(ROOM_UPDATE_ENTER_DONE)
            EVT_CALL(SetGroupVisibility, MODEL_g1, 0)
            EVT_CALL(SetGroupVisibility, MODEL_g35, 0)
        EVT_CASE_EQ(ROOM_UPDATE_EXIT_BEGIN)
            EVT_CALL(SetGroupVisibility, MODEL_g1, 1)
            EVT_CALL(SetGroupVisibility, MODEL_g35, 1)
        EVT_CASE_EQ(ROOM_UPDATE_EXIT_END)
            EVT_CALL(SetGroupVisibility, MODEL_g29, 0)
            EVT_CALL(SetGroupVisibility, MODEL_g81, 0)
            EVT_CALL(SetGroupVisibility, MODEL_g31, 0)
            EVT_CALL(SetGroupVisibility, MODEL_g51, 0)
    EVT_END_SWITCH
    EVT_RETURN
    EVT_END
};
...
EvtScript N(EVS_ManageUpstairsVisibility) = {
    ...
    EVT_IF_LT(LVar1, 100)
        EVT_CALL(SetGroupVisibility, MODEL_g31, 0)
        EVT_CALL(SetGroupVisibility, MODEL_g50, 1)
    EVT_ELSE
        EVT_CALL(SetGroupVisibility, MODEL_g31, 1)
        EVT_CALL(SetGroupVisibility, MODEL_g50, 0)
    EVT_END_IF
    EVT_RETURN
    EVT_END
};
