...
API_CALLABLE(N(Zipline_AdjustMoveDownSound)) {
    ...
    if (*args++ != 0) {
        ...
        sfx_adjust_env_sound_pos(SOUND_LRAW_168, SOUND_SPACE_MODE_0, script->functionTemp[0], script->functionTemp[1], script->functionTemp[2]);
    }
...
API_CALLABLE(N(Zipline_AdjustMoveUpSound)) {
    ...
    if (*args++ != 0) {
        ...
        script->functionTemp[1] = evt_get_variable(script, *args++);
        script->functionTemp[2] = evt_get_variable(script, *args++);
        sfx_adjust_env_sound_pos(SOUND_LRAW_185, SOUND_SPACE_MODE_0, script->functionTemp[0], script->functionTemp[1], script->functionTemp[2]);
    }
    return ApiStatus_DONE2;
}
