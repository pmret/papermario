...
EvtScript N(exitSingleDoor_80240100) = {
    ...
    EVT_WAIT(17)
    EVT_CALL(GotoMap, EVT_ADDR("arn_12"), 1)
    EVT_WAIT(100)
    EVT_RETURN
    EVT_END
};
...
EvtScript N(exitSingleDoor_802401A4) = {
    ...
    EVT_EXEC(ExitSingleDoor)
    EVT_WAIT(17)
    EVT_CALL(GotoMap, EVT_ADDR("arn_11"), 0)
    EVT_WAIT(100)
    EVT_RETURN
    EVT_END
};
...
EvtScript N(main) = {
    ...
    EVT_CALL(MakeNpcs, 0, EVT_ADDR(N(npcGroupList_80240BCC)))
...
EvtScript N(npcAI_802404AC) = {
    EVT_CALL(BasicAI_Main, EVT_ADDR(N(npcAISettings_8024047C)))
    EVT_RETURN
    EVT_END
};
...
EvtScript N(idle_802404F8) = {
    ...
    EVT_CALL(NpcJump0, NPC_SELF, LocalVar(0), 0, LocalVar(2), 8)
    ...
    EVT_CALL(PlaySoundAtNpc, NPC_SELF, 0x20C8, 0)
    EVT_CALL(NpcJump0, NPC_SELF, LocalVar(0), 0, LocalVar(2), 12)
    EVT_CALL(SetNpcAnimation, NPC_SELF, NPC_ANIM_tubbas_heart_Palette_00_Anim_14)
    EVT_WAIT(1)
    EVT_CALL(SetNpcAnimation, NPC_SELF, NPC_ANIM_tubbas_heart_Palette_00_Anim_13)
    EVT_SUB(LocalVar(0), 80)
...
EvtScript N(init_80240720) = {
    EVT_CALL(BindNpcIdle, NPC_SELF, EVT_ADDR(N(idle_802404F8)))
...
EvtScript N(init_8024076C) = {
    EVT_IF_LT(GB_StoryProgress, -12)
        EVT_IF_EQ(GF_ARN13_Defeated_Goomba, 1)
            ...
            EVT_RETURN
        EVT_END_IF
        EVT_CALL(BindNpcDefeat, NPC_SELF, EVT_ADDR(N(defeat_802406F4)))
    EVT_END_IF
    EVT_CALL(BindNpcDefeat, NPC_SELF, EVT_ADDR(N(defeat_802406F4)))
    EVT_RETURN
    EVT_END
...
EvtScript N(makeEntities) = {
    ...
    EVT_CALL(AssignScript, EVT_ADDR(N(80240BF0)))
    EVT_RETURN
    EVT_END
};
