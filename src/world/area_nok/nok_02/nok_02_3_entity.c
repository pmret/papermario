...
EvtScript N(D_80245D00_9DCD20) = {
    EVT_CALL(N(ItemEntityJumpToPos), MV_Unk_0A, EVT_FLOAT(227.0), EVT_FLOAT(0.0), EVT_FLOAT(247.0), 20, EVT_FLOAT(1.0))
    EVT_RETURN
    EVT_END
};

EvtScript N(D_80245D34_9DCD54) = {
...
EvtScript N(EVS_MakeEntities) = {
    ...
    EVT_SWITCH(GB_StoryProgress)
        EVT_CASE_LT(STORY_CH1_KOOPER_JOINED_PARTY)
            EVT_IF_EQ(GF_NOK02_RecoveredShellB, FALSE)
                EVT_CALL(AssignScript, EVT_PTR(N(EVS_8024B488)))
            EVT_END_IF
        EVT_CASE_GE(STORY_CH1_KOOPER_JOINED_PARTY)
            EVT_IF_EQ(GF_NOK02_Item_StarPiece, FALSE)
                EVT_CALL(AssignScript, EVT_PTR(N(D_80245D00_9DCD20)))
    ...
    EVT_IF_EQ(GF_NOK02_WarpPipe, FALSE)
        ...
        EVT_END_IF
    EVT_END_IF
    EVT_CALL(MakeEntity, EVT_PTR(Entity_BlueWarpPipe), 365, 0, 90, 30, nok_02_ENTRY_2, EVT_PTR(N(D_80245D34_9DCD54)), EVT_INDEX_OF_GAME_FLAG(GF_NOK02_WarpPipe), MAKE_ENTITY_END)
    EVT_RETURN
    EVT_END
};
