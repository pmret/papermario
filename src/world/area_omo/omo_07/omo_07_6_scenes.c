...
EvtScript N(EVS_Scene_KammySetAmbush) = {
    ...
    Set(GF_OMO07_SpawnedPeachChoice2, TRUE)
    Set(AB_OMO_CurrentPeachChoice, GB_OMO_PeachChoice2)
    Switch(AB_OMO_CurrentPeachChoice)
        CaseEq(0)
            Set(MV_AmbushID, NPC_Fuzzy)
        CaseEq(1)
            Set(MV_AmbushID, NPC_HammerBros)
        CaseEq(2)
            Set(MV_AmbushID, ITEM_THUNDER_RAGE)
    EndSwitch
    Call(SetNpcPos, NPC_Kammy, 1000, 15, 0)
    ...
    EndIf
    Call(UseSettingsFrom, CAM_DEFAULT, 1060, 0, 0)
    Call(SetPanTarget, CAM_DEFAULT, 1060, 0, 0)
    ...
    IfNe(AB_OMO_CurrentPeachChoice, 2)
        ...
        Wait(5 * DT)
    EndIf
    Call(GotoMap, Ref("omo_07"), omo_07_ENTRY_0)
    Wait(100 * DT)
    Return
    End
};
