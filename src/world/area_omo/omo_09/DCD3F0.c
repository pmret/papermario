...
/*
ApiStatus N(func_80240720_DCD3F0)(ScriptInstance* script, s32 isInitialCall) {
    s32 i;

    if (N(D_80247010_DD3CE0) == NULL) {
        N(D_80247010_DD3CE0) = heap_malloc(16 * sizeof(s32));
        for (i = 0; i < 16; i++) {
            N(D_80247010_DD3CE0)[i] = script->varTable[i].s;
        }
    } else {
        for (i = 0; i < 16; i++) {
            script->varTable[i].s = N(D_80247010_DD3CE0)[i];
        }
