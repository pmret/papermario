...
EvtScript N(EVS_ExitDoors_pra_03_1) = {
    ...
    Wait(17)
    Call(GotoMap, Ref("pra_03"), pra_03_ENTRY_1)
    Wait(100)
    Return
    End
};
...
EvtScript N(EVS_ExitDoors_pra_05_0) = {
    ...
    Exec(ExitSplitDoubleDoor)
    Wait(17)
    Call(GotoMap, Ref("pra_05"), pra_05_ENTRY_0)
    Wait(100)
    Return
    End
};
...
EvtScript N(EVS_BindExitTriggers) = {
    BindTrigger(Ref(N(EVS_ExitDoors_pra_03_1)), TRIGGER_WALL_PRESS_A, COLLIDER_deilittsw, 1, 0)
    BindTrigger(Ref(N(EVS_ExitDoors_pra_05_0)), TRIGGER_WALL_PRESS_A, COLLIDER_deilittse, 1, 0)
    Return
    End
};
...
EvtScript N(EVS_EnterMap) = {
    ...
    Switch(LVar0)
        CaseEq(pra_38_ENTRY_0)
            Set(LVar2, MODEL_o772)
            Set(LVar3, MODEL_o768)
            Set(LVar4, MODEL_o844)
            Set(LVar5, MODEL_o846)
            ExecWait(EnterSplitDoubleDoor)
        CaseEq(pra_38_ENTRY_1)
            Set(LVar2, MODEL_o1019)
            Set(LVar3, MODEL_o1021)
            Set(LVar4, MODEL_o1020)
            Set(LVar5, MODEL_o1022)
