        ...
        if (!(playerStatus->flags & PS_FLAG_SPINNING)) {
            if (playerStatus->alpha1 != D_802D9D70) {
                if (playerStatus->alpha1 < 254) {
                    ...
                    spr_set_player_imgfx_update_comp(PLAYER_SPRITE_AUX2, -1, IMGFX_UPD_SET_ALPHA, 0, 0, 0, playerStatus->alpha1, 0);
                } else {
                    renderMode = RENDER_MODE_ALPHATEST;
                    spr_set_player_imgfx_update_comp(PLAYER_SPRITE_AUX2, -1, IMGFX_UPD_CLEAR, 0, 0, 0, 0, 0);
                }
            }
    ...
    if (playerStatus->flags & PS_FLAG_HAS_REFLECTION) {
        ...
        if (!(playerStatus->flags & PS_FLAG_SPINNING)) {
            if (playerStatus->alpha1 != D_802D9D71) {
                if (playerStatus->alpha1 < 254) {
                    renderMode = RENDER_MODE_SURFACE_XLU_LAYER1;
                    spr_set_player_imgfx_update_comp(PLAYER_SPRITE_AUX1, -1, IMGFX_UPD_SET_ALPHA, 0, 0, 0, playerStatus->alpha1, 0);
                } else {
                    renderMode = RENDER_MODE_ALPHATEST;
                    spr_set_player_imgfx_update_comp(PLAYER_SPRITE_AUX1, -1, IMGFX_UPD_CLEAR, 0, 0, 0, 0, 0);
                }
            }
...
void N(appendGfx_reflect_player_floor_fancy)(PlayerStatus* playerStatus) {
    ...
    for (i = 0; i < 2; i++) {
        ...
        if (i == 0) {
            ...
            spr_set_player_imgfx_update_comp(PLAYER_SPRITE_AUX1, -1, IMGFX_UPD_SET_COLOR, tint, tint, tint, 255, 0);
            ...
            px = playerStatus->position.x;
            pz = playerStatus->position.z;
            spr_set_player_imgfx_update_comp(PLAYER_SPRITE_AUX1, -1, IMGFX_UPD_SET_ALPHA, 0, 0, 0, 64, 0);
