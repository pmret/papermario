...
EvtScript N(EVS_RoomListener_Shop) = {
    EVT_SWITCH(LVar0)
        EVT_CASE_EQ(ROOM_UPDATE_ENTER_BEGIN)
            EVT_SWITCH(GB_StoryProgress)
                ...
                EVT_CASE_LT(STORY_CH2_SHADY_MOUSE_ENTERED_SHOP)
                    ...
                    EVT_RETURN
            EVT_END_SWITCH
            EVT_CALL(SetGroupVisibility, MODEL_mise_naka, 1)
        EVT_CASE_EQ(ROOM_UPDATE_EXIT_BEGIN)
            ...
            EVT_END_THREAD
        EVT_CASE_EQ(ROOM_UPDATE_EXIT_END)
            EVT_CALL(SetGroupVisibility, MODEL_mise_naka, 0)
...
EvtScript N(EVS_RoomListener_RightHouse) = {
    EVT_SWITCH(LVar0)
        EVT_CASE_EQ(ROOM_UPDATE_ENTER_BEGIN)
            EVT_CALL(SetGroupVisibility, MODEL_ie2_naka, 1)
        EVT_CASE_EQ(ROOM_UPDATE_EXIT_BEGIN)
            // do nothing
        EVT_CASE_EQ(ROOM_UPDATE_EXIT_END)
            EVT_CALL(SetGroupVisibility, MODEL_ie2_naka, 0)
    EVT_END_SWITCH
    EVT_RETURN
    EVT_END
};
