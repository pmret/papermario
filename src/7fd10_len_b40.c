...
void func_800E6860(void) {
    ...
    if (gPartnerActionStatus.actionState.b[0] != 0 && gPartnerActionStatus.actionState.b[3] == PARTNER_BOW) {
...
s32 func_800E6904(void) {
    ...
    if (partnerActionStatus->actionState.b[0] == PARTNER_ACTION_NONE) {
        ...
        if (partnerActionStatus->actionState.b[3] == PARTNER_WATT) {
            return TRUE;
        } else if (partnerActionStatus->actionState.b[3] == PARTNER_BOW) {
            if (actionState == ACTION_STATE_RIDE) {
                return TRUE;
            }
        } else if (partnerActionStatus->actionState.b[3] == PARTNER_LAKILESTER) {
            if (actionState == ACTION_STATE_RIDE) {
                return TRUE;
            }
        ...
        !is_picking_up_item())
    {
        if (!(gPlayerStatus.animFlags & PLAYER_STATUS_ANIM_FLAGS_8BIT_MARIO)) {
            if (partnerActionStatus->actionState.b[0] == PARTNER_ACTION_NONE) {
                ...
                } else if (partnerActionStatus->actionState.b[3] == PARTNER_SUSHIE) {
                    sfx_play_sound(SOUND_MENU_ERROR);
                }
            }
        } else {
