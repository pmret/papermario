...
s32 draw_box(s32 flags, WindowStyleCustom* windowStyle, s32 posX, s32 posY, s32 posZ, s32 width, s32 height, u8 opacity,
    ...
    Matrix4f sp20;
    Matrix4f sp60;
    u8 spF7, spFF, sp10F, sp107, sp117, sp11F, sp127, sp12F;
    u8* sp138;
    u8* sp13C;
    s32 sp140;
    s32 sp144;
    ...
    if ((u32)windowStyle < 23) {
        ...
        spF7 = windowStyle->color1[0];
        spFF = windowStyle->color1[1];
        sp107 = windowStyle->color1[2];
        sp117 = windowStyle->color2[0];
        sp11F = windowStyle->color2[1];
        sp127 = windowStyle->color2[2];
        sp12F = windowStyle->color2[3];
        sp10F = windowStyle->color1[3] * opacity / 255;
        corners = &windowStyle->corners;
        background = &windowStyle->background;
    }

    sp13C = corners->imgData;
    sp140 = background->packedTileFormat >> 4;
    bgWidth = background->width;
    sp138 = background->imgData;
    ...
    if (bgHeight <= 1) {
        ...
        s6 = 6;
    } else if (bgHeight <= 128) {
        s6 = 7;
    } else if (bgHeight <= 256) {
        s6 = 8;
    } else if (bgHeight <= 512) {
        s6 = 9;
    } else {
        s6 = 10;
    }
    ...
    if (width <= 0 || height <= 0 || opacity == 0 || posX <= -768 || posY <= -768 || posX >= SCREEN_WIDTH || posY >= SCREEN_HEIGHT ||
        posX + width >= 768 || posY + height >= 768 || posX + width <= 0 || posY + height <= 0) {
        return TRUE;
    }

    if (flags & DRAW_FLAGS_ROTSCALE) {
        //something with D_8010BBB0, D_8010D6B0
        //sets phi_fp
    }
    ...
    if (sp138) {
        switch (background->packedTileFormat & 0xF) {
            case G_IM_SIZ_4b:
                gDPLoadTextureTile_4b(gMasterGfxPos++, sp138, sp140, bgWidth, bgHeight, 0, 0, bgWidth - 1, bgHeight - 1, 0, G_TX_NOMIRROR | G_TX_WRAP, G_TX_NOMIRROR | G_TX_WRAP, 4, 4, G_TX_NOLOD, G_TX_NOLOD);
                break;
            case G_IM_SIZ_8b:
                gDPLoadTextureTile(gMasterGfxPos++, sp138, sp140, G_IM_SIZ_8b, bgWidth, bgHeight, 0, 0, bgWidth - 1, bgHeight - 1, 0, G_TX_NOMIRROR | G_TX_WRAP, G_TX_NOMIRROR | G_TX_WRAP, 4, 4, G_TX_NOLOD, G_TX_NOLOD);
                break;
            case G_IM_SIZ_16b:
                gDPLoadTextureTile(gMasterGfxPos++, sp138, sp140, G_IM_SIZ_16b, bgWidth, bgHeight, 0, 0, bgWidth - 1, bgHeight - 1, 0, G_TX_NOMIRROR | G_TX_WRAP, G_TX_NOMIRROR | G_TX_WRAP, 4, 4, G_TX_NOLOD, G_TX_NOLOD);
                break;
            case G_IM_SIZ_32b:
                gDPLoadTextureTile(gMasterGfxPos++, sp138, sp140, G_IM_SIZ_32b, bgWidth, bgHeight, 0, 0, bgWidth - 1, bgHeight - 1, 0, G_TX_NOMIRROR | G_TX_WRAP, G_TX_NOMIRROR | G_TX_WRAP, 4, 4, G_TX_NOLOD, G_TX_NOLOD);
                break;
        }

        if (flags & DRAW_FLAGS_2) {
            s32 a3 = (gGameStatusPtr->frameCounter * 4) % 512;
            gDPSetTileSize(gMasterGfxPos++, G_TX_RENDERTILE, (width / 2 * 4) + (511 - a3), (height / 2 * 4) + a3, (width / 2 + bgWidth - 1) * 4 + (511 - a3), (height / 2 + bgHeight - 1)* 4 + a3);
        } else {
