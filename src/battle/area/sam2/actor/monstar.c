...
API_CALLABLE(func_80218000_63D1E0) {
    ...
    if (isInitialCall) {
        ...
        func_802DE780(part->spriteInstanceID, 0, IMGFX_ALLOC_COLOR_BUF, 20, 0, 0, 255, 0);
    }

    func_802DE780(part->spriteInstanceID, 1, IMGFX_OVERLAY, (s32)&N(MonstarDetailTexture), 255, 0, 255, 0);
    ...
    for (i = 0; i < 20; i++) {
        func_802DE780(part->spriteInstanceID, 0, IMGFX_COLOR_BUF_SET_C, i, colR[i] << 0x18 | colG[i] << 0x10 | colB[i] << 8 | 255, 0, 255, 0);
    }
