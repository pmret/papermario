...
EvtScript N(idle) = {
    ...
    EVT_LABEL(2)
    EVT_CALL(GetActorVar, ACTOR_SELF, 1, LocalVar(5))
    EVT_WAIT(1)
...
EvtScript N(handleEvent) = {
    ...
    EVT_SWITCH(LocalVar(0))
        ...
        EVT_CASE_OR_EQ(EVENT_HIT)
            ...
            EVT_SET_CONST(LocalVar(1), NPC_ANIM_monstar_Palette_00_Anim_8)
            EVT_EXEC_WAIT(DoNormalHit)
        EVT_END_CASE_GROUP
        EVT_CASE_EQ(EVENT_BURN_HIT)
            EVT_SET(LocalVar(0), 1)
            EVT_SET(LocalVar(1), NPC_ANIM_monstar_Palette_00_Anim_8)
        ...
        EVT_CASE_EQ(EVENT_UNKNOWN_TRIGGER)
            ...
            EVT_SWITCH(LocalVar(0))
                EVT_CASE_EQ(0)
                    EVT_SET_CONST(LocalVar(0), 1)
                    EVT_SET_CONST(LocalVar(1), NPC_ANIM_monstar_Palette_00_Anim_1)
                EVT_CASE_EQ(1)
                    EVT_SET_CONST(LocalVar(0), 1)
                    EVT_SET_CONST(LocalVar(1), NPC_ANIM_monstar_Palette_00_Anim_4)
            EVT_END_SWITCH
...
EvtScript N(unused) = {
    ...
    EVT_CALL(PlayEffect, EFFECT_RADIAL_SHIMMER, 10, LocalVar(0), LocalVar(1), LocalVar(2), EVT_FLOAT(1.0), 300, 0, 0, 0, 0, 0, 0, 0)
    EVT_WAIT(75)
...
EvtScript N(attack) = {
    ...
    EVT_THREAD
        EVT_WAIT(10)
        EVT_LOOP(18)
            EVT_CALL(RandInt, 150, LocalVar(0))
            EVT_ADD(LocalVar(0), 100)
            EVT_CALL(N(StartRumbleWithParams), LocalVar(0), 20)
            EVT_WAIT(10)
        EVT_END_LOOP
    EVT_END_THREAD
    EVT_THREAD
        EVT_LOOP(60)
            ...
            EVT_CALL(RandInt, 400, LocalVar(1))
            EVT_SUB(LocalVar(1), 100)
            EVT_CALL(RandInt, 100, LocalVar(2))
            EVT_SUB(LocalVar(2), 50)
            EVT_SET(LocalVar(3), LocalVar(1))
            EVT_SUB(LocalVar(3), 200)
            EVT_SET(LocalVar(4), LocalVar(2))
            EVT_CALL(RandInt, 10, LocalVar(5))
            EVT_ADD(LocalVar(5), 10)
            EVT_CALL(PlayEffect, EFFECT_STAR, LocalVar(0), LocalVar(1), 240, LocalVar(2), LocalVar(3), 0, LocalVar(4), LocalVar(5), 0, 0, 0, 0, 0)
            EVT_WAIT(3)
        EVT_END_LOOP
    EVT_END_THREAD
    EVT_WAIT(8)
    ...
    EVT_CALL(PlayEffect, EFFECT_RADIAL_SHIMMER, 11, LocalVar(0), LocalVar(1), LocalVar(2), EVT_FLOAT(0.8), 160, 0, 0, 0, 0, 0, 0, 0)
    EVT_CALL(SetAnimation, ACTOR_SELF, 1, NPC_ANIM_monstar_Palette_00_Anim_7)
    EVT_CALL(EnemyTestTarget, ACTOR_SELF, LocalVar(0), 0, 0, 0, BS_FLAGS1_10)
    EVT_SWITCH(LocalVar(0))
    ...
    EVT_CALL(EnemyDamageTarget, ACTOR_SELF, LocalVar(0), DAMAGE_TYPE_COSMIC | DAMAGE_TYPE_NO_CONTACT, 0, 0, 1, BS_FLAGS1_SP_EVT_ACTIVE)
    EVT_SWITCH(LocalVar(0))
...
EvtScript N(OnBurn) = {
    ...
    EVT_SWITCH(LocalVar(1))
        EVT_CASE_EQ(EVENT_BURN_HIT)
            EVT_CALL(GetPartEventFlags, ACTOR_SELF, LocalVar(0), LocalVar(1))
            EVT_IF_NOT_FLAG(LocalVar(1), ACTOR_EVENT_FLAG_FIREY | ACTOR_EVENT_FLAG_EXPLOSIVE)
                EVT_CALL(GetActorPos, ACTOR_SELF, LocalVar(0), LocalVar(1), LocalVar(2))
