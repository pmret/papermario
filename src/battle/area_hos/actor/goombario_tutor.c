...
EvtScript N(takeTurn_80219444) = {
    ...
    EVT_CALL(GetBattleFlags2, LocalVar(0))
...
EvtScript N(80219C74) = {
    ...
    EVT_LOOP(0)
        EVT_CALL(CheckButtonDown, 32768, LocalVar(0))
        EVT_IF_EQ(LocalVar(0), 0)
            EVT_BREAK_LOOP
        EVT_END_IF
        EVT_WAIT(1)
    EVT_END_LOOP
    EVT_LOOP(0)
        EVT_CALL(CheckButtonPress, 32768, LocalVar(0))
        EVT_IF_EQ(LocalVar(0), 1)
            EVT_BREAK_LOOP
        EVT_END_IF
        EVT_WAIT(1)
    ...
    EVT_END_LOOP
    EVT_LOOP(0)
        EVT_CALL(CheckButtonPress, 262144, LocalVar(0))
        EVT_IF_EQ(LocalVar(0), 1)
            EVT_BREAK_LOOP
        EVT_END_IF
        EVT_WAIT(1)
    ...
    EVT_IF_EQ(LocalVar(0), 7)
        EVT_GOTO(99)
    EVT_END_IF
    EVT_CALL(SetActionCommandMode, 1)
    EVT_CALL(WaitForState, 8)
    EVT_CALL(UseIdleAnimation, ACTOR_PARTNER, FALSE)
    EVT_CALL(func_80269524, LocalVar(0))
    EVT_SWITCH(LocalVar(0))
        EVT_CASE_EQ(-1)
    ...
    EVT_CALL(WaitForState, 6)
    EVT_CALL(UseIdleAnimation, ACTOR_PARTNER, FALSE)
    EVT_CALL(GetBlockResult, LocalVar(0))
    EVT_SWITCH(LocalVar(0))
        EVT_CASE_EQ(-1)
            EVT_CALL(ActorSpeak, 655402, 256, 1, 2097161, 2097157)
        EVT_CASE_EQ(0)
