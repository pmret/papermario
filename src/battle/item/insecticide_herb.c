...
EvtScript script = {
    EVT_SET_CONST(LocalVar(10), 0x00000087)
    ...
    EVT_CALL(PlayEffect, 0x7, 3, LocalVar(0), LocalVar(1), LocalVar(2), 0, 0, 0, 0, 0, 0, 0, 0, 0)
    EVT_THREAD
        EVT_CALL(N(func_802A12E0_72AA30), LocalVar(0), LocalVar(1), LocalVar(2))
        EVT_WAIT(3)
        EVT_CALL(N(func_802A12E0_72AA30), LocalVar(0), LocalVar(1), LocalVar(2))
        EVT_WAIT(3)
        EVT_CALL(N(func_802A12E0_72AA30), LocalVar(0), LocalVar(1), LocalVar(2))
    ...
    EVT_IF_EQ(LocalVar(9), 0)
        EVT_CALL(ItemDamageEnemy, LocalVar(0), 0, 0, 0, 32)
    EVT_ELSE
        EVT_CALL(InitTargetIterator)
        EVT_CALL(SetGoalToTarget, ACTOR_SELF)
