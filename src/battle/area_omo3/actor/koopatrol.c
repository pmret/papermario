...
#define NAMESPACE b_area_omo3_koopatrol
...
EvtScript N(attackShellMissile) = {
    ...
    EVT_CALL(RunToGoal, ACTOR_SELF, 0, FALSE)
    EVT_WAIT(2)
    EVT_CALL(EnemyDamageTarget, ACTOR_SELF, LVarA, 0, 65535, 0, 10, BS_FLAGS1_SP_EVT_ACTIVE)
    EVT_SWITCH(LVarA)
        EVT_CASE_OR_EQ(HIT_RESULT_HIT)
        EVT_CASE_OR_EQ(HIT_RESULT_QUAKE_IMMUNE)
            EVT_CALL(UseBattleCamPreset, BTL_CAM_PRESET_C)
...
EvtScript N(summonFriend) = {
    ...
    EVT_CALL(SetActorSounds, LVar0, 0, 0, 0)
