...
void entity_BlueWarpPipe_enter_pipe_init(Entity* bluePipe) {
    ...
    spr_set_player_fold_update_all(ANIM_Mario1_Idle, FOLD_UPD_SET_ANIM, FOLD_ANIM_VERTICAL_PIPE_CURL, 1, 1, 0, 0);
...
void entity_BlueWarpPipe_enter_pipe_update(Entity* entity) {
    ...
    if (pipeData->timer == -1) {
        ...
        spr_set_player_fold_update_all(ANIM_Mario1_Idle, FOLD_UPD_CLEAR, 0, 0, 0, 0, 0);
        exec_entity_commandlist(entity);
    }
}
