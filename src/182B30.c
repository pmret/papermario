...
void func_8025C8A0(s32 isNpcSprite, ActorPart* part, s32 yaw, s32 arg3) {
    if (part->decorationTable->unk_751 != 0) {
        ...
        if (isNpcSprite == SPRITE_MODE_PLAYER) {
            spr_set_player_imgfx_update_all(PLAYER_SPRITE_MAIN, IMGFX_CLEAR, 0, 0, 0, 0, 0);
        } else {
...
void func_8025C918(s32 isNpcSprite, ActorPart* part, s32 yaw, s32 arg3) {
    ...
    if (decor->unk_751 != 0) {
        ...
        decor->unk758 = 0;
        if (isNpcSprite == SPRITE_MODE_PLAYER) {
            spr_set_player_imgfx_update_all(PLAYER_SPRITE_MAIN, IMGFX_ALLOC_COLOR_BUF, 20, 0, 0, 255, 0);
        } else {
            set_npc_all_imgfx(part->spriteInstanceID, IMGFX_ALLOC_COLOR_BUF, 20, 0, 0, 255, 0);
        }
    }
    ...
    for (i = 0; i < ARRAY_COUNT(rbuf); i++) {
        ...
        if (isNpcSprite == SPRITE_MODE_PLAYER) {
            spr_set_player_imgfx_update_all(PLAYER_SPRITE_MAIN, IMGFX_COLOR_BUF_SET_MODULATE, i, color, 0, 255, 0);
        } else {
            set_npc_all_imgfx(part->spriteInstanceID, IMGFX_COLOR_BUF_SET_MODULATE, i, color, 0, 255, 0);
        }
    }
