...
void btl_state_update_begin_partner_turn(void) {
    ...
    if (gBattleState2 == 0) {
        if (partner == NULL) {
            D_8029F254 = 1;
            gBattleState2 = 0xA;
        } else if (((gBattleStatus.flags2 & 6) == 6) || (partner->flags & 0x200000)) {
            btl_set_state(9);
            return;
        } else {
            func_8024E40C(2);
...
block_27:
    ...
    if (gBattleState2 == 0xA) {
        ...
        gBattleStatus.flags2 &= 0xFFEFFFFF;
        if (D_8029F254 == 0) {
            btl_set_state(0xB);
        } else {
            gBattleStatus.flags2 |= 4;
