
.section .text80028EE0, "ax"

func_80028EE0:
/* 0042E0 80028EE0 27BDFFE8 */  addiu $sp, $sp, -0x18
/* 0042E4 80028EE4 AFBF0010 */  sw    $ra, 0x10($sp)
/* 0042E8 80028EE8 0C017FBC */  jal   func_8005FEF0
/* 0042EC 80028EEC 0000202D */   move  $a0, $zero
/* 0042F0 80028EF0 0000202D */  move  $a0, $zero
/* 0042F4 80028EF4 0C017FE4 */  jal   func_8005FF90
/* 0042F8 80028EF8 24050002 */   addiu $a1, $zero, 2
/* 0042FC 80028EFC 8FBF0010 */  lw    $ra, 0x10($sp)
/* 004300 80028F00 03E00008 */  jr    $ra
/* 004304 80028F04 27BD0018 */   addiu $sp, $sp, 0x18

func_80028F08:
/* 004308 80028F08 3C028007 */  lui   $v0, 0x8007
/* 00430C 80028F0C 8C42419C */  lw    $v0, 0x419c($v0)
/* 004310 80028F10 27BDFFE0 */  addiu $sp, $sp, -0x20
/* 004314 80028F14 AFB10014 */  sw    $s1, 0x14($sp)
/* 004318 80028F18 0080882D */  move  $s1, $a0
/* 00431C 80028F1C AFBF0018 */  sw    $ra, 0x18($sp)
/* 004320 80028F20 AFB00010 */  sw    $s0, 0x10($sp)
/* 004324 80028F24 80420071 */  lb    $v0, 0x71($v0)
/* 004328 80028F28 14400013 */  bnez  $v0, .L80028F78
/* 00432C 80028F2C 00A0802D */   move  $s0, $a1
/* 004330 80028F30 3C038007 */  lui   $v1, 0x8007
/* 004334 80028F34 24634260 */  addiu $v1, $v1, 0x4260
/* 004338 80028F38 94620000 */  lhu   $v0, ($v1)
/* 00433C 80028F3C 1040000E */  beqz  $v0, .L80028F78
/* 004340 80028F40 00021840 */   sll   $v1, $v0, 1
/* 004344 80028F44 0070102A */  slt   $v0, $v1, $s0
/* 004348 80028F48 54400001 */  bnezl $v0, .L80028F50
/* 00434C 80028F4C 0060802D */   move  $s0, $v1
.L80028F50:
/* 004350 80028F50 0C017FBC */  jal   func_8005FEF0
/* 004354 80028F54 0000202D */   move  $a0, $zero
/* 004358 80028F58 14400007 */  bnez  $v0, .L80028F78
/* 00435C 80028F5C 0000202D */   move  $a0, $zero
/* 004360 80028F60 0C017FE4 */  jal   func_8005FF90
/* 004364 80028F64 24050002 */   addiu $a1, $zero, 2
/* 004368 80028F68 0000202D */  move  $a0, $zero
/* 00436C 80028F6C 3225FFFF */  andi  $a1, $s1, 0xffff
/* 004370 80028F70 0C017FA4 */  jal   func_8005FE90
/* 004374 80028F74 3206FFFF */   andi  $a2, $s0, 0xffff
.L80028F78:
/* 004378 80028F78 8FBF0018 */  lw    $ra, 0x18($sp)
/* 00437C 80028F7C 8FB10014 */  lw    $s1, 0x14($sp)
/* 004380 80028F80 8FB00010 */  lw    $s0, 0x10($sp)
/* 004384 80028F84 03E00008 */  jr    $ra
/* 004388 80028F88 27BD0020 */   addiu $sp, $sp, 0x20

func_80028F8C:
/* 00438C 80028F8C 27BDFFE8 */  addiu $sp, $sp, -0x18
/* 004390 80028F90 3C028007 */  lui   $v0, 0x8007
/* 004394 80028F94 8C42419C */  lw    $v0, 0x419c($v0)
/* 004398 80028F98 3C048007 */  lui   $a0, 0x8007
/* 00439C 80028F9C 24844264 */  addiu $a0, $a0, 0x4264
/* 0043A0 80028FA0 AFBF0010 */  sw    $ra, 0x10($sp)
/* 0043A4 80028FA4 8C830000 */  lw    $v1, ($a0)
/* 0043A8 80028FA8 8C420000 */  lw    $v0, ($v0)
/* 0043AC 80028FAC 10620003 */  beq   $v1, $v0, .L80028FBC
/* 0043B0 80028FB0 00000000 */   nop   
/* 0043B4 80028FB4 0C00A3F8 */  jal   func_80028FE0
/* 0043B8 80028FB8 AC820000 */   sw    $v0, ($a0)
.L80028FBC:
/* 0043BC 80028FBC 3C038007 */  lui   $v1, 0x8007
/* 0043C0 80028FC0 24634260 */  addiu $v1, $v1, 0x4260
/* 0043C4 80028FC4 94620000 */  lhu   $v0, ($v1)
/* 0043C8 80028FC8 10400002 */  beqz  $v0, .L80028FD4
/* 0043CC 80028FCC 2442FFFF */   addiu $v0, $v0, -1
/* 0043D0 80028FD0 A4620000 */  sh    $v0, ($v1)
.L80028FD4:
/* 0043D4 80028FD4 8FBF0010 */  lw    $ra, 0x10($sp)
/* 0043D8 80028FD8 03E00008 */  jr    $ra
/* 0043DC 80028FDC 27BD0018 */   addiu $sp, $sp, 0x18

func_80028FE0:
/* 0043E0 80028FE0 2402012C */  addiu $v0, $zero, 0x12c
/* 0043E4 80028FE4 3C018007 */  lui   $at, 0x8007
/* 0043E8 80028FE8 03E00008 */  jr    $ra
/* 0043EC 80028FEC A4224260 */   sh    $v0, 0x4260($at)

/* 0043F0 80028FF0 460C6302 */  mul.s $f12, $f12, $f12
/* 0043F4 80028FF4 00000000 */  nop   
/* 0043F8 80028FF8 460E7382 */  mul.s $f14, $f14, $f14
/* 0043FC 80028FFC 00000000 */  nop   
/* 004400 80029000 460E6300 */  add.s $f12, $f12, $f14
/* 004404 80029004 27BDFFE8 */  addiu $sp, $sp, -0x18
/* 004408 80029008 46006004 */  sqrt.s $f0, $f12
/* 00440C 8002900C 46000032 */  c.eq.s $f0, $f0
/* 004410 80029010 00000000 */  nop   
/* 004414 80029014 45010003 */  bc1t  .L80029024
/* 004418 80029018 AFBF0010 */   sw    $ra, 0x10($sp)
/* 00441C 8002901C 0C0187BC */  jal   func_80061EF0
/* 004420 80029020 00000000 */   nop   
.L80029024:
/* 004424 80029024 8FBF0010 */  lw    $ra, 0x10($sp)
/* 004428 80029028 03E00008 */  jr    $ra
/* 00442C 8002902C 27BD0018 */   addiu $sp, $sp, 0x18

func_80029030:
/* 004430 80029030 2CA20020 */  sltiu $v0, $a1, 0x20
/* 004434 80029034 1440000B */  bnez  $v0, .L80029064
/* 004438 80029038 2482000F */   addiu $v0, $a0, 0xf
/* 00443C 8002903C 2403FFF0 */  addiu $v1, $zero, -0x10
/* 004440 80029040 00431024 */  and   $v0, $v0, $v1
/* 004444 80029044 00441823 */  subu  $v1, $v0, $a0
/* 004448 80029048 00A32823 */  subu  $a1, $a1, $v1
/* 00444C 8002904C 24A3FFF0 */  addiu $v1, $a1, -0x10
/* 004450 80029050 AC400000 */  sw    $zero, ($v0)
/* 004454 80029054 AC430004 */  sw    $v1, 4($v0)
/* 004458 80029058 A4400008 */  sh    $zero, 8($v0)
/* 00445C 8002905C 03E00008 */  jr    $ra
/* 004460 80029060 AC45000C */   sw    $a1, 0xc($v0)

.L80029064:
/* 004464 80029064 03E00008 */  jr    $ra
/* 004468 80029068 2402FFFF */   addiu $v0, $zero, -1

func_8002906C:
/* 00446C 8002906C 24A5000F */  addiu $a1, $a1, 0xf
/* 004470 80029070 2402FFF0 */  addiu $v0, $zero, -0x10
/* 004474 80029074 00A22824 */  and   $a1, $a1, $v0
/* 004478 80029078 14A00003 */  bnez  $a1, .L80029088
/* 00447C 8002907C 0000382D */   move  $a3, $zero
/* 004480 80029080 03E00008 */  jr    $ra
/* 004484 80029084 00E0102D */   move  $v0, $a3

.L80029088:
/* 004488 80029088 0000402D */  move  $t0, $zero
/* 00448C 8002908C 0100502D */  move  $t2, $t0
/* 004490 80029090 0080302D */  move  $a2, $a0
.L80029094:
/* 004494 80029094 94C20008 */  lhu   $v0, 8($a2)
/* 004498 80029098 1440000C */  bnez  $v0, .L800290CC
/* 00449C 8002909C 00000000 */   nop   
/* 0044A0 800290A0 8CC30004 */  lw    $v1, 4($a2)
/* 0044A4 800290A4 0065102B */  sltu  $v0, $v1, $a1
/* 0044A8 800290A8 14400008 */  bnez  $v0, .L800290CC
/* 0044AC 800290AC 0068102B */   sltu  $v0, $v1, $t0
/* 0044B0 800290B0 54400004 */  bnezl $v0, .L800290C4
/* 0044B4 800290B4 00C0382D */   move  $a3, $a2
/* 0044B8 800290B8 15000004 */  bnez  $t0, .L800290CC
/* 0044BC 800290BC 00000000 */   nop   
/* 0044C0 800290C0 00C0382D */  move  $a3, $a2
.L800290C4:
/* 0044C4 800290C4 0060402D */  move  $t0, $v1
/* 0044C8 800290C8 8CEA0000 */  lw    $t2, ($a3)
.L800290CC:
/* 0044CC 800290CC 8CC40000 */  lw    $a0, ($a2)
/* 0044D0 800290D0 5480FFF0 */  bnezl $a0, .L80029094
/* 0044D4 800290D4 0080302D */   move  $a2, $a0
/* 0044D8 800290D8 11000020 */  beqz  $t0, .L8002915C
/* 0044DC 800290DC 24A90010 */   addiu $t1, $a1, 0x10
/* 0044E0 800290E0 0109102B */  sltu  $v0, $t0, $t1
/* 0044E4 800290E4 54400012 */  bnezl $v0, .L80029130
/* 0044E8 800290E8 ACEA0000 */   sw    $t2, ($a3)
/* 0044EC 800290EC 00E91021 */  addu  $v0, $a3, $t1
/* 0044F0 800290F0 ACE20000 */  sw    $v0, ($a3)
/* 0044F4 800290F4 0040302D */  move  $a2, $v0
/* 0044F8 800290F8 3C038007 */  lui   $v1, 0x8007
/* 0044FC 800290FC 24634270 */  addiu $v1, $v1, 0x4270
/* 004500 80029100 ACE50004 */  sw    $a1, 4($a3)
/* 004504 80029104 94640000 */  lhu   $a0, ($v1)
/* 004508 80029108 24020001 */  addiu $v0, $zero, 1
/* 00450C 8002910C A4E20008 */  sh    $v0, 8($a3)
/* 004510 80029110 24820001 */  addiu $v0, $a0, 1
/* 004514 80029114 A4620000 */  sh    $v0, ($v1)
/* 004518 80029118 01091023 */  subu  $v0, $t0, $t1
/* 00451C 8002911C A4E4000A */  sh    $a0, 0xa($a3)
/* 004520 80029120 ACCA0000 */  sw    $t2, ($a2)
/* 004524 80029124 ACC20004 */  sw    $v0, 4($a2)
/* 004528 80029128 0800A455 */  j     func_80029154
/* 00452C 8002912C A4C00008 */   sh    $zero, 8($a2)

.L80029130:
/* 004530 80029130 3C038007 */  lui   $v1, 0x8007
/* 004534 80029134 24634270 */  addiu $v1, $v1, 0x4270
/* 004538 80029138 ACE80004 */  sw    $t0, 4($a3)
/* 00453C 8002913C 94640000 */  lhu   $a0, ($v1)
/* 004540 80029140 24020001 */  addiu $v0, $zero, 1
/* 004544 80029144 A4E20008 */  sh    $v0, 8($a3)
/* 004548 80029148 24820001 */  addiu $v0, $a0, 1
/* 00454C 8002914C A4620000 */  sh    $v0, ($v1)
/* 004550 80029150 A4E4000A */  sh    $a0, 0xa($a3)
func_80029154:
/* 004554 80029154 03E00008 */  jr    $ra
/* 004558 80029158 24E20010 */   addiu $v0, $a3, 0x10

.L8002915C:
/* 00455C 8002915C 03E00008 */  jr    $ra
/* 004560 80029160 0000102D */   move  $v0, $zero

func_80029164:
/* 004564 80029164 24A5000F */  addiu $a1, $a1, 0xf
/* 004568 80029168 2402FFF0 */  addiu $v0, $zero, -0x10
/* 00456C 8002916C 00A22824 */  and   $a1, $a1, $v0
/* 004570 80029170 14A00003 */  bnez  $a1, .L80029180
/* 004574 80029174 0000402D */   move  $t0, $zero
/* 004578 80029178 03E00008 */  jr    $ra
/* 00457C 8002917C 0100102D */   move  $v0, $t0

.L80029180:
/* 004580 80029180 0000302D */  move  $a2, $zero
/* 004584 80029184 00C0382D */  move  $a3, $a2
.L80029188:
/* 004588 80029188 94820008 */  lhu   $v0, 8($a0)
/* 00458C 8002918C 14400008 */  bnez  $v0, .L800291B0
/* 004590 80029190 00000000 */   nop   
/* 004594 80029194 8C830004 */  lw    $v1, 4($a0)
/* 004598 80029198 0065102B */  sltu  $v0, $v1, $a1
/* 00459C 8002919C 14400004 */  bnez  $v0, .L800291B0
/* 0045A0 800291A0 00000000 */   nop   
/* 0045A4 800291A4 0080402D */  move  $t0, $a0
/* 0045A8 800291A8 0060302D */  move  $a2, $v1
/* 0045AC 800291AC 8C870000 */  lw    $a3, ($a0)
.L800291B0:
/* 0045B0 800291B0 8C840000 */  lw    $a0, ($a0)
/* 0045B4 800291B4 1480FFF4 */  bnez  $a0, .L80029188
/* 0045B8 800291B8 00000000 */   nop   
/* 0045BC 800291BC 10C00015 */  beqz  $a2, .L80029214
/* 0045C0 800291C0 24A30010 */   addiu $v1, $a1, 0x10
/* 0045C4 800291C4 00C3102B */  sltu  $v0, $a2, $v1
/* 0045C8 800291C8 1440000C */  bnez  $v0, .L800291FC
/* 0045CC 800291CC 0100202D */   move  $a0, $t0
/* 0045D0 800291D0 00861021 */  addu  $v0, $a0, $a2
/* 0045D4 800291D4 00451023 */  subu  $v0, $v0, $a1
/* 0045D8 800291D8 AC820000 */  sw    $v0, ($a0)
/* 0045DC 800291DC 00C31023 */  subu  $v0, $a2, $v1
/* 0045E0 800291E0 AC820004 */  sw    $v0, 4($a0)
/* 0045E4 800291E4 A4800008 */  sh    $zero, 8($a0)
/* 0045E8 800291E8 8C840000 */  lw    $a0, ($a0)
/* 0045EC 800291EC 24020001 */  addiu $v0, $zero, 1
/* 0045F0 800291F0 AC870000 */  sw    $a3, ($a0)
/* 0045F4 800291F4 0800A482 */  j     func_80029208
/* 0045F8 800291F8 AC850004 */   sw    $a1, 4($a0)

.L800291FC:
/* 0045FC 800291FC 24020001 */  addiu $v0, $zero, 1
/* 004600 80029200 AC870000 */  sw    $a3, ($a0)
/* 004604 80029204 AC860004 */  sw    $a2, 4($a0)
func_80029208:
/* 004608 80029208 A4820008 */  sh    $v0, 8($a0)
/* 00460C 8002920C 03E00008 */  jr    $ra
/* 004610 80029210 24820010 */   addiu $v0, $a0, 0x10

.L80029214:
/* 004614 80029214 03E00008 */  jr    $ra
/* 004618 80029218 0000102D */   move  $v0, $zero

func_8002921C:
/* 00461C 8002921C 14A00003 */  bnez  $a1, .L8002922C
/* 004620 80029220 00000000 */   nop   
.L80029224:
/* 004624 80029224 03E00008 */  jr    $ra
/* 004628 80029228 24020001 */   addiu $v0, $zero, 1

.L8002922C:
/* 00462C 8002922C 94A2FFF8 */  lhu   $v0, -8($a1)
/* 004630 80029230 1040FFFC */  beqz  $v0, .L80029224
/* 004634 80029234 24A6FFF0 */   addiu $a2, $a1, -0x10
/* 004638 80029238 8CA7FFF0 */  lw    $a3, -0x10($a1)
/* 00463C 8002923C 8CA5FFF4 */  lw    $a1, -0xc($a1)
/* 004640 80029240 00E0182D */  move  $v1, $a3
/* 004644 80029244 50600009 */  beql  $v1, $zero, .L8002926C
/* 004648 80029248 0080182D */   move  $v1, $a0
/* 00464C 8002924C 94620008 */  lhu   $v0, 8($v1)
/* 004650 80029250 54400006 */  bnezl $v0, .L8002926C
/* 004654 80029254 0080182D */   move  $v1, $a0
/* 004658 80029258 8C670000 */  lw    $a3, ($v1)
/* 00465C 8002925C 8C630004 */  lw    $v1, 4($v1)
/* 004660 80029260 24A20010 */  addiu $v0, $a1, 0x10
/* 004664 80029264 00432821 */  addu  $a1, $v0, $v1
/* 004668 80029268 0080182D */  move  $v1, $a0
func_8002926C:
.L8002926C:
/* 00466C 8002926C 8C640000 */  lw    $a0, ($v1)
/* 004670 80029270 10860007 */  beq   $a0, $a2, .L80029290
/* 004674 80029274 00C3102B */   sltu  $v0, $a2, $v1
/* 004678 80029278 1440000D */  bnez  $v0, .L800292B0
/* 00467C 8002927C 00C0182D */   move  $v1, $a2
/* 004680 80029280 1080000A */  beqz  $a0, .L800292AC
/* 004684 80029284 0080182D */   move  $v1, $a0
/* 004688 80029288 0800A49B */  j     func_8002926C
/* 00468C 8002928C 00000000 */   nop   

.L80029290:
/* 004690 80029290 94620008 */  lhu   $v0, 8($v1)
/* 004694 80029294 54400006 */  bnezl $v0, .L800292B0
/* 004698 80029298 00C0182D */   move  $v1, $a2
/* 00469C 8002929C 0060302D */  move  $a2, $v1
/* 0046A0 800292A0 8CC30004 */  lw    $v1, 4($a2)
/* 0046A4 800292A4 24A20010 */  addiu $v0, $a1, 0x10
/* 0046A8 800292A8 00432821 */  addu  $a1, $v0, $v1
.L800292AC:
/* 0046AC 800292AC 00C0182D */  move  $v1, $a2
.L800292B0:
/* 0046B0 800292B0 0000102D */  move  $v0, $zero
/* 0046B4 800292B4 AC670000 */  sw    $a3, ($v1)
/* 0046B8 800292B8 AC650004 */  sw    $a1, 4($v1)
/* 0046BC 800292BC 03E00008 */  jr    $ra
/* 0046C0 800292C0 A4600008 */   sh    $zero, 8($v1)

func_800292C4:
/* 0046C4 800292C4 27BDFFD8 */  addiu $sp, $sp, -0x28
/* 0046C8 800292C8 AFB3001C */  sw    $s3, 0x1c($sp)
/* 0046CC 800292CC 0080982D */  move  $s3, $a0
/* 0046D0 800292D0 AFB20018 */  sw    $s2, 0x18($sp)
/* 0046D4 800292D4 00A0902D */  move  $s2, $a1
/* 0046D8 800292D8 AFB10014 */  sw    $s1, 0x14($sp)
/* 0046DC 800292DC 2651FFF0 */  addiu $s1, $s2, -0x10
/* 0046E0 800292E0 24C6000F */  addiu $a2, $a2, 0xf
/* 0046E4 800292E4 2402FFF0 */  addiu $v0, $zero, -0x10
/* 0046E8 800292E8 AFBF0020 */  sw    $ra, 0x20($sp)
/* 0046EC 800292EC AFB00010 */  sw    $s0, 0x10($sp)
/* 0046F0 800292F0 9643FFF8 */  lhu   $v1, -8($s2)
/* 0046F4 800292F4 10600016 */  beqz  $v1, .L80029350
/* 0046F8 800292F8 00C28024 */   and   $s0, $a2, $v0
/* 0046FC 800292FC 8E45FFF0 */  lw    $a1, -0x10($s2)
/* 004700 80029300 8E44FFF4 */  lw    $a0, -0xc($s2)
/* 004704 80029304 00A0182D */  move  $v1, $a1
/* 004708 80029308 10600009 */  beqz  $v1, .L80029330
/* 00470C 8002930C 0090102B */   sltu  $v0, $a0, $s0
/* 004710 80029310 94620008 */  lhu   $v0, 8($v1)
/* 004714 80029314 14400006 */  bnez  $v0, .L80029330
/* 004718 80029318 0090102B */   sltu  $v0, $a0, $s0
/* 00471C 8002931C 8C650000 */  lw    $a1, ($v1)
/* 004720 80029320 8C630004 */  lw    $v1, 4($v1)
/* 004724 80029324 24820010 */  addiu $v0, $a0, 0x10
/* 004728 80029328 00432021 */  addu  $a0, $v0, $v1
/* 00472C 8002932C 0090102B */  sltu  $v0, $a0, $s0
.L80029330:
/* 004730 80029330 10400011 */  beqz  $v0, .L80029378
/* 004734 80029334 0220182D */   move  $v1, $s1
/* 004738 80029338 0260202D */  move  $a0, $s3
/* 00473C 8002933C 0C00A41B */  jal   func_8002906C
/* 004740 80029340 0200282D */   move  $a1, $s0
/* 004744 80029344 0040882D */  move  $s1, $v0
/* 004748 80029348 16200003 */  bnez  $s1, .L80029358
/* 00474C 8002934C 0220202D */   move  $a0, $s1
.L80029350:
/* 004750 80029350 0800A4EE */  j     func_800293B8
/* 004754 80029354 0000102D */   move  $v0, $zero

.L80029358:
/* 004758 80029358 0240282D */  move  $a1, $s2
/* 00475C 8002935C 0C01929D */  jal   func_80064A74
/* 004760 80029360 0200302D */   move  $a2, $s0
/* 004764 80029364 0260202D */  move  $a0, $s3
/* 004768 80029368 0C00A487 */  jal   func_8002921C
/* 00476C 8002936C 0240282D */   move  $a1, $s2
/* 004770 80029370 0800A4EE */  j     func_800293B8
/* 004774 80029374 0220102D */   move  $v0, $s1

.L80029378:
/* 004778 80029378 26020010 */  addiu $v0, $s0, 0x10
/* 00477C 8002937C 0044102B */  sltu  $v0, $v0, $a0
/* 004780 80029380 1040000A */  beqz  $v0, .L800293AC
/* 004784 80029384 02501021 */   addu  $v0, $s2, $s0
/* 004788 80029388 AC620000 */  sw    $v0, ($v1)
/* 00478C 8002938C AC700004 */  sw    $s0, 4($v1)
/* 004790 80029390 0040182D */  move  $v1, $v0
/* 004794 80029394 00901023 */  subu  $v0, $a0, $s0
/* 004798 80029398 2442FFF0 */  addiu $v0, $v0, -0x10
/* 00479C 8002939C AC650000 */  sw    $a1, ($v1)
/* 0047A0 800293A0 AC620004 */  sw    $v0, 4($v1)
/* 0047A4 800293A4 0800A4ED */  j     func_800293B4
/* 0047A8 800293A8 A4600008 */   sh    $zero, 8($v1)

.L800293AC:
/* 0047AC 800293AC AC650000 */  sw    $a1, ($v1)
/* 0047B0 800293B0 AC640004 */  sw    $a0, 4($v1)
func_800293B4:
/* 0047B4 800293B4 0240102D */  move  $v0, $s2
func_800293B8:
/* 0047B8 800293B8 8FBF0020 */  lw    $ra, 0x20($sp)
/* 0047BC 800293BC 8FB3001C */  lw    $s3, 0x1c($sp)
/* 0047C0 800293C0 8FB20018 */  lw    $s2, 0x18($sp)
/* 0047C4 800293C4 8FB10014 */  lw    $s1, 0x14($sp)
/* 0047C8 800293C8 8FB00010 */  lw    $s0, 0x10($sp)
/* 0047CC 800293CC 03E00008 */  jr    $ra
/* 0047D0 800293D0 27BD0028 */   addiu $sp, $sp, 0x28

func_800293D4:
/* 0047D4 800293D4 3C06B60B */  lui   $a2, 0xb60b
/* 0047D8 800293D8 34C660B7 */  ori   $a2, $a2, 0x60b7
/* 0047DC 800293DC 00042400 */  sll   $a0, $a0, 0x10
/* 0047E0 800293E0 00042C03 */  sra   $a1, $a0, 0x10
/* 0047E4 800293E4 00A60018 */  mult  $a1, $a2
/* 0047E8 800293E8 000427C3 */  sra   $a0, $a0, 0x1f
/* 0047EC 800293EC 00003810 */  mfhi  $a3
/* 0047F0 800293F0 00E51021 */  addu  $v0, $a3, $a1
/* 0047F4 800293F4 00021203 */  sra   $v0, $v0, 8
/* 0047F8 800293F8 00441023 */  subu  $v0, $v0, $a0
/* 0047FC 800293FC 00021840 */  sll   $v1, $v0, 1
/* 004800 80029400 00621821 */  addu  $v1, $v1, $v0
/* 004804 80029404 00031100 */  sll   $v0, $v1, 4
/* 004808 80029408 00431023 */  subu  $v0, $v0, $v1
/* 00480C 8002940C 000210C0 */  sll   $v0, $v0, 3
/* 004810 80029410 00A22823 */  subu  $a1, $a1, $v0
/* 004814 80029414 00051400 */  sll   $v0, $a1, 0x10
/* 004818 80029418 04410002 */  bgez  $v0, .L80029424
/* 00481C 8002941C 00A0182D */   move  $v1, $a1
/* 004820 80029420 24A30168 */  addiu $v1, $a1, 0x168
.L80029424:
/* 004824 80029424 00031C00 */  sll   $v1, $v1, 0x10
/* 004828 80029428 00032C03 */  sra   $a1, $v1, 0x10
/* 00482C 8002942C 00A60018 */  mult  $a1, $a2
/* 004830 80029430 00031FC3 */  sra   $v1, $v1, 0x1f
/* 004834 80029434 00003810 */  mfhi  $a3
/* 004838 80029438 00E51021 */  addu  $v0, $a3, $a1
/* 00483C 8002943C 000211C3 */  sra   $v0, $v0, 7
/* 004840 80029440 00431023 */  subu  $v0, $v0, $v1
/* 004844 80029444 00021840 */  sll   $v1, $v0, 1
/* 004848 80029448 00621821 */  addu  $v1, $v1, $v0
/* 00484C 8002944C 00031100 */  sll   $v0, $v1, 4
/* 004850 80029450 00431023 */  subu  $v0, $v0, $v1
/* 004854 80029454 00021080 */  sll   $v0, $v0, 2
/* 004858 80029458 00A21823 */  subu  $v1, $a1, $v0
/* 00485C 8002945C 00031400 */  sll   $v0, $v1, 0x10
/* 004860 80029460 00021403 */  sra   $v0, $v0, 0x10
/* 004864 80029464 2842005B */  slti  $v0, $v0, 0x5b
/* 004868 80029468 14400003 */  bnez  $v0, .L80029478
/* 00486C 8002946C 0060202D */   move  $a0, $v1
/* 004870 80029470 240200B4 */  addiu $v0, $zero, 0xb4
/* 004874 80029474 00432023 */  subu  $a0, $v0, $v1
.L80029478:
/* 004878 80029478 28A200B4 */  slti  $v0, $a1, 0xb4
/* 00487C 8002947C 10400006 */  beqz  $v0, .L80029498
/* 004880 80029480 00041400 */   sll   $v0, $a0, 0x10
/* 004884 80029484 00021383 */  sra   $v0, $v0, 0xe
/* 004888 80029488 3C018007 */  lui   $at, 0x8007
/* 00488C 8002948C 00220821 */  addu  $at, $at, $v0
/* 004890 80029490 03E00008 */  jr    $ra
/* 004894 80029494 C4204274 */   lwc1  $f0, 0x4274($at)

.L80029498:
/* 004898 80029498 00021383 */  sra   $v0, $v0, 0xe
/* 00489C 8002949C 3C018007 */  lui   $at, 0x8007
/* 0048A0 800294A0 00220821 */  addu  $at, $at, $v0
/* 0048A4 800294A4 C4204274 */  lwc1  $f0, 0x4274($at)
/* 0048A8 800294A8 03E00008 */  jr    $ra
/* 0048AC 800294AC 46000007 */   neg.s $f0, $f0

/* 0048B0 800294B0 04800002 */  bltz  $a0, .L800294BC
/* 0048B4 800294B4 2402FFFF */   addiu $v0, $zero, -1
/* 0048B8 800294B8 0004102A */  slt   $v0, $zero, $a0
.L800294BC:
/* 0048BC 800294BC 03E00008 */  jr    $ra
/* 0048C0 800294C0 00000000 */   nop   

func_800294C4:
/* 0048C4 800294C4 27BDFF88 */  addiu $sp, $sp, -0x78
/* 0048C8 800294C8 AFB7006C */  sw    $s7, 0x6c($sp)
/* 0048CC 800294CC 00C0B82D */  move  $s7, $a2
/* 0048D0 800294D0 AFB40060 */  sw    $s4, 0x60($sp)
/* 0048D4 800294D4 24140026 */  addiu $s4, $zero, 0x26
/* 0048D8 800294D8 AFBE0070 */  sw    $fp, 0x70($sp)
/* 0048DC 800294DC 0000F02D */  move  $fp, $zero
/* 0048E0 800294E0 AFB3005C */  sw    $s3, 0x5c($sp)
/* 0048E4 800294E4 AFB20058 */  sw    $s2, 0x58($sp)
/* 0048E8 800294E8 0080982D */  move  $s3, $a0
/* 0048EC 800294EC 000497C3 */  sra   $s2, $a0, 0x1f
/* 0048F0 800294F0 AFBF0074 */  sw    $ra, 0x74($sp)
/* 0048F4 800294F4 AFB60068 */  sw    $s6, 0x68($sp)
/* 0048F8 800294F8 AFB50064 */  sw    $s5, 0x64($sp)
/* 0048FC 800294FC AFB10054 */  sw    $s1, 0x54($sp)
/* 004900 80029500 AFB00050 */  sw    $s0, 0x50($sp)
/* 004904 80029504 0641000A */  bgez  $s2, .L80029530
/* 004908 80029508 AFA5007C */   sw    $a1, 0x7c($sp)
/* 00490C 8002950C 2402FFFF */  addiu $v0, $zero, -1
/* 004910 80029510 02620019 */  multu $s3, $v0
/* 004914 80029514 241E0001 */  addiu $fp, $zero, 1
/* 004918 80029518 00001010 */  mfhi  $v0
/* 00491C 8002951C 00001812 */  mflo  $v1
/* 004920 80029520 00531023 */  subu  $v0, $v0, $s3
/* 004924 80029524 00521023 */  subu  $v0, $v0, $s2
/* 004928 80029528 0040902D */  move  $s2, $v0
/* 00492C 8002952C 0060982D */  move  $s3, $v1
.L80029530:
/* 004930 80029530 A3A00037 */  sb    $zero, 0x37($sp)
/* 004934 80029534 27B60010 */  addiu $s6, $sp, 0x10
/* 004938 80029538 0017AFC3 */  sra   $s5, $s7, 0x1f
func_8002953C:
/* 00493C 8002953C 02E0882D */  move  $s1, $s7
/* 004940 80029540 02A0802D */  move  $s0, $s5
/* 004944 80029544 0240202D */  move  $a0, $s2
/* 004948 80029548 0260282D */  move  $a1, $s3
/* 00494C 8002954C 0200302D */  move  $a2, $s0
/* 004950 80029550 0C01B770 */  jal   func_8006DDC0
/* 004954 80029554 0220382D */   move  $a3, $s1
/* 004958 80029558 0240202D */  move  $a0, $s2
/* 00495C 8002955C 0260282D */  move  $a1, $s3
/* 004960 80029560 0200302D */  move  $a2, $s0
/* 004964 80029564 0220382D */  move  $a3, $s1
/* 004968 80029568 3C088007 */  lui   $t0, 0x8007
/* 00496C 8002956C 250843E0 */  addiu $t0, $t0, 0x43e0
/* 004970 80029570 00681021 */  addu  $v0, $v1, $t0
/* 004974 80029574 90430000 */  lbu   $v1, ($v0)
/* 004978 80029578 02D41021 */  addu  $v0, $s6, $s4
/* 00497C 8002957C 0C01B600 */  jal   func_8006D800
/* 004980 80029580 A0430000 */   sb    $v1, ($v0)
/* 004984 80029584 0040902D */  move  $s2, $v0
/* 004988 80029588 0060982D */  move  $s3, $v1
/* 00498C 8002958C 02721025 */  or    $v0, $s3, $s2
/* 004990 80029590 10400005 */  beqz  $v0, .L800295A8
/* 004994 80029594 00000000 */   nop   
/* 004998 80029598 12800003 */  beqz  $s4, .L800295A8
/* 00499C 8002959C 00000000 */   nop   
/* 0049A0 800295A0 0800A54F */  j     func_8002953C
/* 0049A4 800295A4 2694FFFF */   addiu $s4, $s4, -1

.L800295A8:
/* 0049A8 800295A8 13C00004 */  beqz  $fp, .L800295BC
/* 0049AC 800295AC 2402002D */   addiu $v0, $zero, 0x2d
/* 0049B0 800295B0 2694FFFF */  addiu $s4, $s4, -1
/* 0049B4 800295B4 03B41821 */  addu  $v1, $sp, $s4
/* 0049B8 800295B8 A0620010 */  sb    $v0, 0x10($v1)
.L800295BC:
/* 0049BC 800295BC 27A50010 */  addiu $a1, $sp, 0x10
/* 0049C0 800295C0 8FA4007C */  lw    $a0, 0x7c($sp)
/* 0049C4 800295C4 0C01BB50 */  jal   func_8006ED40
/* 0049C8 800295C8 00B42821 */   addu  $a1, $a1, $s4
/* 0049CC 800295CC 8FA2007C */  lw    $v0, 0x7c($sp)
/* 0049D0 800295D0 8FBF0074 */  lw    $ra, 0x74($sp)
/* 0049D4 800295D4 8FBE0070 */  lw    $fp, 0x70($sp)
/* 0049D8 800295D8 8FB7006C */  lw    $s7, 0x6c($sp)
/* 0049DC 800295DC 8FB60068 */  lw    $s6, 0x68($sp)
/* 0049E0 800295E0 8FB50064 */  lw    $s5, 0x64($sp)
/* 0049E4 800295E4 8FB40060 */  lw    $s4, 0x60($sp)
/* 0049E8 800295E8 8FB3005C */  lw    $s3, 0x5c($sp)
/* 0049EC 800295EC 8FB20058 */  lw    $s2, 0x58($sp)
/* 0049F0 800295F0 8FB10054 */  lw    $s1, 0x54($sp)
/* 0049F4 800295F4 8FB00050 */  lw    $s0, 0x50($sp)
/* 0049F8 800295F8 03E00008 */  jr    $ra
/* 0049FC 800295FC 27BD0078 */   addiu $sp, $sp, 0x78

func_80029600:
/* 004A00 80029600 18A00005 */  blez  $a1, .L80029618
/* 004A04 80029604 00000000 */   nop   
.L80029608:
/* 004A08 80029608 A0800000 */  sb    $zero, ($a0)
/* 004A0C 8002960C 24A5FFFF */  addiu $a1, $a1, -1
/* 004A10 80029610 1CA0FFFD */  bgtz  $a1, .L80029608
/* 004A14 80029614 24840001 */   addiu $a0, $a0, 1
.L80029618:
/* 004A18 80029618 03E00008 */  jr    $ra
/* 004A1C 8002961C 00000000 */   nop   

func_80029620:
/* 004A20 80029620 44853000 */  mtc1  $a1, $f6
/* 004A24 80029624 C4800000 */  lwc1  $f0, ($a0)
/* 004A28 80029628 46060002 */  mul.s $f0, $f0, $f6
/* 004A2C 8002962C 00000000 */  nop   
/* 004A30 80029630 44864000 */  mtc1  $a2, $f8
/* 004A34 80029634 C4820010 */  lwc1  $f2, 0x10($a0)
/* 004A38 80029638 46081082 */  mul.s $f2, $f2, $f8
/* 004A3C 8002963C 00000000 */  nop   
/* 004A40 80029640 44875000 */  mtc1  $a3, $f10
/* 004A44 80029644 C4840020 */  lwc1  $f4, 0x20($a0)
/* 004A48 80029648 460A2102 */  mul.s $f4, $f4, $f10
/* 004A4C 8002964C 00000000 */  nop   
/* 004A50 80029650 46020000 */  add.s $f0, $f0, $f2
/* 004A54 80029654 8FA20014 */  lw    $v0, 0x14($sp)
/* 004A58 80029658 8FA30018 */  lw    $v1, 0x18($sp)
/* 004A5C 8002965C 46040000 */  add.s $f0, $f0, $f4
/* 004A60 80029660 C4820030 */  lwc1  $f2, 0x30($a0)
/* 004A64 80029664 8FA5001C */  lw    $a1, 0x1c($sp)
/* 004A68 80029668 46020000 */  add.s $f0, $f0, $f2
/* 004A6C 8002966C 8FA60020 */  lw    $a2, 0x20($sp)
/* 004A70 80029670 E4400000 */  swc1  $f0, ($v0)
/* 004A74 80029674 C4800004 */  lwc1  $f0, 4($a0)
/* 004A78 80029678 46060002 */  mul.s $f0, $f0, $f6
/* 004A7C 8002967C 00000000 */  nop   
/* 004A80 80029680 C4840014 */  lwc1  $f4, 0x14($a0)
/* 004A84 80029684 46082102 */  mul.s $f4, $f4, $f8
/* 004A88 80029688 00000000 */  nop   
/* 004A8C 8002968C C4820024 */  lwc1  $f2, 0x24($a0)
/* 004A90 80029690 460A1082 */  mul.s $f2, $f2, $f10
/* 004A94 80029694 00000000 */  nop   
/* 004A98 80029698 46040000 */  add.s $f0, $f0, $f4
/* 004A9C 8002969C 46020000 */  add.s $f0, $f0, $f2
/* 004AA0 800296A0 C4820034 */  lwc1  $f2, 0x34($a0)
/* 004AA4 800296A4 46020000 */  add.s $f0, $f0, $f2
/* 004AA8 800296A8 E4600000 */  swc1  $f0, ($v1)
/* 004AAC 800296AC C4800008 */  lwc1  $f0, 8($a0)
/* 004AB0 800296B0 46060002 */  mul.s $f0, $f0, $f6
/* 004AB4 800296B4 00000000 */  nop   
/* 004AB8 800296B8 C4840018 */  lwc1  $f4, 0x18($a0)
/* 004ABC 800296BC 46082102 */  mul.s $f4, $f4, $f8
/* 004AC0 800296C0 00000000 */  nop   
/* 004AC4 800296C4 C4820028 */  lwc1  $f2, 0x28($a0)
/* 004AC8 800296C8 460A1082 */  mul.s $f2, $f2, $f10
/* 004ACC 800296CC 00000000 */  nop   
/* 004AD0 800296D0 46040000 */  add.s $f0, $f0, $f4
/* 004AD4 800296D4 46020000 */  add.s $f0, $f0, $f2
/* 004AD8 800296D8 C4820038 */  lwc1  $f2, 0x38($a0)
/* 004ADC 800296DC 46020000 */  add.s $f0, $f0, $f2
/* 004AE0 800296E0 E4A00000 */  swc1  $f0, ($a1)
/* 004AE4 800296E4 C480000C */  lwc1  $f0, 0xc($a0)
/* 004AE8 800296E8 46060002 */  mul.s $f0, $f0, $f6
/* 004AEC 800296EC 00000000 */  nop   
/* 004AF0 800296F0 C484001C */  lwc1  $f4, 0x1c($a0)
/* 004AF4 800296F4 46082102 */  mul.s $f4, $f4, $f8
/* 004AF8 800296F8 00000000 */  nop   
/* 004AFC 800296FC C482002C */  lwc1  $f2, 0x2c($a0)
/* 004B00 80029700 460A1082 */  mul.s $f2, $f2, $f10
/* 004B04 80029704 00000000 */  nop   
/* 004B08 80029708 46040000 */  add.s $f0, $f0, $f4
/* 004B0C 8002970C 46020000 */  add.s $f0, $f0, $f2
/* 004B10 80029710 C482003C */  lwc1  $f2, 0x3c($a0)
/* 004B14 80029714 46020000 */  add.s $f0, $f0, $f2
/* 004B18 80029718 03E00008 */  jr    $ra
/* 004B1C 8002971C E4C00000 */   swc1  $f0, ($a2)

func_80029720:
/* 004B20 80029720 27BDFFE8 */  addiu $sp, $sp, -0x18
/* 004B24 80029724 AFBF0010 */  sw    $ra, 0x10($sp)
/* 004B28 80029728 0C019194 */  jal   func_80064650
/* 004B2C 8002972C 24060040 */   addiu $a2, $zero, 0x40
/* 004B30 80029730 8FBF0010 */  lw    $ra, 0x10($sp)
/* 004B34 80029734 03E00008 */  jr    $ra
/* 004B38 80029738 27BD0018 */   addiu $sp, $sp, 0x18

func_8002973C:
/* 004B3C 8002973C 27BDFFD8 */  addiu $sp, $sp, -0x28
/* 004B40 80029740 AFB20018 */  sw    $s2, 0x18($sp)
/* 004B44 80029744 0080902D */  move  $s2, $a0
/* 004B48 80029748 AFB3001C */  sw    $s3, 0x1c($sp)
/* 004B4C 8002974C 00C0982D */  move  $s3, $a2
/* 004B50 80029750 0260202D */  move  $a0, $s3
/* 004B54 80029754 AFB10014 */  sw    $s1, 0x14($sp)
/* 004B58 80029758 00B28823 */  subu  $s1, $a1, $s2
/* 004B5C 8002975C 0220282D */  move  $a1, $s1
/* 004B60 80029760 AFBF0020 */  sw    $ra, 0x20($sp)
/* 004B64 80029764 0C0185F4 */  jal   func_800617D0
/* 004B68 80029768 AFB00010 */   sw    $s0, 0x10($sp)
/* 004B6C 8002976C 24022000 */  addiu $v0, $zero, 0x2000
/* 004B70 80029770 0051102B */  sltu  $v0, $v0, $s1
/* 004B74 80029774 1040000A */  beqz  $v0, .L800297A0
/* 004B78 80029778 0000802D */   move  $s0, $zero
/* 004B7C 8002977C 02502021 */  addu  $a0, $s2, $s0
.L80029780:
/* 004B80 80029780 02702821 */  addu  $a1, $s3, $s0
/* 004B84 80029784 0C017D24 */  jal   func_8005F490
/* 004B88 80029788 24062000 */   addiu $a2, $zero, 0x2000
/* 004B8C 8002978C 26102000 */  addiu $s0, $s0, 0x2000
/* 004B90 80029790 26022000 */  addiu $v0, $s0, 0x2000
/* 004B94 80029794 0051102B */  sltu  $v0, $v0, $s1
/* 004B98 80029798 1440FFF9 */  bnez  $v0, .L80029780
/* 004B9C 8002979C 02502021 */   addu  $a0, $s2, $s0
.L800297A0:
/* 004BA0 800297A0 12110004 */  beq   $s0, $s1, .L800297B4
/* 004BA4 800297A4 02502021 */   addu  $a0, $s2, $s0
/* 004BA8 800297A8 02702821 */  addu  $a1, $s3, $s0
/* 004BAC 800297AC 0C017D24 */  jal   func_8005F490
/* 004BB0 800297B0 02303023 */   subu  $a2, $s1, $s0
.L800297B4:
/* 004BB4 800297B4 0220102D */  move  $v0, $s1
/* 004BB8 800297B8 8FBF0020 */  lw    $ra, 0x20($sp)
/* 004BBC 800297BC 8FB3001C */  lw    $s3, 0x1c($sp)
/* 004BC0 800297C0 8FB20018 */  lw    $s2, 0x18($sp)
/* 004BC4 800297C4 8FB10014 */  lw    $s1, 0x14($sp)
/* 004BC8 800297C8 8FB00010 */  lw    $s0, 0x10($sp)
/* 004BCC 800297CC 03E00008 */  jr    $ra
/* 004BD0 800297D0 27BD0028 */   addiu $sp, $sp, 0x28

/* 004BD4 800297D4 27BDFFD8 */  addiu $sp, $sp, -0x28
/* 004BD8 800297D8 AFB20018 */  sw    $s2, 0x18($sp)
/* 004BDC 800297DC 0080902D */  move  $s2, $a0
/* 004BE0 800297E0 AFB00010 */  sw    $s0, 0x10($sp)
/* 004BE4 800297E4 0000802D */  move  $s0, $zero
/* 004BE8 800297E8 AFB3001C */  sw    $s3, 0x1c($sp)
/* 004BEC 800297EC 00C0982D */  move  $s3, $a2
/* 004BF0 800297F0 AFB10014 */  sw    $s1, 0x14($sp)
/* 004BF4 800297F4 00B28823 */  subu  $s1, $a1, $s2
/* 004BF8 800297F8 24022000 */  addiu $v0, $zero, 0x2000
/* 004BFC 800297FC 0051102B */  sltu  $v0, $v0, $s1
/* 004C00 80029800 1040000A */  beqz  $v0, .L8002982C
/* 004C04 80029804 AFBF0020 */   sw    $ra, 0x20($sp)
/* 004C08 80029808 02502021 */  addu  $a0, $s2, $s0
.L8002980C:
/* 004C0C 8002980C 02702821 */  addu  $a1, $s3, $s0
/* 004C10 80029810 0C00A618 */  jal   func_80029860
/* 004C14 80029814 24062000 */   addiu $a2, $zero, 0x2000
/* 004C18 80029818 26102000 */  addiu $s0, $s0, 0x2000
/* 004C1C 8002981C 26022000 */  addiu $v0, $s0, 0x2000
/* 004C20 80029820 0051102B */  sltu  $v0, $v0, $s1
/* 004C24 80029824 1440FFF9 */  bnez  $v0, .L8002980C
/* 004C28 80029828 02502021 */   addu  $a0, $s2, $s0
.L8002982C:
/* 004C2C 8002982C 12110004 */  beq   $s0, $s1, .L80029840
/* 004C30 80029830 02502021 */   addu  $a0, $s2, $s0
/* 004C34 80029834 02702821 */  addu  $a1, $s3, $s0
/* 004C38 80029838 0C00A618 */  jal   func_80029860
/* 004C3C 8002983C 02303023 */   subu  $a2, $s1, $s0
.L80029840:
/* 004C40 80029840 0220102D */  move  $v0, $s1
/* 004C44 80029844 8FBF0020 */  lw    $ra, 0x20($sp)
/* 004C48 80029848 8FB3001C */  lw    $s3, 0x1c($sp)
/* 004C4C 8002984C 8FB20018 */  lw    $s2, 0x18($sp)
/* 004C50 80029850 8FB10014 */  lw    $s1, 0x14($sp)
/* 004C54 80029854 8FB00010 */  lw    $s0, 0x10($sp)
/* 004C58 80029858 03E00008 */  jr    $ra
/* 004C5C 8002985C 27BD0028 */   addiu $sp, $sp, 0x28

func_80029860:
/* 004C60 80029860 27BDFFA0 */  addiu $sp, $sp, -0x60
/* 004C64 80029864 AFB20050 */  sw    $s2, 0x50($sp)
/* 004C68 80029868 0080902D */  move  $s2, $a0
/* 004C6C 8002986C AFB40058 */  sw    $s4, 0x58($sp)
/* 004C70 80029870 00A0A02D */  move  $s4, $a1
/* 004C74 80029874 AFB30054 */  sw    $s3, 0x54($sp)
/* 004C78 80029878 00C0982D */  move  $s3, $a2
/* 004C7C 8002987C 0260282D */  move  $a1, $s3
/* 004C80 80029880 AFBF005C */  sw    $ra, 0x5c($sp)
/* 004C84 80029884 AFB1004C */  sw    $s1, 0x4c($sp)
/* 004C88 80029888 0C018614 */  jal   func_80061850
/* 004C8C 8002988C AFB00048 */   sw    $s0, 0x48($sp)
/* 004C90 80029890 27B00028 */  addiu $s0, $sp, 0x28
/* 004C94 80029894 0200202D */  move  $a0, $s0
/* 004C98 80029898 27B10040 */  addiu $s1, $sp, 0x40
/* 004C9C 8002989C 0220282D */  move  $a1, $s1
/* 004CA0 800298A0 0C019560 */  jal   func_80065580
/* 004CA4 800298A4 24060001 */   addiu $a2, $zero, 1
/* 004CA8 800298A8 27A50010 */  addiu $a1, $sp, 0x10
/* 004CAC 800298AC 3C04800A */  lui   $a0, 0x800a
/* 004CB0 800298B0 8C84A638 */  lw    $a0, -0x59c8($a0)
/* 004CB4 800298B4 24060001 */  addiu $a2, $zero, 1
/* 004CB8 800298B8 A3A00012 */  sb    $zero, 0x12($sp)
/* 004CBC 800298BC AFB00014 */  sw    $s0, 0x14($sp)
/* 004CC0 800298C0 AFB20018 */  sw    $s2, 0x18($sp)
/* 004CC4 800298C4 AFB4001C */  sw    $s4, 0x1c($sp)
/* 004CC8 800298C8 0C018388 */  jal   func_80060E20
/* 004CCC 800298CC AFB30020 */   sw    $s3, 0x20($sp)
/* 004CD0 800298D0 0200202D */  move  $a0, $s0
/* 004CD4 800298D4 0220282D */  move  $a1, $s1
/* 004CD8 800298D8 0C0195BC */  jal   func_800656F0
/* 004CDC 800298DC 24060001 */   addiu $a2, $zero, 1
/* 004CE0 800298E0 8FBF005C */  lw    $ra, 0x5c($sp)
/* 004CE4 800298E4 8FB40058 */  lw    $s4, 0x58($sp)
/* 004CE8 800298E8 8FB30054 */  lw    $s3, 0x54($sp)
/* 004CEC 800298EC 8FB20050 */  lw    $s2, 0x50($sp)
/* 004CF0 800298F0 8FB1004C */  lw    $s1, 0x4c($sp)
/* 004CF4 800298F4 8FB00048 */  lw    $s0, 0x48($sp)
/* 004CF8 800298F8 03E00008 */  jr    $ra
/* 004CFC 800298FC 27BD0060 */   addiu $sp, $sp, 0x60

func_80029900:
/* 004D00 80029900 3C035D58 */  lui   $v1, 0x5d58
/* 004D04 80029904 3C048007 */  lui   $a0, 0x8007
/* 004D08 80029908 24844410 */  addiu $a0, $a0, 0x4410
/* 004D0C 8002990C 8C820000 */  lw    $v0, ($a0)
/* 004D10 80029910 34638B65 */  ori   $v1, $v1, 0x8b65
/* 004D14 80029914 00430018 */  mult  $v0, $v1
/* 004D18 80029918 3C038007 */  lui   $v1, 0x8007
/* 004D1C 8002991C 8C63419C */  lw    $v1, 0x419c($v1)
/* 004D20 80029920 00002812 */  mflo  $a1
/* 004D24 80029924 24A20001 */  addiu $v0, $a1, 1
/* 004D28 80029928 AC820000 */  sw    $v0, ($a0)
/* 004D2C 8002992C 03E00008 */  jr    $ra
/* 004D30 80029930 AC620138 */   sw    $v0, 0x138($v1)

func_80029934:
/* 004D34 80029934 27BDFFE8 */  addiu $sp, $sp, -0x18
/* 004D38 80029938 AFBF0010 */  sw    $ra, 0x10($sp)
/* 004D3C 8002993C 0C00A640 */  jal   func_80029900
/* 004D40 80029940 00000000 */   nop   
/* 004D44 80029944 30427FFF */  andi  $v0, $v0, 0x7fff
/* 004D48 80029948 44821000 */  mtc1  $v0, $f2
/* 004D4C 8002994C 00000000 */  nop   
/* 004D50 80029950 04410006 */  bgez  $v0, .L8002996C
/* 004D54 80029954 468010A1 */   cvt.d.w $f2, $f2
/* 004D58 80029958 3C0141F0 */  lui   $at, 0x41f0
/* 004D5C 8002995C 44810800 */  mtc1  $at, $f1
/* 004D60 80029960 44800000 */  mtc1  $zero, $f0
/* 004D64 80029964 00000000 */  nop   
/* 004D68 80029968 46201080 */  add.d $f2, $f2, $f0
.L8002996C:
/* 004D6C 8002996C 3C013F00 */  lui   $at, 0x3f00
/* 004D70 80029970 44810800 */  mtc1  $at, $f1
/* 004D74 80029974 44800000 */  mtc1  $zero, $f0
/* 004D78 80029978 00000000 */  nop   
/* 004D7C 8002997C 46201002 */  mul.d $f0, $f2, $f0
/* 004D80 80029980 00000000 */  nop   
/* 004D84 80029984 8FBF0010 */  lw    $ra, 0x10($sp)
/* 004D88 80029988 46200020 */  cvt.s.d $f0, $f0
/* 004D8C 8002998C 03E00008 */  jr    $ra
/* 004D90 80029990 27BD0018 */   addiu $sp, $sp, 0x18

func_80029994:
/* 004D94 80029994 27BDFFE0 */  addiu $sp, $sp, -0x20
/* 004D98 80029998 AFB00010 */  sw    $s0, 0x10($sp)
/* 004D9C 8002999C 2410FFFF */  addiu $s0, $zero, -1
/* 004DA0 800299A0 AFB10014 */  sw    $s1, 0x14($sp)
/* 004DA4 800299A4 24910001 */  addiu $s1, $a0, 1
/* 004DA8 800299A8 16200002 */  bnez  $s1, .L800299B4
/* 004DAC 800299AC 0211001B */   divu  $zero, $s0, $s1
/* 004DB0 800299B0 0007000D */  break 7
.L800299B4:
/* 004DB4 800299B4 00008012 */  mflo  $s0
/* 004DB8 800299B8 16000002 */  bnez  $s0, .L800299C4
/* 004DBC 800299BC AFBF0018 */   sw    $ra, 0x18($sp)
/* 004DC0 800299C0 24100001 */  addiu $s0, $zero, 1
.L800299C4:
/* 004DC4 800299C4 0C00A640 */  jal   func_80029900
/* 004DC8 800299C8 00000000 */   nop   
/* 004DCC 800299CC 16000002 */  bnez  $s0, .L800299D8
/* 004DD0 800299D0 0050001B */   divu  $zero, $v0, $s0
/* 004DD4 800299D4 0007000D */  break 7
.L800299D8:
/* 004DD8 800299D8 00001812 */  mflo  $v1
/* 004DDC 800299DC 0071102B */  sltu  $v0, $v1, $s1
/* 004DE0 800299E0 1040FFF8 */  beqz  $v0, .L800299C4
/* 004DE4 800299E4 0060102D */   move  $v0, $v1
/* 004DE8 800299E8 8FBF0018 */  lw    $ra, 0x18($sp)
/* 004DEC 800299EC 8FB10014 */  lw    $s1, 0x14($sp)
/* 004DF0 800299F0 8FB00010 */  lw    $s0, 0x10($sp)
/* 004DF4 800299F4 03E00008 */  jr    $ra
/* 004DF8 800299F8 27BD0020 */   addiu $sp, $sp, 0x20

func_800299FC:
/* 004DFC 800299FC 27BDFFE8 */  addiu $sp, $sp, -0x18
/* 004E00 80029A00 AFBF0010 */  sw    $ra, 0x10($sp)
/* 004E04 80029A04 04820001 */  bltzl $a0, .L80029A0C
/* 004E08 80029A08 00042023 */   negu  $a0, $a0
.L80029A0C:
/* 004E0C 80029A0C 10800018 */  beqz  $a0, .L80029A70
/* 004E10 80029A10 0000102D */   move  $v0, $zero
/* 004E14 80029A14 24020001 */  addiu $v0, $zero, 1
/* 004E18 80029A18 54820006 */  bnel  $a0, $v0, .L80029A34
/* 004E1C 80029A1C 24020064 */   addiu $v0, $zero, 0x64
/* 004E20 80029A20 0C00A665 */  jal   func_80029994
/* 004E24 80029A24 240403E8 */   addiu $a0, $zero, 0x3e8
/* 004E28 80029A28 284201F5 */  slti  $v0, $v0, 0x1f5
/* 004E2C 80029A2C 0800A69C */  j     func_80029A70
/* 004E30 80029A30 38420001 */   xori  $v0, $v0, 1

.L80029A34:
/* 004E34 80029A34 10820005 */  beq   $a0, $v0, .L80029A4C
/* 004E38 80029A38 00000000 */   nop   
/* 004E3C 80029A3C 0C00A665 */  jal   func_80029994
/* 004E40 80029A40 00000000 */   nop   
/* 004E44 80029A44 0800A69C */  j     func_80029A70
/* 004E48 80029A48 00000000 */   nop   

.L80029A4C:
/* 004E4C 80029A4C 0C00A665 */  jal   func_80029994
/* 004E50 80029A50 240403F1 */   addiu $a0, $zero, 0x3f1
/* 004E54 80029A54 3C036666 */  lui   $v1, 0x6666
/* 004E58 80029A58 34636667 */  ori   $v1, $v1, 0x6667
/* 004E5C 80029A5C 00430018 */  mult  $v0, $v1
/* 004E60 80029A60 000217C3 */  sra   $v0, $v0, 0x1f
/* 004E64 80029A64 00002810 */  mfhi  $a1
/* 004E68 80029A68 00051883 */  sra   $v1, $a1, 2
/* 004E6C 80029A6C 00621023 */  subu  $v0, $v1, $v0
func_80029A70:
.L80029A70:
/* 004E70 80029A70 8FBF0010 */  lw    $ra, 0x10($sp)
/* 004E74 80029A74 03E00008 */  jr    $ra
/* 004E78 80029A78 27BD0018 */   addiu $sp, $sp, 0x18

/* 004E7C 80029A7C 44801000 */  mtc1  $zero, $f2
/* 004E80 80029A80 00000000 */  nop   
/* 004E84 80029A84 460C103C */  c.lt.s $f2, $f12
/* 004E88 80029A88 00000000 */  nop   
/* 004E8C 80029A8C 45010009 */  bc1t  .L80029AB4
/* 004E90 80029A90 00000000 */   nop   
/* 004E94 80029A94 3C01BF80 */  lui   $at, 0xbf80
/* 004E98 80029A98 44810000 */  mtc1  $at, $f0
/* 004E9C 80029A9C 4602603C */  c.lt.s $f12, $f2
/* 004EA0 80029AA0 00000000 */  nop   
/* 004EA4 80029AA4 45010005 */  bc1t  .L80029ABC
/* 004EA8 80029AA8 00000000 */   nop   
/* 004EAC 80029AAC 03E00008 */  jr    $ra
/* 004EB0 80029AB0 46001006 */   mov.s $f0, $f2

.L80029AB4:
/* 004EB4 80029AB4 3C013F80 */  lui   $at, 0x3f80
/* 004EB8 80029AB8 44810000 */  mtc1  $at, $f0
.L80029ABC:
/* 004EBC 80029ABC 03E00008 */  jr    $ra
/* 004EC0 80029AC0 00000000 */   nop   

func_80029AC4:
/* 004EC4 80029AC4 44800000 */  mtc1  $zero, $f0
/* 004EC8 80029AC8 00000000 */  nop   
/* 004ECC 80029ACC 460C003E */  c.le.s $f0, $f12
/* 004ED0 80029AD0 00000000 */  nop   
/* 004ED4 80029AD4 4502000A */  bc1fl .L80029B00
/* 004ED8 80029AD8 460060A1 */   cvt.d.s $f2, $f12
/* 004EDC 80029ADC 3C013FE0 */  lui   $at, 0x3fe0
/* 004EE0 80029AE0 44811800 */  mtc1  $at, $f3
/* 004EE4 80029AE4 44801000 */  mtc1  $zero, $f2
/* 004EE8 80029AE8 46006021 */  cvt.d.s $f0, $f12
/* 004EEC 80029AEC 46220000 */  add.d $f0, $f0, $f2
/* 004EF0 80029AF0 4620010D */  trunc.w.d $f4, $f0
/* 004EF4 80029AF4 44022000 */  mfc1  $v0, $f4
/* 004EF8 80029AF8 03E00008 */  jr    $ra
/* 004EFC 80029AFC 00000000 */   nop   

.L80029B00:
/* 004F00 80029B00 3C013FE0 */  lui   $at, 0x3fe0
/* 004F04 80029B04 44810800 */  mtc1  $at, $f1
/* 004F08 80029B08 44800000 */  mtc1  $zero, $f0
/* 004F0C 80029B0C 00000000 */  nop   
/* 004F10 80029B10 46220001 */  sub.d $f0, $f0, $f2
/* 004F14 80029B14 4620010D */  trunc.w.d $f4, $f0
/* 004F18 80029B18 44022000 */  mfc1  $v0, $f4
/* 004F1C 80029B1C 03E00008 */  jr    $ra
/* 004F20 80029B20 00021023 */   negu  $v0, $v0

func_80029B24:
/* 004F24 80029B24 3C014561 */  lui   $at, 0x4561
/* 004F28 80029B28 44811000 */  mtc1  $at, $f2
/* 004F2C 80029B2C 46006005 */  abs.s $f0, $f12
/* 004F30 80029B30 4600103C */  c.lt.s $f2, $f0
/* 004F34 80029B34 00000000 */  nop   
/* 004F38 80029B38 45000026 */  bc1f  .L80029BD4
/* 004F3C 80029B3C 00000000 */   nop   
/* 004F40 80029B40 4600610D */  trunc.w.s $f4, $f12
/* 004F44 80029B44 44042000 */  mfc1  $a0, $f4
/* 004F48 80029B48 00000000 */  nop   
/* 004F4C 80029B4C 0480000F */  bltz  $a0, .L80029B8C
/* 004F50 80029B50 3C02B60B */   lui   $v0, 0xb60b
/* 004F54 80029B54 344260B7 */  ori   $v0, $v0, 0x60b7
/* 004F58 80029B58 00820018 */  mult  $a0, $v0
/* 004F5C 80029B5C 00041FC3 */  sra   $v1, $a0, 0x1f
/* 004F60 80029B60 00002810 */  mfhi  $a1
/* 004F64 80029B64 00A41021 */  addu  $v0, $a1, $a0
/* 004F68 80029B68 00021203 */  sra   $v0, $v0, 8
/* 004F6C 80029B6C 00431023 */  subu  $v0, $v0, $v1
/* 004F70 80029B70 00021840 */  sll   $v1, $v0, 1
/* 004F74 80029B74 00621821 */  addu  $v1, $v1, $v0
/* 004F78 80029B78 00031100 */  sll   $v0, $v1, 4
/* 004F7C 80029B7C 00431023 */  subu  $v0, $v0, $v1
/* 004F80 80029B80 000210C0 */  sll   $v0, $v0, 3
/* 004F84 80029B84 0800A6F1 */  j     func_80029BC4
/* 004F88 80029B88 00822023 */   subu  $a0, $a0, $v0

.L80029B8C:
/* 004F8C 80029B8C 00042023 */  negu  $a0, $a0
/* 004F90 80029B90 344260B7 */  ori   $v0, $v0, 0x60b7
/* 004F94 80029B94 00820018 */  mult  $a0, $v0
/* 004F98 80029B98 00041FC3 */  sra   $v1, $a0, 0x1f
/* 004F9C 80029B9C 00002810 */  mfhi  $a1
/* 004FA0 80029BA0 00A41021 */  addu  $v0, $a1, $a0
/* 004FA4 80029BA4 00021203 */  sra   $v0, $v0, 8
/* 004FA8 80029BA8 00431023 */  subu  $v0, $v0, $v1
/* 004FAC 80029BAC 00021840 */  sll   $v1, $v0, 1
/* 004FB0 80029BB0 00621821 */  addu  $v1, $v1, $v0
/* 004FB4 80029BB4 00031100 */  sll   $v0, $v1, 4
/* 004FB8 80029BB8 00431023 */  subu  $v0, $v0, $v1
/* 004FBC 80029BBC 000210C0 */  sll   $v0, $v0, 3
/* 004FC0 80029BC0 00442023 */  subu  $a0, $v0, $a0
func_80029BC4:
/* 004FC4 80029BC4 44840000 */  mtc1  $a0, $f0
/* 004FC8 80029BC8 00000000 */  nop   
/* 004FCC 80029BCC 03E00008 */  jr    $ra
/* 004FD0 80029BD0 46800020 */   cvt.s.w $f0, $f0

.L80029BD4:
/* 004FD4 80029BD4 44801000 */  mtc1  $zero, $f2
/* 004FD8 80029BD8 3C0143B4 */  lui   $at, 0x43b4
/* 004FDC 80029BDC 44810000 */  mtc1  $at, $f0
.L80029BE0:
/* 004FE0 80029BE0 4602603C */  c.lt.s $f12, $f2
/* 004FE4 80029BE4 00000000 */  nop   
/* 004FE8 80029BE8 45030001 */  bc1tl .L80029BF0
/* 004FEC 80029BEC 46006300 */   add.s $f12, $f12, $f0
.L80029BF0:
/* 004FF0 80029BF0 460C003E */  c.le.s $f0, $f12
/* 004FF4 80029BF4 00000000 */  nop   
/* 004FF8 80029BF8 45030001 */  bc1tl .L80029C00
/* 004FFC 80029BFC 46006301 */   sub.s $f12, $f12, $f0
.L80029C00:
/* 005000 80029C00 460C103E */  c.le.s $f2, $f12
/* 005004 80029C04 00000000 */  nop   
/* 005008 80029C08 4500FFF5 */  bc1f  .L80029BE0
/* 00500C 80029C0C 00000000 */   nop   
/* 005010 80029C10 4600603C */  c.lt.s $f12, $f0
/* 005014 80029C14 00000000 */  nop   
/* 005018 80029C18 4500FFF1 */  bc1f  .L80029BE0
/* 00501C 80029C1C 00000000 */   nop   
/* 005020 80029C20 03E00008 */  jr    $ra
/* 005024 80029C24 46006006 */   mov.s $f0, $f12

func_80029C28:
/* 005028 80029C28 460C7001 */  sub.s $f0, $f14, $f12
/* 00502C 80029C2C 3C014334 */  lui   $at, 0x4334
/* 005030 80029C30 44811000 */  mtc1  $at, $f2
/* 005034 80029C34 46000005 */  abs.s $f0, $f0
/* 005038 80029C38 4600103E */  c.le.s $f2, $f0
/* 00503C 80029C3C 00000000 */  nop   
/* 005040 80029C40 4500000D */  bc1f  .L80029C78
/* 005044 80029C44 00000000 */   nop   
/* 005048 80029C48 460C703C */  c.lt.s $f14, $f12
/* 00504C 80029C4C 00000000 */  nop   
/* 005050 80029C50 45000005 */  bc1f  .L80029C68
/* 005054 80029C54 00000000 */   nop   
/* 005058 80029C58 3C0143B4 */  lui   $at, 0x43b4
/* 00505C 80029C5C 44810000 */  mtc1  $at, $f0
/* 005060 80029C60 0800A71E */  j     func_80029C78
/* 005064 80029C64 46007380 */   add.s $f14, $f14, $f0

.L80029C68:
/* 005068 80029C68 3C0143B4 */  lui   $at, 0x43b4
/* 00506C 80029C6C 44810000 */  mtc1  $at, $f0
/* 005070 80029C70 00000000 */  nop   
/* 005074 80029C74 46007381 */  sub.s $f14, $f14, $f0
func_80029C78:
.L80029C78:
/* 005078 80029C78 03E00008 */  jr    $ra
/* 00507C 80029C7C 460C7001 */   sub.s $f0, $f14, $f12

func_80029C80:
/* 005080 80029C80 44863000 */  mtc1  $a2, $f6
/* 005084 80029C84 27BDFFC8 */  addiu $sp, $sp, -0x38
/* 005088 80029C88 F7BA0030 */  sdc1  $f26, 0x30($sp)
/* 00508C 80029C8C 460C3681 */  sub.s $f26, $f6, $f12
/* 005090 80029C90 44873000 */  mtc1  $a3, $f6
/* 005094 80029C94 AFBF0010 */  sw    $ra, 0x10($sp)
/* 005098 80029C98 F7B80028 */  sdc1  $f24, 0x28($sp)
/* 00509C 80029C9C 460E3601 */  sub.s $f24, $f6, $f14
/* 0050A0 80029CA0 F7B60020 */  sdc1  $f22, 0x20($sp)
/* 0050A4 80029CA4 F7B40018 */  sdc1  $f20, 0x18($sp)
/* 0050A8 80029CA8 4600D105 */  abs.s $f4, $f26
/* 0050AC 80029CAC 4600C005 */  abs.s $f0, $f24
/* 0050B0 80029CB0 4604003C */  c.lt.s $f0, $f4
/* 0050B4 80029CB4 00000000 */  nop   
/* 0050B8 80029CB8 45000029 */  bc1f  .L80029D60
/* 0050BC 80029CBC 00000000 */   nop   
/* 0050C0 80029CC0 3C014234 */  lui   $at, 0x4234
/* 0050C4 80029CC4 44811000 */  mtc1  $at, $f2
/* 0050C8 80029CC8 46040003 */  div.s $f0, $f0, $f4
/* 0050CC 80029CCC 46020502 */  mul.s $f20, $f0, $f2
/* 0050D0 80029CD0 00000000 */  nop   
/* 0050D4 80029CD4 0C00A6B1 */  jal   func_80029AC4
/* 0050D8 80029CD8 4614A300 */   add.s $f12, $f20, $f20
/* 0050DC 80029CDC 00021080 */  sll   $v0, $v0, 2
/* 0050E0 80029CE0 3C018007 */  lui   $at, 0x8007
/* 0050E4 80029CE4 00220821 */  addu  $at, $at, $v0
/* 0050E8 80029CE8 C4204414 */  lwc1  $f0, 0x4414($at)
/* 0050EC 80029CEC 4600A502 */  mul.s $f20, $f20, $f0
/* 0050F0 80029CF0 00000000 */  nop   
/* 0050F4 80029CF4 44800000 */  mtc1  $zero, $f0
/* 0050F8 80029CF8 00000000 */  nop   
/* 0050FC 80029CFC 461A003E */  c.le.s $f0, $f26
/* 005100 80029D00 00000000 */  nop   
/* 005104 80029D04 4500000D */  bc1f  .L80029D3C
/* 005108 80029D08 00000000 */   nop   
/* 00510C 80029D0C 4618003E */  c.le.s $f0, $f24
/* 005110 80029D10 00000000 */  nop   
/* 005114 80029D14 45000005 */  bc1f  .L80029D2C
/* 005118 80029D18 00000000 */   nop   
/* 00511C 80029D1C 3C0142B4 */  lui   $at, 0x42b4
/* 005120 80029D20 44810000 */  mtc1  $at, $f0
/* 005124 80029D24 0800A781 */  j     func_80029E04
/* 005128 80029D28 4600A000 */   add.s $f0, $f20, $f0

.L80029D2C:
/* 00512C 80029D2C 3C0142B4 */  lui   $at, 0x42b4
/* 005130 80029D30 44810000 */  mtc1  $at, $f0
/* 005134 80029D34 0800A781 */  j     func_80029E04
/* 005138 80029D38 46140001 */   sub.s $f0, $f0, $f20

.L80029D3C:
/* 00513C 80029D3C 4618003E */  c.le.s $f0, $f24
/* 005140 80029D40 3C014387 */  lui   $at, 0x4387
/* 005144 80029D44 44810000 */  mtc1  $at, $f0
/* 005148 80029D48 4503002E */  bc1tl .L80029E04
/* 00514C 80029D4C 46140001 */   sub.s $f0, $f0, $f20
/* 005150 80029D50 3C014387 */  lui   $at, 0x4387
/* 005154 80029D54 44810000 */  mtc1  $at, $f0
/* 005158 80029D58 0800A781 */  j     func_80029E04
/* 00515C 80029D5C 4600A000 */   add.s $f0, $f20, $f0

.L80029D60:
/* 005160 80029D60 4480B000 */  mtc1  $zero, $f22
/* 005164 80029D64 00000000 */  nop   
/* 005168 80029D68 46160032 */  c.eq.s $f0, $f22
/* 00516C 80029D6C 00000000 */  nop   
/* 005170 80029D70 45020003 */  bc1fl .L80029D80
/* 005174 80029D74 46002003 */   div.s $f0, $f4, $f0
/* 005178 80029D78 0800A781 */  j     func_80029E04
/* 00517C 80029D7C 4600B006 */   mov.s $f0, $f22

.L80029D80:
/* 005180 80029D80 3C014234 */  lui   $at, 0x4234
/* 005184 80029D84 44811000 */  mtc1  $at, $f2
/* 005188 80029D88 00000000 */  nop   
/* 00518C 80029D8C 46020502 */  mul.s $f20, $f0, $f2
/* 005190 80029D90 00000000 */  nop   
/* 005194 80029D94 0C00A6B1 */  jal   func_80029AC4
/* 005198 80029D98 4614A300 */   add.s $f12, $f20, $f20
/* 00519C 80029D9C 00021080 */  sll   $v0, $v0, 2
/* 0051A0 80029DA0 3C018007 */  lui   $at, 0x8007
/* 0051A4 80029DA4 00220821 */  addu  $at, $at, $v0
/* 0051A8 80029DA8 C4204414 */  lwc1  $f0, 0x4414($at)
/* 0051AC 80029DAC 4618B03E */  c.le.s $f22, $f24
/* 0051B0 80029DB0 4600A502 */  mul.s $f20, $f20, $f0
/* 0051B4 80029DB4 00000000 */  nop   
/* 0051B8 80029DB8 4500000A */  bc1f  .L80029DE4
/* 0051BC 80029DBC 00000000 */   nop   
/* 0051C0 80029DC0 461AB03E */  c.le.s $f22, $f26
/* 0051C4 80029DC4 3C014334 */  lui   $at, 0x4334
/* 0051C8 80029DC8 44810000 */  mtc1  $at, $f0
/* 0051CC 80029DCC 4503000D */  bc1tl .L80029E04
/* 0051D0 80029DD0 46140001 */   sub.s $f0, $f0, $f20
/* 0051D4 80029DD4 3C014334 */  lui   $at, 0x4334
/* 0051D8 80029DD8 44810000 */  mtc1  $at, $f0
/* 0051DC 80029DDC 0800A781 */  j     func_80029E04
/* 0051E0 80029DE0 4600A000 */   add.s $f0, $f20, $f0

.L80029DE4:
/* 0051E4 80029DE4 461AB03E */  c.le.s $f22, $f26
/* 0051E8 80029DE8 00000000 */  nop   
/* 0051EC 80029DEC 45010005 */  bc1t  .L80029E04
/* 0051F0 80029DF0 4600A006 */   mov.s $f0, $f20
/* 0051F4 80029DF4 3C0143B4 */  lui   $at, 0x43b4
/* 0051F8 80029DF8 44810000 */  mtc1  $at, $f0
/* 0051FC 80029DFC 00000000 */  nop   
/* 005200 80029E00 46140001 */  sub.s $f0, $f0, $f20
func_80029E04:
.L80029E04:
/* 005204 80029E04 8FBF0010 */  lw    $ra, 0x10($sp)
/* 005208 80029E08 D7BA0030 */  ldc1  $f26, 0x30($sp)
/* 00520C 80029E0C D7B80028 */  ldc1  $f24, 0x28($sp)
/* 005210 80029E10 D7B60020 */  ldc1  $f22, 0x20($sp)
/* 005214 80029E14 D7B40018 */  ldc1  $f20, 0x18($sp)
/* 005218 80029E18 03E00008 */  jr    $ra
/* 00521C 80029E1C 27BD0038 */   addiu $sp, $sp, 0x38

/* 005220 80029E20 44806000 */  mtc1  $zero, $f12
/* 005224 80029E24 3C028007 */  lui   $v0, 0x8007
/* 005228 80029E28 8C42419C */  lw    $v0, 0x419c($v0)
/* 00522C 80029E2C 27BDFFE8 */  addiu $sp, $sp, -0x18
/* 005230 80029E30 AFBF0010 */  sw    $ra, 0x10($sp)
/* 005234 80029E34 8C460128 */  lw    $a2, 0x128($v0)
/* 005238 80029E38 8C470130 */  lw    $a3, 0x130($v0)
/* 00523C 80029E3C 0C00A720 */  jal   func_80029C80
/* 005240 80029E40 46006386 */   mov.s $f14, $f12
/* 005244 80029E44 8FBF0010 */  lw    $ra, 0x10($sp)
/* 005248 80029E48 03E00008 */  jr    $ra
/* 00524C 80029E4C 27BD0018 */   addiu $sp, $sp, 0x18

/* 005250 80029E50 27BDFFE8 */  addiu $sp, $sp, -0x18
/* 005254 80029E54 AFB00010 */  sw    $s0, 0x10($sp)
/* 005258 80029E58 3C108007 */  lui   $s0, 0x8007
/* 00525C 80029E5C 2610419C */  addiu $s0, $s0, 0x419c
/* 005260 80029E60 AFBF0014 */  sw    $ra, 0x14($sp)
/* 005264 80029E64 8E020000 */  lw    $v0, ($s0)
/* 005268 80029E68 C4420128 */  lwc1  $f2, 0x128($v0)
/* 00526C 80029E6C 46021082 */  mul.s $f2, $f2, $f2
/* 005270 80029E70 00000000 */  nop   
/* 005274 80029E74 C4400130 */  lwc1  $f0, 0x130($v0)
/* 005278 80029E78 46000002 */  mul.s $f0, $f0, $f0
/* 00527C 80029E7C 00000000 */  nop   
/* 005280 80029E80 46001300 */  add.s $f12, $f2, $f0
/* 005284 80029E84 46006104 */  sqrt.s $f4, $f12
/* 005288 80029E88 46042032 */  c.eq.s $f4, $f4
/* 00528C 80029E8C 00000000 */  nop   
/* 005290 80029E90 45010005 */  bc1t  .L80029EA8
/* 005294 80029E94 00000000 */   nop   
/* 005298 80029E98 0C0187BC */  jal   func_80061EF0
/* 00529C 80029E9C 00000000 */   nop   
/* 0052A0 80029EA0 46000106 */  mov.s $f4, $f0
/* 0052A4 80029EA4 8E020000 */  lw    $v0, ($s0)
.L80029EA8:
/* 0052A8 80029EA8 44806000 */  mtc1  $zero, $f12
/* 0052AC 80029EAC C440012C */  lwc1  $f0, 0x12c($v0)
/* 0052B0 80029EB0 44062000 */  mfc1  $a2, $f4
/* 0052B4 80029EB4 46000007 */  neg.s $f0, $f0
/* 0052B8 80029EB8 44070000 */  mfc1  $a3, $f0
/* 0052BC 80029EBC 0C00A720 */  jal   func_80029C80
/* 0052C0 80029EC0 46006386 */   mov.s $f14, $f12
/* 0052C4 80029EC4 8FBF0014 */  lw    $ra, 0x14($sp)
/* 0052C8 80029EC8 8FB00010 */  lw    $s0, 0x10($sp)
/* 0052CC 80029ECC 03E00008 */  jr    $ra
/* 0052D0 80029ED0 27BD0018 */   addiu $sp, $sp, 0x18

func_80029ED4:
/* 0052D4 80029ED4 44861000 */  mtc1  $a2, $f2
/* 0052D8 80029ED8 00000000 */  nop   
/* 0052DC 80029EDC 460C1301 */  sub.s $f12, $f2, $f12
/* 0052E0 80029EE0 460C6302 */  mul.s $f12, $f12, $f12
/* 0052E4 80029EE4 00000000 */  nop   
/* 0052E8 80029EE8 44871000 */  mtc1  $a3, $f2
/* 0052EC 80029EEC 00000000 */  nop   
/* 0052F0 80029EF0 460E1381 */  sub.s $f14, $f2, $f14
/* 0052F4 80029EF4 460E7382 */  mul.s $f14, $f14, $f14
/* 0052F8 80029EF8 00000000 */  nop   
/* 0052FC 80029EFC 460E6300 */  add.s $f12, $f12, $f14
/* 005300 80029F00 27BDFFE8 */  addiu $sp, $sp, -0x18
/* 005304 80029F04 46006004 */  sqrt.s $f0, $f12
/* 005308 80029F08 46000032 */  c.eq.s $f0, $f0
/* 00530C 80029F0C 00000000 */  nop   
/* 005310 80029F10 45010003 */  bc1t  .L80029F20
/* 005314 80029F14 AFBF0010 */   sw    $ra, 0x10($sp)
/* 005318 80029F18 0C0187BC */  jal   func_80061EF0
/* 00531C 80029F1C 00000000 */   nop   
.L80029F20:
/* 005320 80029F20 8FBF0010 */  lw    $ra, 0x10($sp)
/* 005324 80029F24 03E00008 */  jr    $ra
/* 005328 80029F28 27BD0018 */   addiu $sp, $sp, 0x18

func_80029F2C:
/* 00532C 80029F2C 44872000 */  mtc1  $a3, $f4
/* 005330 80029F30 00000000 */  nop   
/* 005334 80029F34 460C2301 */  sub.s $f12, $f4, $f12
/* 005338 80029F38 460C6302 */  mul.s $f12, $f12, $f12
/* 00533C 80029F3C 00000000 */  nop   
/* 005340 80029F40 27BDFFE8 */  addiu $sp, $sp, -0x18
/* 005344 80029F44 C7A20028 */  lwc1  $f2, 0x28($sp)
/* 005348 80029F48 460E1081 */  sub.s $f2, $f2, $f14
/* 00534C 80029F4C 46021082 */  mul.s $f2, $f2, $f2
/* 005350 80029F50 00000000 */  nop   
/* 005354 80029F54 C7A0002C */  lwc1  $f0, 0x2c($sp)
/* 005358 80029F58 44862000 */  mtc1  $a2, $f4
/* 00535C 80029F5C 00000000 */  nop   
/* 005360 80029F60 46040001 */  sub.s $f0, $f0, $f4
/* 005364 80029F64 46000002 */  mul.s $f0, $f0, $f0
/* 005368 80029F68 00000000 */  nop   
/* 00536C 80029F6C 46026300 */  add.s $f12, $f12, $f2
/* 005370 80029F70 46006300 */  add.s $f12, $f12, $f0
/* 005374 80029F74 46006004 */  sqrt.s $f0, $f12
/* 005378 80029F78 46000032 */  c.eq.s $f0, $f0
/* 00537C 80029F7C 00000000 */  nop   
/* 005380 80029F80 45010003 */  bc1t  .L80029F90
/* 005384 80029F84 AFBF0010 */   sw    $ra, 0x10($sp)
/* 005388 80029F88 0C0187BC */  jal   func_80061EF0
/* 00538C 80029F8C 00000000 */   nop   
.L80029F90:
/* 005390 80029F90 8FBF0010 */  lw    $ra, 0x10($sp)
/* 005394 80029F94 03E00008 */  jr    $ra
/* 005398 80029F98 27BD0018 */   addiu $sp, $sp, 0x18

func_80029F9C:
/* 00539C 80029F9C 27BDFFD0 */  addiu $sp, $sp, -0x30
/* 0053A0 80029FA0 AFB00018 */  sw    $s0, 0x18($sp)
/* 0053A4 80029FA4 0080802D */  move  $s0, $a0
/* 0053A8 80029FA8 AFB1001C */  sw    $s1, 0x1c($sp)
/* 0053AC 80029FAC 00A0882D */  move  $s1, $a1
/* 0053B0 80029FB0 3C0140C9 */  lui   $at, 0x40c9
/* 0053B4 80029FB4 34210FD0 */  ori   $at, $at, 0xfd0
/* 0053B8 80029FB8 44810000 */  mtc1  $at, $f0
/* 0053BC 80029FBC 44872000 */  mtc1  $a3, $f4
/* 0053C0 80029FC0 27A50010 */  addiu $a1, $sp, 0x10
/* 0053C4 80029FC4 F7B40028 */  sdc1  $f20, 0x28($sp)
/* 0053C8 80029FC8 46002002 */  mul.s $f0, $f4, $f0
/* 0053CC 80029FCC 00000000 */  nop   
/* 0053D0 80029FD0 4486A000 */  mtc1  $a2, $f20
/* 0053D4 80029FD4 3C0143B4 */  lui   $at, 0x43b4
/* 0053D8 80029FD8 44816000 */  mtc1  $at, $f12
/* 0053DC 80029FDC 27A60014 */  addiu $a2, $sp, 0x14
/* 0053E0 80029FE0 AFBF0020 */  sw    $ra, 0x20($sp)
/* 0053E4 80029FE4 0C00A82D */  jal   func_8002A0B4
/* 0053E8 80029FE8 460C0303 */   div.s $f12, $f0, $f12
/* 0053EC 80029FEC C7A20010 */  lwc1  $f2, 0x10($sp)
/* 0053F0 80029FF0 4602A082 */  mul.s $f2, $f20, $f2
/* 0053F4 80029FF4 00000000 */  nop   
/* 0053F8 80029FF8 C6000000 */  lwc1  $f0, ($s0)
/* 0053FC 80029FFC 46020000 */  add.s $f0, $f0, $f2
/* 005400 8002A000 E6000000 */  swc1  $f0, ($s0)
/* 005404 8002A004 C7A00014 */  lwc1  $f0, 0x14($sp)
/* 005408 8002A008 4600A502 */  mul.s $f20, $f20, $f0
/* 00540C 8002A00C 00000000 */  nop   
/* 005410 8002A010 C6200000 */  lwc1  $f0, ($s1)
/* 005414 8002A014 46140001 */  sub.s $f0, $f0, $f20
/* 005418 8002A018 E6200000 */  swc1  $f0, ($s1)
/* 00541C 8002A01C 8FBF0020 */  lw    $ra, 0x20($sp)
/* 005420 8002A020 8FB1001C */  lw    $s1, 0x1c($sp)
/* 005424 8002A024 8FB00018 */  lw    $s0, 0x18($sp)
/* 005428 8002A028 D7B40028 */  ldc1  $f20, 0x28($sp)
/* 00542C 8002A02C 03E00008 */  jr    $ra
/* 005430 8002A030 27BD0030 */   addiu $sp, $sp, 0x30

func_8002A034:
/* 005434 8002A034 3C014980 */  lui   $at, 0x4980
/* 005438 8002A038 44812000 */  mtc1  $at, $f4
/* 00543C 8002A03C 00000000 */  nop   
/* 005440 8002A040 460C203E */  c.le.s $f4, $f12
/* 005444 8002A044 00000000 */  nop   
/* 005448 8002A048 45010008 */  bc1t  .L8002A06C
/* 00544C 8002A04C 46006086 */   mov.s $f2, $f12
/* 005450 8002A050 3C01C980 */  lui   $at, 0xc980
/* 005454 8002A054 44810000 */  mtc1  $at, $f0
/* 005458 8002A058 00000000 */  nop   
/* 00545C 8002A05C 4600603E */  c.le.s $f12, $f0
/* 005460 8002A060 00000000 */  nop   
/* 005464 8002A064 4500000F */  bc1f  .L8002A0A4
/* 005468 8002A068 00000000 */   nop   
.L8002A06C:
/* 00546C 8002A06C 3C013580 */  lui   $at, 0x3580
/* 005470 8002A070 44810000 */  mtc1  $at, $f0
/* 005474 8002A074 00000000 */  nop   
/* 005478 8002A078 46006302 */  mul.s $f12, $f12, $f0
/* 00547C 8002A07C 00000000 */  nop   
/* 005480 8002A080 4600618D */  trunc.w.s $f6, $f12
/* 005484 8002A084 44023000 */  mfc1  $v0, $f6
/* 005488 8002A088 00000000 */  nop   
/* 00548C 8002A08C 44820000 */  mtc1  $v0, $f0
/* 005490 8002A090 00000000 */  nop   
/* 005494 8002A094 46800020 */  cvt.s.w $f0, $f0
/* 005498 8002A098 46040002 */  mul.s $f0, $f0, $f4
/* 00549C 8002A09C 00000000 */  nop   
/* 0054A0 8002A0A0 46001301 */  sub.s $f12, $f2, $f0
.L8002A0A4:
/* 0054A4 8002A0A4 4600618D */  trunc.w.s $f6, $f12
/* 0054A8 8002A0A8 44023000 */  mfc1  $v0, $f6
/* 0054AC 8002A0AC 03E00008 */  jr    $ra
/* 0054B0 8002A0B0 3042FFFF */   andi  $v0, $v0, 0xffff

func_8002A0B4:
/* 0054B4 8002A0B4 3C014622 */  lui   $at, 0x4622
/* 0054B8 8002A0B8 3421F983 */  ori   $at, $at, 0xf983
/* 0054BC 8002A0BC 44810000 */  mtc1  $at, $f0
/* 0054C0 8002A0C0 27BDFFD8 */  addiu $sp, $sp, -0x28
/* 0054C4 8002A0C4 AFB10014 */  sw    $s1, 0x14($sp)
/* 0054C8 8002A0C8 00A0882D */  move  $s1, $a1
/* 0054CC 8002A0CC AFB20018 */  sw    $s2, 0x18($sp)
/* 0054D0 8002A0D0 46006302 */  mul.s $f12, $f12, $f0
/* 0054D4 8002A0D4 00000000 */  nop   
/* 0054D8 8002A0D8 AFBF001C */  sw    $ra, 0x1c($sp)
/* 0054DC 8002A0DC AFB00010 */  sw    $s0, 0x10($sp)
/* 0054E0 8002A0E0 F7B40020 */  sdc1  $f20, 0x20($sp)
/* 0054E4 8002A0E4 0C00A80D */  jal   func_8002A034
/* 0054E8 8002A0E8 00C0902D */   move  $s2, $a2
/* 0054EC 8002A0EC 3050FFFF */  andi  $s0, $v0, 0xffff
/* 0054F0 8002A0F0 0C01917C */  jal   func_800645F0
/* 0054F4 8002A0F4 0200202D */   move  $a0, $s0
/* 0054F8 8002A0F8 00021400 */  sll   $v0, $v0, 0x10
/* 0054FC 8002A0FC 00021403 */  sra   $v0, $v0, 0x10
/* 005500 8002A100 3C018009 */  lui   $at, 0x8009
/* 005504 8002A104 D4347D60 */  ldc1  $f20, 0x7d60($at)
/* 005508 8002A108 44820000 */  mtc1  $v0, $f0
/* 00550C 8002A10C 00000000 */  nop   
/* 005510 8002A110 46800021 */  cvt.d.w $f0, $f0
/* 005514 8002A114 46340002 */  mul.d $f0, $f0, $f20
/* 005518 8002A118 00000000 */  nop   
/* 00551C 8002A11C 0200202D */  move  $a0, $s0
/* 005520 8002A120 46200020 */  cvt.s.d $f0, $f0
/* 005524 8002A124 0C0187C0 */  jal   func_80061F00
/* 005528 8002A128 E6200000 */   swc1  $f0, ($s1)
/* 00552C 8002A12C 00021400 */  sll   $v0, $v0, 0x10
/* 005530 8002A130 00021403 */  sra   $v0, $v0, 0x10
/* 005534 8002A134 44820000 */  mtc1  $v0, $f0
/* 005538 8002A138 00000000 */  nop   
/* 00553C 8002A13C 46800021 */  cvt.d.w $f0, $f0
/* 005540 8002A140 46340002 */  mul.d $f0, $f0, $f20
/* 005544 8002A144 00000000 */  nop   
/* 005548 8002A148 46200020 */  cvt.s.d $f0, $f0
/* 00554C 8002A14C E6400000 */  swc1  $f0, ($s2)
/* 005550 8002A150 8FBF001C */  lw    $ra, 0x1c($sp)
/* 005554 8002A154 8FB20018 */  lw    $s2, 0x18($sp)
/* 005558 8002A158 8FB10014 */  lw    $s1, 0x14($sp)
/* 00555C 8002A15C 8FB00010 */  lw    $s0, 0x10($sp)
/* 005560 8002A160 D7B40020 */  ldc1  $f20, 0x20($sp)
/* 005564 8002A164 03E00008 */  jr    $ra
/* 005568 8002A168 27BD0028 */   addiu $sp, $sp, 0x28

func_8002A16C:
/* 00556C 8002A16C 3C014622 */  lui   $at, 0x4622
/* 005570 8002A170 3421F983 */  ori   $at, $at, 0xf983
/* 005574 8002A174 44810000 */  mtc1  $at, $f0
/* 005578 8002A178 00000000 */  nop   
/* 00557C 8002A17C 46006302 */  mul.s $f12, $f12, $f0
/* 005580 8002A180 00000000 */  nop   
/* 005584 8002A184 27BDFFE8 */  addiu $sp, $sp, -0x18
/* 005588 8002A188 AFBF0010 */  sw    $ra, 0x10($sp)
/* 00558C 8002A18C 0C00A80D */  jal   func_8002A034
/* 005590 8002A190 00000000 */   nop   
/* 005594 8002A194 0C01917C */  jal   func_800645F0
/* 005598 8002A198 3044FFFF */   andi  $a0, $v0, 0xffff
/* 00559C 8002A19C 00021400 */  sll   $v0, $v0, 0x10
/* 0055A0 8002A1A0 00021403 */  sra   $v0, $v0, 0x10
/* 0055A4 8002A1A4 3C018009 */  lui   $at, 0x8009
/* 0055A8 8002A1A8 D4227D68 */  ldc1  $f2, 0x7d68($at)
/* 0055AC 8002A1AC 44820000 */  mtc1  $v0, $f0
/* 0055B0 8002A1B0 00000000 */  nop   
/* 0055B4 8002A1B4 46800021 */  cvt.d.w $f0, $f0
/* 0055B8 8002A1B8 46220002 */  mul.d $f0, $f0, $f2
/* 0055BC 8002A1BC 00000000 */  nop   
/* 0055C0 8002A1C0 8FBF0010 */  lw    $ra, 0x10($sp)
/* 0055C4 8002A1C4 46200020 */  cvt.s.d $f0, $f0
/* 0055C8 8002A1C8 03E00008 */  jr    $ra
/* 0055CC 8002A1CC 27BD0018 */   addiu $sp, $sp, 0x18

func_8002A1D0:
/* 0055D0 8002A1D0 3C014622 */  lui   $at, 0x4622
/* 0055D4 8002A1D4 3421F983 */  ori   $at, $at, 0xf983
/* 0055D8 8002A1D8 44810000 */  mtc1  $at, $f0
/* 0055DC 8002A1DC 00000000 */  nop   
/* 0055E0 8002A1E0 46006302 */  mul.s $f12, $f12, $f0
/* 0055E4 8002A1E4 00000000 */  nop   
/* 0055E8 8002A1E8 27BDFFE8 */  addiu $sp, $sp, -0x18
/* 0055EC 8002A1EC AFBF0010 */  sw    $ra, 0x10($sp)
/* 0055F0 8002A1F0 0C00A80D */  jal   func_8002A034
/* 0055F4 8002A1F4 00000000 */   nop   
/* 0055F8 8002A1F8 0C0187C0 */  jal   func_80061F00
/* 0055FC 8002A1FC 3044FFFF */   andi  $a0, $v0, 0xffff
/* 005600 8002A200 00021400 */  sll   $v0, $v0, 0x10
/* 005604 8002A204 00021403 */  sra   $v0, $v0, 0x10
/* 005608 8002A208 3C018009 */  lui   $at, 0x8009
/* 00560C 8002A20C D4227D70 */  ldc1  $f2, 0x7d70($at)
/* 005610 8002A210 44820000 */  mtc1  $v0, $f0
/* 005614 8002A214 00000000 */  nop   
/* 005618 8002A218 46800021 */  cvt.d.w $f0, $f0
/* 00561C 8002A21C 46220002 */  mul.d $f0, $f0, $f2
/* 005620 8002A220 00000000 */  nop   
/* 005624 8002A224 8FBF0010 */  lw    $ra, 0x10($sp)
/* 005628 8002A228 46200020 */  cvt.s.d $f0, $f0
/* 00562C 8002A22C 03E00008 */  jr    $ra
/* 005630 8002A230 27BD0018 */   addiu $sp, $sp, 0x18

func_8002A234:
/* 005634 8002A234 3C014336 */  lui   $at, 0x4336
/* 005638 8002A238 34210B61 */  ori   $at, $at, 0xb61
/* 00563C 8002A23C 44810000 */  mtc1  $at, $f0
/* 005640 8002A240 27BDFFD8 */  addiu $sp, $sp, -0x28
/* 005644 8002A244 AFB10014 */  sw    $s1, 0x14($sp)
/* 005648 8002A248 00A0882D */  move  $s1, $a1
/* 00564C 8002A24C AFB20018 */  sw    $s2, 0x18($sp)
/* 005650 8002A250 46006302 */  mul.s $f12, $f12, $f0
/* 005654 8002A254 00000000 */  nop   
/* 005658 8002A258 AFBF001C */  sw    $ra, 0x1c($sp)
/* 00565C 8002A25C AFB00010 */  sw    $s0, 0x10($sp)
/* 005660 8002A260 F7B40020 */  sdc1  $f20, 0x20($sp)
/* 005664 8002A264 0C00A80D */  jal   func_8002A034
/* 005668 8002A268 00C0902D */   move  $s2, $a2
/* 00566C 8002A26C 3050FFFF */  andi  $s0, $v0, 0xffff
/* 005670 8002A270 0C01917C */  jal   func_800645F0
/* 005674 8002A274 0200202D */   move  $a0, $s0
/* 005678 8002A278 00021400 */  sll   $v0, $v0, 0x10
/* 00567C 8002A27C 00021403 */  sra   $v0, $v0, 0x10
/* 005680 8002A280 3C018009 */  lui   $at, 0x8009
/* 005684 8002A284 D4347D78 */  ldc1  $f20, 0x7d78($at)
/* 005688 8002A288 44820000 */  mtc1  $v0, $f0
/* 00568C 8002A28C 00000000 */  nop   
/* 005690 8002A290 46800021 */  cvt.d.w $f0, $f0
/* 005694 8002A294 46340002 */  mul.d $f0, $f0, $f20
/* 005698 8002A298 00000000 */  nop   
/* 00569C 8002A29C 0200202D */  move  $a0, $s0
/* 0056A0 8002A2A0 46200020 */  cvt.s.d $f0, $f0
/* 0056A4 8002A2A4 0C0187C0 */  jal   func_80061F00
/* 0056A8 8002A2A8 E6200000 */   swc1  $f0, ($s1)
/* 0056AC 8002A2AC 00021400 */  sll   $v0, $v0, 0x10
/* 0056B0 8002A2B0 00021403 */  sra   $v0, $v0, 0x10
/* 0056B4 8002A2B4 44820000 */  mtc1  $v0, $f0
/* 0056B8 8002A2B8 00000000 */  nop   
/* 0056BC 8002A2BC 46800021 */  cvt.d.w $f0, $f0
/* 0056C0 8002A2C0 46340002 */  mul.d $f0, $f0, $f20
/* 0056C4 8002A2C4 00000000 */  nop   
/* 0056C8 8002A2C8 46200020 */  cvt.s.d $f0, $f0
/* 0056CC 8002A2CC E6400000 */  swc1  $f0, ($s2)
/* 0056D0 8002A2D0 8FBF001C */  lw    $ra, 0x1c($sp)
/* 0056D4 8002A2D4 8FB20018 */  lw    $s2, 0x18($sp)
/* 0056D8 8002A2D8 8FB10014 */  lw    $s1, 0x14($sp)
/* 0056DC 8002A2DC 8FB00010 */  lw    $s0, 0x10($sp)
/* 0056E0 8002A2E0 D7B40020 */  ldc1  $f20, 0x20($sp)
/* 0056E4 8002A2E4 03E00008 */  jr    $ra
/* 0056E8 8002A2E8 27BD0028 */   addiu $sp, $sp, 0x28

func_8002A2EC:
/* 0056EC 8002A2EC 3C014336 */  lui   $at, 0x4336
/* 0056F0 8002A2F0 34210B61 */  ori   $at, $at, 0xb61
/* 0056F4 8002A2F4 44810000 */  mtc1  $at, $f0
/* 0056F8 8002A2F8 00000000 */  nop   
/* 0056FC 8002A2FC 46006302 */  mul.s $f12, $f12, $f0
/* 005700 8002A300 00000000 */  nop   
/* 005704 8002A304 27BDFFE8 */  addiu $sp, $sp, -0x18
/* 005708 8002A308 AFBF0010 */  sw    $ra, 0x10($sp)
/* 00570C 8002A30C 0C00A80D */  jal   func_8002A034
/* 005710 8002A310 00000000 */   nop   
/* 005714 8002A314 0C01917C */  jal   func_800645F0
/* 005718 8002A318 3044FFFF */   andi  $a0, $v0, 0xffff
/* 00571C 8002A31C 00021400 */  sll   $v0, $v0, 0x10
/* 005720 8002A320 00021403 */  sra   $v0, $v0, 0x10
/* 005724 8002A324 3C018009 */  lui   $at, 0x8009
/* 005728 8002A328 D4227D80 */  ldc1  $f2, 0x7d80($at)
/* 00572C 8002A32C 44820000 */  mtc1  $v0, $f0
/* 005730 8002A330 00000000 */  nop   
/* 005734 8002A334 46800021 */  cvt.d.w $f0, $f0
/* 005738 8002A338 46220002 */  mul.d $f0, $f0, $f2
/* 00573C 8002A33C 00000000 */  nop   
/* 005740 8002A340 8FBF0010 */  lw    $ra, 0x10($sp)
/* 005744 8002A344 46200020 */  cvt.s.d $f0, $f0
/* 005748 8002A348 03E00008 */  jr    $ra
/* 00574C 8002A34C 27BD0018 */   addiu $sp, $sp, 0x18

func_8002A350:
/* 005750 8002A350 3C014336 */  lui   $at, 0x4336
/* 005754 8002A354 34210B61 */  ori   $at, $at, 0xb61
/* 005758 8002A358 44810000 */  mtc1  $at, $f0
/* 00575C 8002A35C 00000000 */  nop   
/* 005760 8002A360 46006302 */  mul.s $f12, $f12, $f0
/* 005764 8002A364 00000000 */  nop   
/* 005768 8002A368 27BDFFE8 */  addiu $sp, $sp, -0x18
/* 00576C 8002A36C AFBF0010 */  sw    $ra, 0x10($sp)
/* 005770 8002A370 0C00A80D */  jal   func_8002A034
/* 005774 8002A374 00000000 */   nop   
/* 005778 8002A378 0C0187C0 */  jal   func_80061F00
/* 00577C 8002A37C 3044FFFF */   andi  $a0, $v0, 0xffff
/* 005780 8002A380 00021400 */  sll   $v0, $v0, 0x10
/* 005784 8002A384 00021403 */  sra   $v0, $v0, 0x10
/* 005788 8002A388 3C018009 */  lui   $at, 0x8009
/* 00578C 8002A38C D4227D88 */  ldc1  $f2, 0x7d88($at)
/* 005790 8002A390 44820000 */  mtc1  $v0, $f0
/* 005794 8002A394 00000000 */  nop   
/* 005798 8002A398 46800021 */  cvt.d.w $f0, $f0
/* 00579C 8002A39C 46220002 */  mul.d $f0, $f0, $f2
/* 0057A0 8002A3A0 00000000 */  nop   
/* 0057A4 8002A3A4 8FBF0010 */  lw    $ra, 0x10($sp)
/* 0057A8 8002A3A8 46200020 */  cvt.s.d $f0, $f0
/* 0057AC 8002A3AC 03E00008 */  jr    $ra
/* 0057B0 8002A3B0 27BD0018 */   addiu $sp, $sp, 0x18

func_8002A3B4:
/* 0057B4 8002A3B4 27BDFFC8 */  addiu $sp, $sp, -0x38
/* 0057B8 8002A3B8 F7B60028 */  sdc1  $f22, 0x28($sp)
/* 0057BC 8002A3BC 4485B000 */  mtc1  $a1, $f22
/* 0057C0 8002A3C0 F7B80030 */  sdc1  $f24, 0x30($sp)
/* 0057C4 8002A3C4 4486C000 */  mtc1  $a2, $f24
/* 0057C8 8002A3C8 AFB00010 */  sw    $s0, 0x10($sp)
/* 0057CC 8002A3CC 8FB00048 */  lw    $s0, 0x48($sp)
/* 0057D0 8002A3D0 AFB10014 */  sw    $s1, 0x14($sp)
/* 0057D4 8002A3D4 AFBF0018 */  sw    $ra, 0x18($sp)
/* 0057D8 8002A3D8 F7B40020 */  sdc1  $f20, 0x20($sp)
/* 0057DC 8002A3DC 16000003 */  bnez  $s0, .L8002A3EC
/* 0057E0 8002A3E0 00E0882D */   move  $s1, $a3
/* 0057E4 8002A3E4 0800AA39 */  j     func_8002A8E4
/* 0057E8 8002A3E8 4600C006 */   mov.s $f0, $f24

.L8002A3EC:
/* 0057EC 8002A3EC 2C82000D */  sltiu $v0, $a0, 0xd
/* 0057F0 8002A3F0 1040013B */  beqz  $v0, .L8002A8E0
/* 0057F4 8002A3F4 00041080 */   sll   $v0, $a0, 2
/* 0057F8 8002A3F8 3C018009 */  lui   $at, 0x8009
/* 0057FC 8002A3FC 00220821 */  addu  $at, $at, $v0
/* 005800 8002A400 8C227D90 */  lw    $v0, 0x7d90($at)
/* 005804 8002A404 00400008 */  jr    $v0
/* 005808 8002A408 00000000 */   nop   
/* 00580C 8002A40C 4616C001 */  sub.s $f0, $f24, $f22
/* 005810 8002A410 44911000 */  mtc1  $s1, $f2
/* 005814 8002A414 00000000 */  nop   
/* 005818 8002A418 468010A0 */  cvt.s.w $f2, $f2
/* 00581C 8002A41C 46020002 */  mul.s $f0, $f0, $f2
/* 005820 8002A420 00000000 */  nop   
/* 005824 8002A424 44901000 */  mtc1  $s0, $f2
/* 005828 8002A428 00000000 */  nop   
/* 00582C 8002A42C 468010A0 */  cvt.s.w $f2, $f2
/* 005830 8002A430 0800A938 */  j     func_8002A4E0
/* 005834 8002A434 46020003 */   div.s $f0, $f0, $f2

/* 005838 8002A438 02310018 */  mult  $s1, $s1
/* 00583C 8002A43C 00004012 */  mflo  $t0
/* 005840 8002A440 00000000 */  nop   
/* 005844 8002A444 0800A92D */  j     func_8002A4B4
/* 005848 8002A448 02100018 */   mult  $s0, $s0

/* 00584C 8002A44C 02310018 */  mult  $s1, $s1
/* 005850 8002A450 00004012 */  mflo  $t0
/* 005854 8002A454 00000000 */  nop   
/* 005858 8002A458 0800A925 */  j     func_8002A494
/* 00585C 8002A45C 02100018 */   mult  $s0, $s0

/* 005860 8002A460 02310018 */  mult  $s1, $s1
/* 005864 8002A464 00004012 */  mflo  $t0
/* 005868 8002A468 00000000 */  nop   
/* 00586C 8002A46C 00000000 */  nop   
/* 005870 8002A470 02100018 */  mult  $s0, $s0
/* 005874 8002A474 00001012 */  mflo  $v0
/* 005878 8002A478 00000000 */  nop   
/* 00587C 8002A47C 00000000 */  nop   
/* 005880 8002A480 01110018 */  mult  $t0, $s1
/* 005884 8002A484 00004012 */  mflo  $t0
/* 005888 8002A488 00000000 */  nop   
/* 00588C 8002A48C 00000000 */  nop   
/* 005890 8002A490 00500018 */  mult  $v0, $s0
func_8002A494:
/* 005894 8002A494 00001012 */  mflo  $v0
/* 005898 8002A498 00000000 */  nop   
/* 00589C 8002A49C 00000000 */  nop   
/* 0058A0 8002A4A0 01110018 */  mult  $t0, $s1
/* 0058A4 8002A4A4 00004012 */  mflo  $t0
/* 0058A8 8002A4A8 00000000 */  nop   
/* 0058AC 8002A4AC 00000000 */  nop   
/* 0058B0 8002A4B0 00500018 */  mult  $v0, $s0
func_8002A4B4:
/* 0058B4 8002A4B4 4616C081 */  sub.s $f2, $f24, $f22
/* 0058B8 8002A4B8 44880000 */  mtc1  $t0, $f0
/* 0058BC 8002A4BC 00000000 */  nop   
/* 0058C0 8002A4C0 46800020 */  cvt.s.w $f0, $f0
/* 0058C4 8002A4C4 46020002 */  mul.s $f0, $f0, $f2
/* 0058C8 8002A4C8 00000000 */  nop   
/* 0058CC 8002A4CC 00001012 */  mflo  $v0
/* 0058D0 8002A4D0 44821000 */  mtc1  $v0, $f2
/* 0058D4 8002A4D4 00000000 */  nop   
/* 0058D8 8002A4D8 468010A0 */  cvt.s.w $f2, $f2
/* 0058DC 8002A4DC 46020003 */  div.s $f0, $f0, $f2
func_8002A4E0:
/* 0058E0 8002A4E0 0800AA39 */  j     func_8002A8E4
/* 0058E4 8002A4E4 4600B000 */   add.s $f0, $f22, $f0

/* 0058E8 8002A4E8 44916000 */  mtc1  $s1, $f12
/* 0058EC 8002A4EC 00000000 */  nop   
/* 0058F0 8002A4F0 46806320 */  cvt.s.w $f12, $f12
/* 0058F4 8002A4F4 4490A000 */  mtc1  $s0, $f20
/* 0058F8 8002A4F8 00000000 */  nop   
/* 0058FC 8002A4FC 4680A520 */  cvt.s.w $f20, $f20
/* 005900 8002A500 46146303 */  div.s $f12, $f12, $f20
/* 005904 8002A504 3C018009 */  lui   $at, 0x8009
/* 005908 8002A508 D4207DC8 */  ldc1  $f0, 0x7dc8($at)
/* 00590C 8002A50C 46006321 */  cvt.d.s $f12, $f12
/* 005910 8002A510 46206302 */  mul.d $f12, $f12, $f0
/* 005914 8002A514 00000000 */  nop   
/* 005918 8002A518 3C014010 */  lui   $at, 0x4010
/* 00591C 8002A51C 44810800 */  mtc1  $at, $f1
/* 005920 8002A520 44800000 */  mtc1  $zero, $f0
/* 005924 8002A524 00000000 */  nop   
/* 005928 8002A528 46206302 */  mul.d $f12, $f12, $f0
/* 00592C 8002A52C 0800A966 */  j     func_8002A598
/* 005930 8002A530 00000000 */   nop   

/* 005934 8002A534 02310018 */  mult  $s1, $s1
/* 005938 8002A538 4490A000 */  mtc1  $s0, $f20
/* 00593C 8002A53C 00000000 */  nop   
/* 005940 8002A540 4680A520 */  cvt.s.w $f20, $f20
/* 005944 8002A544 3C018009 */  lui   $at, 0x8009
/* 005948 8002A548 D4207DD0 */  ldc1  $f0, 0x7dd0($at)
/* 00594C 8002A54C 00001812 */  mflo  $v1
/* 005950 8002A550 44836000 */  mtc1  $v1, $f12
/* 005954 8002A554 00000000 */  nop   
/* 005958 8002A558 46806320 */  cvt.s.w $f12, $f12
/* 00595C 8002A55C 46146303 */  div.s $f12, $f12, $f20
/* 005960 8002A560 46006321 */  cvt.d.s $f12, $f12
/* 005964 8002A564 46206302 */  mul.d $f12, $f12, $f0
/* 005968 8002A568 00000000 */  nop   
/* 00596C 8002A56C 3C014010 */  lui   $at, 0x4010
/* 005970 8002A570 44810800 */  mtc1  $at, $f1
/* 005974 8002A574 44800000 */  mtc1  $zero, $f0
/* 005978 8002A578 00000000 */  nop   
/* 00597C 8002A57C 46206302 */  mul.d $f12, $f12, $f0
/* 005980 8002A580 00000000 */  nop   
/* 005984 8002A584 3C01402E */  lui   $at, 0x402e
/* 005988 8002A588 44810800 */  mtc1  $at, $f1
/* 00598C 8002A58C 44800000 */  mtc1  $zero, $f0
/* 005990 8002A590 00000000 */  nop   
/* 005994 8002A594 46206303 */  div.d $f12, $f12, $f0
func_8002A598:
/* 005998 8002A598 0C00A874 */  jal   func_8002A1D0
/* 00599C 8002A59C 46206320 */   cvt.s.d $f12, $f12
/* 0059A0 8002A5A0 4614A502 */  mul.s $f20, $f20, $f20
/* 0059A4 8002A5A4 00000000 */  nop   
/* 0059A8 8002A5A8 4616C081 */  sub.s $f2, $f24, $f22
/* 0059AC 8002A5AC 46001082 */  mul.s $f2, $f2, $f0
/* 0059B0 8002A5B0 00000000 */  nop   
/* 0059B4 8002A5B4 02111023 */  subu  $v0, $s0, $s1
/* 0059B8 8002A5B8 44820000 */  mtc1  $v0, $f0
/* 0059BC 8002A5BC 00000000 */  nop   
/* 0059C0 8002A5C0 46800020 */  cvt.s.w $f0, $f0
/* 0059C4 8002A5C4 46001082 */  mul.s $f2, $f2, $f0
/* 0059C8 8002A5C8 00000000 */  nop   
/* 0059CC 8002A5CC 46001082 */  mul.s $f2, $f2, $f0
/* 0059D0 8002A5D0 00000000 */  nop   
/* 0059D4 8002A5D4 46141083 */  div.s $f2, $f2, $f20
/* 0059D8 8002A5D8 0800AA39 */  j     func_8002A8E4
/* 0059DC 8002A5DC 4602C001 */   sub.s $f0, $f24, $f2

/* 0059E0 8002A5E0 02113823 */  subu  $a3, $s0, $s1
/* 0059E4 8002A5E4 00E70018 */  mult  $a3, $a3
/* 0059E8 8002A5E8 4616C101 */  sub.s $f4, $f24, $f22
/* 0059EC 8002A5EC 00001812 */  mflo  $v1
/* 0059F0 8002A5F0 44830000 */  mtc1  $v1, $f0
/* 0059F4 8002A5F4 00000000 */  nop   
/* 0059F8 8002A5F8 46800020 */  cvt.s.w $f0, $f0
/* 0059FC 8002A5FC 46040002 */  mul.s $f0, $f0, $f4
/* 005A00 8002A600 00000000 */  nop   
/* 005A04 8002A604 02100018 */  mult  $s0, $s0
/* 005A08 8002A608 00001812 */  mflo  $v1
/* 005A0C 8002A60C 44831000 */  mtc1  $v1, $f2
/* 005A10 8002A610 00000000 */  nop   
/* 005A14 8002A614 468010A0 */  cvt.s.w $f2, $f2
/* 005A18 8002A618 0800A9B3 */  j     func_8002A6CC
/* 005A1C 8002A61C 4604B100 */   add.s $f4, $f22, $f4

/* 005A20 8002A620 02113823 */  subu  $a3, $s0, $s1
/* 005A24 8002A624 00E70018 */  mult  $a3, $a3
/* 005A28 8002A628 00001012 */  mflo  $v0
/* 005A2C 8002A62C 00000000 */  nop   
/* 005A30 8002A630 00000000 */  nop   
/* 005A34 8002A634 00470018 */  mult  $v0, $a3
/* 005A38 8002A638 00004012 */  mflo  $t0
/* 005A3C 8002A63C 00000000 */  nop   
/* 005A40 8002A640 0800A9A4 */  j     func_8002A690
/* 005A44 8002A644 02100018 */   mult  $s0, $s0

/* 005A48 8002A648 02113823 */  subu  $a3, $s0, $s1
/* 005A4C 8002A64C 00E70018 */  mult  $a3, $a3
/* 005A50 8002A650 00001012 */  mflo  $v0
/* 005A54 8002A654 00000000 */  nop   
/* 005A58 8002A658 00000000 */  nop   
/* 005A5C 8002A65C 00470018 */  mult  $v0, $a3
/* 005A60 8002A660 00004012 */  mflo  $t0
/* 005A64 8002A664 00000000 */  nop   
/* 005A68 8002A668 00000000 */  nop   
/* 005A6C 8002A66C 02100018 */  mult  $s0, $s0
/* 005A70 8002A670 00001012 */  mflo  $v0
/* 005A74 8002A674 00000000 */  nop   
/* 005A78 8002A678 00000000 */  nop   
/* 005A7C 8002A67C 01070018 */  mult  $t0, $a3
/* 005A80 8002A680 00004012 */  mflo  $t0
/* 005A84 8002A684 00000000 */  nop   
/* 005A88 8002A688 00000000 */  nop   
/* 005A8C 8002A68C 00500018 */  mult  $v0, $s0
func_8002A690:
/* 005A90 8002A690 4616C101 */  sub.s $f4, $f24, $f22
/* 005A94 8002A694 44880000 */  mtc1  $t0, $f0
/* 005A98 8002A698 00000000 */  nop   
/* 005A9C 8002A69C 46800020 */  cvt.s.w $f0, $f0
/* 005AA0 8002A6A0 46040002 */  mul.s $f0, $f0, $f4
/* 005AA4 8002A6A4 00000000 */  nop   
/* 005AA8 8002A6A8 00001012 */  mflo  $v0
/* 005AAC 8002A6AC 00000000 */  nop   
/* 005AB0 8002A6B0 00000000 */  nop   
/* 005AB4 8002A6B4 00500018 */  mult  $v0, $s0
/* 005AB8 8002A6B8 4604B100 */  add.s $f4, $f22, $f4
/* 005ABC 8002A6BC 00001012 */  mflo  $v0
/* 005AC0 8002A6C0 44821000 */  mtc1  $v0, $f2
/* 005AC4 8002A6C4 00000000 */  nop   
/* 005AC8 8002A6C8 468010A0 */  cvt.s.w $f2, $f2
func_8002A6CC:
/* 005ACC 8002A6CC 46020003 */  div.s $f0, $f0, $f2
/* 005AD0 8002A6D0 0800AA39 */  j     func_8002A8E4
/* 005AD4 8002A6D4 46002001 */   sub.s $f0, $f4, $f0

/* 005AD8 8002A6D8 02310018 */  mult  $s1, $s1
/* 005ADC 8002A6DC 4490A000 */  mtc1  $s0, $f20
/* 005AE0 8002A6E0 00000000 */  nop   
/* 005AE4 8002A6E4 4680A520 */  cvt.s.w $f20, $f20
/* 005AE8 8002A6E8 3C018009 */  lui   $at, 0x8009
/* 005AEC 8002A6EC D4207DD8 */  ldc1  $f0, 0x7dd8($at)
/* 005AF0 8002A6F0 00001812 */  mflo  $v1
/* 005AF4 8002A6F4 44836000 */  mtc1  $v1, $f12
/* 005AF8 8002A6F8 00000000 */  nop   
/* 005AFC 8002A6FC 46806320 */  cvt.s.w $f12, $f12
/* 005B00 8002A700 46146303 */  div.s $f12, $f12, $f20
/* 005B04 8002A704 46006321 */  cvt.d.s $f12, $f12
/* 005B08 8002A708 46206302 */  mul.d $f12, $f12, $f0
/* 005B0C 8002A70C 00000000 */  nop   
/* 005B10 8002A710 3C014010 */  lui   $at, 0x4010
/* 005B14 8002A714 44810800 */  mtc1  $at, $f1
/* 005B18 8002A718 44800000 */  mtc1  $zero, $f0
/* 005B1C 8002A71C 00000000 */  nop   
/* 005B20 8002A720 46206302 */  mul.d $f12, $f12, $f0
/* 005B24 8002A724 00000000 */  nop   
/* 005B28 8002A728 3C014044 */  lui   $at, 0x4044
/* 005B2C 8002A72C 44810800 */  mtc1  $at, $f1
/* 005B30 8002A730 44800000 */  mtc1  $zero, $f0
/* 005B34 8002A734 00000000 */  nop   
/* 005B38 8002A738 46206303 */  div.d $f12, $f12, $f0
/* 005B3C 8002A73C 0C00A874 */  jal   func_8002A1D0
/* 005B40 8002A740 46206320 */   cvt.s.d $f12, $f12
/* 005B44 8002A744 02111023 */  subu  $v0, $s0, $s1
/* 005B48 8002A748 44821000 */  mtc1  $v0, $f2
/* 005B4C 8002A74C 00000000 */  nop   
/* 005B50 8002A750 468010A0 */  cvt.s.w $f2, $f2
/* 005B54 8002A754 46020002 */  mul.s $f0, $f0, $f2
/* 005B58 8002A758 00000000 */  nop   
/* 005B5C 8002A75C 4614A502 */  mul.s $f20, $f20, $f20
/* 005B60 8002A760 00000000 */  nop   
/* 005B64 8002A764 46020002 */  mul.s $f0, $f0, $f2
/* 005B68 8002A768 00000000 */  nop   
/* 005B6C 8002A76C 44801000 */  mtc1  $zero, $f2
/* 005B70 8002A770 46140103 */  div.s $f4, $f0, $f20
/* 005B74 8002A774 4602203C */  c.lt.s $f4, $f2
/* 005B78 8002A778 00000000 */  nop   
/* 005B7C 8002A77C 45030001 */  bc1tl .L8002A784
/* 005B80 8002A780 46002107 */   neg.s $f4, $f4
.L8002A784:
/* 005B84 8002A784 4616C001 */  sub.s $f0, $f24, $f22
/* 005B88 8002A788 46040002 */  mul.s $f0, $f0, $f4
/* 005B8C 8002A78C 00000000 */  nop   
/* 005B90 8002A790 0800AA39 */  j     func_8002A8E4
/* 005B94 8002A794 4600C001 */   sub.s $f0, $f24, $f0

/* 005B98 8002A798 44916000 */  mtc1  $s1, $f12
/* 005B9C 8002A79C 00000000 */  nop   
/* 005BA0 8002A7A0 46806320 */  cvt.s.w $f12, $f12
/* 005BA4 8002A7A4 3C018009 */  lui   $at, 0x8009
/* 005BA8 8002A7A8 D4207DE0 */  ldc1  $f0, 0x7de0($at)
/* 005BAC 8002A7AC 46006321 */  cvt.d.s $f12, $f12
/* 005BB0 8002A7B0 46206302 */  mul.d $f12, $f12, $f0
/* 005BB4 8002A7B4 00000000 */  nop   
/* 005BB8 8002A7B8 44900000 */  mtc1  $s0, $f0
/* 005BBC 8002A7BC 00000000 */  nop   
/* 005BC0 8002A7C0 46800020 */  cvt.s.w $f0, $f0
/* 005BC4 8002A7C4 46000021 */  cvt.d.s $f0, $f0
/* 005BC8 8002A7C8 46206303 */  div.d $f12, $f12, $f0
/* 005BCC 8002A7CC 0C00A874 */  jal   func_8002A1D0
/* 005BD0 8002A7D0 46206320 */   cvt.s.d $f12, $f12
/* 005BD4 8002A7D4 4616C081 */  sub.s $f2, $f24, $f22
/* 005BD8 8002A7D8 3C013FF0 */  lui   $at, 0x3ff0
/* 005BDC 8002A7DC 44812800 */  mtc1  $at, $f5
/* 005BE0 8002A7E0 44802000 */  mtc1  $zero, $f4
/* 005BE4 8002A7E4 46000021 */  cvt.d.s $f0, $f0
/* 005BE8 8002A7E8 46202101 */  sub.d $f4, $f4, $f0
/* 005BEC 8002A7EC 460010A1 */  cvt.d.s $f2, $f2
/* 005BF0 8002A7F0 46241082 */  mul.d $f2, $f2, $f4
/* 005BF4 8002A7F4 00000000 */  nop   
/* 005BF8 8002A7F8 3C013FE0 */  lui   $at, 0x3fe0
/* 005BFC 8002A7FC 44810800 */  mtc1  $at, $f1
/* 005C00 8002A800 44800000 */  mtc1  $zero, $f0
/* 005C04 8002A804 00000000 */  nop   
/* 005C08 8002A808 46201082 */  mul.d $f2, $f2, $f0
/* 005C0C 8002A80C 00000000 */  nop   
/* 005C10 8002A810 4600B021 */  cvt.d.s $f0, $f22
/* 005C14 8002A814 46220000 */  add.d $f0, $f0, $f2
/* 005C18 8002A818 0800AA39 */  j     func_8002A8E4
/* 005C1C 8002A81C 46200020 */   cvt.s.d $f0, $f0

/* 005C20 8002A820 44916000 */  mtc1  $s1, $f12
/* 005C24 8002A824 00000000 */  nop   
/* 005C28 8002A828 46806320 */  cvt.s.w $f12, $f12
/* 005C2C 8002A82C 3C018009 */  lui   $at, 0x8009
/* 005C30 8002A830 D4207DE8 */  ldc1  $f0, 0x7de8($at)
/* 005C34 8002A834 46006321 */  cvt.d.s $f12, $f12
/* 005C38 8002A838 46206302 */  mul.d $f12, $f12, $f0
/* 005C3C 8002A83C 00000000 */  nop   
/* 005C40 8002A840 44900000 */  mtc1  $s0, $f0
/* 005C44 8002A844 00000000 */  nop   
/* 005C48 8002A848 46800020 */  cvt.s.w $f0, $f0
/* 005C4C 8002A84C 46000021 */  cvt.d.s $f0, $f0
/* 005C50 8002A850 46206303 */  div.d $f12, $f12, $f0
/* 005C54 8002A854 0C00A85B */  jal   func_8002A16C
/* 005C58 8002A858 46206320 */   cvt.s.d $f12, $f12
/* 005C5C 8002A85C 4616C081 */  sub.s $f2, $f24, $f22
/* 005C60 8002A860 46001082 */  mul.s $f2, $f2, $f0
/* 005C64 8002A864 00000000 */  nop   
/* 005C68 8002A868 0800AA39 */  j     func_8002A8E4
/* 005C6C 8002A86C 4602B000 */   add.s $f0, $f22, $f2

/* 005C70 8002A870 44916000 */  mtc1  $s1, $f12
/* 005C74 8002A874 00000000 */  nop   
/* 005C78 8002A878 46806320 */  cvt.s.w $f12, $f12
/* 005C7C 8002A87C 3C018009 */  lui   $at, 0x8009
/* 005C80 8002A880 D4207DF0 */  ldc1  $f0, 0x7df0($at)
/* 005C84 8002A884 46006321 */  cvt.d.s $f12, $f12
/* 005C88 8002A888 46206302 */  mul.d $f12, $f12, $f0
/* 005C8C 8002A88C 00000000 */  nop   
/* 005C90 8002A890 44900000 */  mtc1  $s0, $f0
/* 005C94 8002A894 00000000 */  nop   
/* 005C98 8002A898 46800020 */  cvt.s.w $f0, $f0
/* 005C9C 8002A89C 46000021 */  cvt.d.s $f0, $f0
/* 005CA0 8002A8A0 46206303 */  div.d $f12, $f12, $f0
/* 005CA4 8002A8A4 0C00A874 */  jal   func_8002A1D0
/* 005CA8 8002A8A8 46206320 */   cvt.s.d $f12, $f12
/* 005CAC 8002A8AC 4616C101 */  sub.s $f4, $f24, $f22
/* 005CB0 8002A8B0 3C013FF0 */  lui   $at, 0x3ff0
/* 005CB4 8002A8B4 44811800 */  mtc1  $at, $f3
/* 005CB8 8002A8B8 44801000 */  mtc1  $zero, $f2
/* 005CBC 8002A8BC 46000021 */  cvt.d.s $f0, $f0
/* 005CC0 8002A8C0 46201081 */  sub.d $f2, $f2, $f0
/* 005CC4 8002A8C4 46002121 */  cvt.d.s $f4, $f4
/* 005CC8 8002A8C8 46222102 */  mul.d $f4, $f4, $f2
/* 005CCC 8002A8CC 00000000 */  nop   
/* 005CD0 8002A8D0 4600B021 */  cvt.d.s $f0, $f22
/* 005CD4 8002A8D4 46240000 */  add.d $f0, $f0, $f4
/* 005CD8 8002A8D8 0800AA39 */  j     func_8002A8E4
/* 005CDC 8002A8DC 46200020 */   cvt.s.d $f0, $f0

.L8002A8E0:
/* 005CE0 8002A8E0 44800000 */  mtc1  $zero, $f0
func_8002A8E4:
/* 005CE4 8002A8E4 8FBF0018 */  lw    $ra, 0x18($sp)
/* 005CE8 8002A8E8 8FB10014 */  lw    $s1, 0x14($sp)
/* 005CEC 8002A8EC 8FB00010 */  lw    $s0, 0x10($sp)
/* 005CF0 8002A8F0 D7B80030 */  ldc1  $f24, 0x30($sp)
/* 005CF4 8002A8F4 D7B60028 */  ldc1  $f22, 0x28($sp)
/* 005CF8 8002A8F8 D7B40020 */  ldc1  $f20, 0x20($sp)
/* 005CFC 8002A8FC 03E00008 */  jr    $ra
/* 005D00 8002A900 27BD0038 */   addiu $sp, $sp, 0x38

func_8002A904:
/* 005D04 8002A904 27BDFFF8 */  addiu $sp, $sp, -8
/* 005D08 8002A908 0080602D */  move  $t4, $a0
/* 005D0C 8002A90C 00A0682D */  move  $t5, $a1
/* 005D10 8002A910 00E0582D */  move  $t3, $a3
/* 005D14 8002A914 3C08800A */  lui   $t0, 0x800a
/* 005D18 8002A918 2508A66C */  addiu $t0, $t0, -0x5994
/* 005D1C 8002A91C 3C02E700 */  lui   $v0, 0xe700
/* 005D20 8002A920 AFB00000 */  sw    $s0, ($sp)
/* 005D24 8002A924 8D070000 */  lw    $a3, ($t0)
/* 005D28 8002A928 97B9001A */  lhu   $t9, 0x1a($sp)
/* 005D2C 8002A92C 97B0001E */  lhu   $s0, 0x1e($sp)
/* 005D30 8002A930 97AF0022 */  lhu   $t7, 0x22($sp)
/* 005D34 8002A934 97B80026 */  lhu   $t8, 0x26($sp)
/* 005D38 8002A938 00E0182D */  move  $v1, $a3
/* 005D3C 8002A93C 24E70008 */  addiu $a3, $a3, 8
/* 005D40 8002A940 AD070000 */  sw    $a3, ($t0)
/* 005D44 8002A944 AC620000 */  sw    $v0, ($v1)
/* 005D48 8002A948 24E20008 */  addiu $v0, $a3, 8
/* 005D4C 8002A94C AC600004 */  sw    $zero, 4($v1)
/* 005D50 8002A950 AD020000 */  sw    $v0, ($t0)
/* 005D54 8002A954 3C02DE00 */  lui   $v0, 0xde00
/* 005D58 8002A958 ACE20000 */  sw    $v0, ($a3)
/* 005D5C 8002A95C 3C028007 */  lui   $v0, 0x8007
/* 005D60 8002A960 24424580 */  addiu $v0, $v0, 0x4580
/* 005D64 8002A964 316300FF */  andi  $v1, $t3, 0xff
/* 005D68 8002A968 ACE20004 */  sw    $v0, 4($a3)
/* 005D6C 8002A96C 240200FF */  addiu $v0, $zero, 0xff
/* 005D70 8002A970 1462000A */  bne   $v1, $v0, .L8002A99C
/* 005D74 8002A974 00C0702D */   move  $t6, $a2
/* 005D78 8002A978 3C05E200 */  lui   $a1, 0xe200
/* 005D7C 8002A97C 34A5001C */  ori   $a1, $a1, 0x1c
/* 005D80 8002A980 3C030F0A */  lui   $v1, 0xf0a
/* 005D84 8002A984 34634000 */  ori   $v1, $v1, 0x4000
/* 005D88 8002A988 3C06FCFF */  lui   $a2, 0xfcff
/* 005D8C 8002A98C 34C6FFFF */  ori   $a2, $a2, 0xffff
/* 005D90 8002A990 3C04FFFD */  lui   $a0, 0xfffd
/* 005D94 8002A994 0800AA6F */  j     func_8002A9BC
/* 005D98 8002A998 3484FCFE */   ori   $a0, $a0, 0xfcfe

.L8002A99C:
/* 005D9C 8002A99C 3C05E200 */  lui   $a1, 0xe200
/* 005DA0 8002A9A0 34A5001C */  ori   $a1, $a1, 0x1c
/* 005DA4 8002A9A4 3C030050 */  lui   $v1, 0x50
/* 005DA8 8002A9A8 34634240 */  ori   $v1, $v1, 0x4240
/* 005DAC 8002A9AC 3C06FCFF */  lui   $a2, 0xfcff
/* 005DB0 8002A9B0 34C6FFFF */  ori   $a2, $a2, 0xffff
/* 005DB4 8002A9B4 3C04FFFD */  lui   $a0, 0xfffd
/* 005DB8 8002A9B8 3484F6FB */  ori   $a0, $a0, 0xf6fb
func_8002A9BC:
/* 005DBC 8002A9BC 24E20010 */  addiu $v0, $a3, 0x10
/* 005DC0 8002A9C0 AD020000 */  sw    $v0, ($t0)
/* 005DC4 8002A9C4 24E20018 */  addiu $v0, $a3, 0x18
/* 005DC8 8002A9C8 ACE50008 */  sw    $a1, 8($a3)
/* 005DCC 8002A9CC ACE3000C */  sw    $v1, 0xc($a3)
/* 005DD0 8002A9D0 AD020000 */  sw    $v0, ($t0)
/* 005DD4 8002A9D4 ACE60010 */  sw    $a2, 0x10($a3)
/* 005DD8 8002A9D8 ACE40014 */  sw    $a0, 0x14($a3)
/* 005DDC 8002A9DC 3C09E200 */  lui   $t1, 0xe200
/* 005DE0 8002A9E0 3529001C */  ori   $t1, $t1, 0x1c
/* 005DE4 8002A9E4 3C070F0A */  lui   $a3, 0xf0a
/* 005DE8 8002A9E8 34E77008 */  ori   $a3, $a3, 0x7008
/* 005DEC 8002A9EC 3C0AFCFF */  lui   $t2, 0xfcff
/* 005DF0 8002A9F0 354AFFFF */  ori   $t2, $t2, 0xffff
/* 005DF4 8002A9F4 3C08FFFC */  lui   $t0, 0xfffc
/* 005DF8 8002A9F8 3C06800A */  lui   $a2, 0x800a
/* 005DFC 8002A9FC 24C6A66C */  addiu $a2, $a2, -0x5994
/* 005E00 8002AA00 3508F279 */  ori   $t0, $t0, 0xf279
/* 005E04 8002AA04 3C02FA00 */  lui   $v0, 0xfa00
/* 005E08 8002AA08 8CC50000 */  lw    $a1, ($a2)
/* 005E0C 8002AA0C 000C1E00 */  sll   $v1, $t4, 0x18
/* 005E10 8002AA10 00A0202D */  move  $a0, $a1
/* 005E14 8002AA14 24A50008 */  addiu $a1, $a1, 8
/* 005E18 8002AA18 ACC50000 */  sw    $a1, ($a2)
/* 005E1C 8002AA1C AC820000 */  sw    $v0, ($a0)
/* 005E20 8002AA20 31A200FF */  andi  $v0, $t5, 0xff
/* 005E24 8002AA24 00021400 */  sll   $v0, $v0, 0x10
/* 005E28 8002AA28 00621825 */  or    $v1, $v1, $v0
/* 005E2C 8002AA2C 31C200FF */  andi  $v0, $t6, 0xff
/* 005E30 8002AA30 00021200 */  sll   $v0, $v0, 8
/* 005E34 8002AA34 00621825 */  or    $v1, $v1, $v0
/* 005E38 8002AA38 316200FF */  andi  $v0, $t3, 0xff
/* 005E3C 8002AA3C 00621825 */  or    $v1, $v1, $v0
/* 005E40 8002AA40 24A20008 */  addiu $v0, $a1, 8
/* 005E44 8002AA44 AC830004 */  sw    $v1, 4($a0)
/* 005E48 8002AA48 31E403FF */  andi  $a0, $t7, 0x3ff
/* 005E4C 8002AA4C 00042380 */  sll   $a0, $a0, 0xe
/* 005E50 8002AA50 ACC20000 */  sw    $v0, ($a2)
/* 005E54 8002AA54 330203FF */  andi  $v0, $t8, 0x3ff
/* 005E58 8002AA58 00021080 */  sll   $v0, $v0, 2
/* 005E5C 8002AA5C 3C03F600 */  lui   $v1, 0xf600
/* 005E60 8002AA60 00431025 */  or    $v0, $v0, $v1
/* 005E64 8002AA64 00822025 */  or    $a0, $a0, $v0
/* 005E68 8002AA68 332303FF */  andi  $v1, $t9, 0x3ff
/* 005E6C 8002AA6C 00031B80 */  sll   $v1, $v1, 0xe
/* 005E70 8002AA70 320203FF */  andi  $v0, $s0, 0x3ff
/* 005E74 8002AA74 00021080 */  sll   $v0, $v0, 2
/* 005E78 8002AA78 00621825 */  or    $v1, $v1, $v0
/* 005E7C 8002AA7C 24A20010 */  addiu $v0, $a1, 0x10
/* 005E80 8002AA80 ACA40000 */  sw    $a0, ($a1)
/* 005E84 8002AA84 ACA30004 */  sw    $v1, 4($a1)
/* 005E88 8002AA88 ACC20000 */  sw    $v0, ($a2)
/* 005E8C 8002AA8C 3C02E700 */  lui   $v0, 0xe700
/* 005E90 8002AA90 ACA20008 */  sw    $v0, 8($a1)
/* 005E94 8002AA94 24A20018 */  addiu $v0, $a1, 0x18
/* 005E98 8002AA98 ACA0000C */  sw    $zero, 0xc($a1)
/* 005E9C 8002AA9C ACC20000 */  sw    $v0, ($a2)
/* 005EA0 8002AAA0 24A20020 */  addiu $v0, $a1, 0x20
/* 005EA4 8002AAA4 ACA90010 */  sw    $t1, 0x10($a1)
/* 005EA8 8002AAA8 ACA70014 */  sw    $a3, 0x14($a1)
/* 005EAC 8002AAAC ACC20000 */  sw    $v0, ($a2)
/* 005EB0 8002AAB0 ACAA0018 */  sw    $t2, 0x18($a1)
/* 005EB4 8002AAB4 ACA8001C */  sw    $t0, 0x1c($a1)
/* 005EB8 8002AAB8 8FB00000 */  lw    $s0, ($sp)
/* 005EBC 8002AABC 03E00008 */  jr    $ra
/* 005EC0 8002AAC0 27BD0008 */   addiu $sp, $sp, 8

/* 005EC4 8002AAC4 27BDFFD8 */  addiu $sp, $sp, -0x28
/* 005EC8 8002AAC8 0080402D */  move  $t0, $a0
/* 005ECC 8002AACC 00A0482D */  move  $t1, $a1
/* 005ED0 8002AAD0 00C0502D */  move  $t2, $a2
/* 005ED4 8002AAD4 00E0582D */  move  $t3, $a3
/* 005ED8 8002AAD8 00061C00 */  sll   $v1, $a2, 0x10
/* 005EDC 8002AADC 00041400 */  sll   $v0, $a0, 0x10
/* 005EE0 8002AAE0 97AC003A */  lhu   $t4, 0x3a($sp)
/* 005EE4 8002AAE4 97AD003E */  lhu   $t5, 0x3e($sp)
/* 005EE8 8002AAE8 97AE0042 */  lhu   $t6, 0x42($sp)
/* 005EEC 8002AAEC 97AF0046 */  lhu   $t7, 0x46($sp)
/* 005EF0 8002AAF0 0062182A */  slt   $v1, $v1, $v0
/* 005EF4 8002AAF4 10600003 */  beqz  $v1, .L8002AB04
/* 005EF8 8002AAF8 AFBF0020 */   sw    $ra, 0x20($sp)
/* 005EFC 8002AAFC 0080502D */  move  $t2, $a0
/* 005F00 8002AB00 00C0402D */  move  $t0, $a2
.L8002AB04:
/* 005F04 8002AB04 00071400 */  sll   $v0, $a3, 0x10
/* 005F08 8002AB08 00051C00 */  sll   $v1, $a1, 0x10
/* 005F0C 8002AB0C 0043102A */  slt   $v0, $v0, $v1
/* 005F10 8002AB10 10400003 */  beqz  $v0, .L8002AB20
/* 005F14 8002AB14 3102FFFF */   andi  $v0, $t0, 0xffff
/* 005F18 8002AB18 00A0582D */  move  $t3, $a1
/* 005F1C 8002AB1C 00E0482D */  move  $t1, $a3
.L8002AB20:
/* 005F20 8002AB20 AFA20010 */  sw    $v0, 0x10($sp)
/* 005F24 8002AB24 3122FFFF */  andi  $v0, $t1, 0xffff
/* 005F28 8002AB28 AFA20014 */  sw    $v0, 0x14($sp)
/* 005F2C 8002AB2C 3142FFFF */  andi  $v0, $t2, 0xffff
/* 005F30 8002AB30 AFA20018 */  sw    $v0, 0x18($sp)
/* 005F34 8002AB34 3162FFFF */  andi  $v0, $t3, 0xffff
/* 005F38 8002AB38 318400FF */  andi  $a0, $t4, 0xff
/* 005F3C 8002AB3C 31A500FF */  andi  $a1, $t5, 0xff
/* 005F40 8002AB40 31C600FF */  andi  $a2, $t6, 0xff
/* 005F44 8002AB44 31E700FF */  andi  $a3, $t7, 0xff
/* 005F48 8002AB48 0C00AA41 */  jal   func_8002A904
/* 005F4C 8002AB4C AFA2001C */   sw    $v0, 0x1c($sp)
/* 005F50 8002AB50 8FBF0020 */  lw    $ra, 0x20($sp)
/* 005F54 8002AB54 03E00008 */  jr    $ra
/* 005F58 8002AB58 27BD0028 */   addiu $sp, $sp, 0x28

func_8002AB5C:
/* 005F5C 8002AB5C 27BDFFD8 */  addiu $sp, $sp, -0x28
/* 005F60 8002AB60 0080402D */  move  $t0, $a0
/* 005F64 8002AB64 00A0482D */  move  $t1, $a1
/* 005F68 8002AB68 00C0502D */  move  $t2, $a2
/* 005F6C 8002AB6C 00E0582D */  move  $t3, $a3
/* 005F70 8002AB70 00061C00 */  sll   $v1, $a2, 0x10
/* 005F74 8002AB74 00041400 */  sll   $v0, $a0, 0x10
/* 005F78 8002AB78 97AC003A */  lhu   $t4, 0x3a($sp)
/* 005F7C 8002AB7C 97AD003E */  lhu   $t5, 0x3e($sp)
/* 005F80 8002AB80 97AE0042 */  lhu   $t6, 0x42($sp)
/* 005F84 8002AB84 97AF0046 */  lhu   $t7, 0x46($sp)
/* 005F88 8002AB88 0062182A */  slt   $v1, $v1, $v0
/* 005F8C 8002AB8C 10600003 */  beqz  $v1, .L8002AB9C
/* 005F90 8002AB90 AFBF0020 */   sw    $ra, 0x20($sp)
/* 005F94 8002AB94 0080502D */  move  $t2, $a0
/* 005F98 8002AB98 00C0402D */  move  $t0, $a2
.L8002AB9C:
/* 005F9C 8002AB9C 00071400 */  sll   $v0, $a3, 0x10
/* 005FA0 8002ABA0 00051C00 */  sll   $v1, $a1, 0x10
/* 005FA4 8002ABA4 0043102A */  slt   $v0, $v0, $v1
/* 005FA8 8002ABA8 10400003 */  beqz  $v0, .L8002ABB8
/* 005FAC 8002ABAC 3102FFFF */   andi  $v0, $t0, 0xffff
/* 005FB0 8002ABB0 00A0582D */  move  $t3, $a1
/* 005FB4 8002ABB4 00E0482D */  move  $t1, $a3
.L8002ABB8:
/* 005FB8 8002ABB8 AFA20010 */  sw    $v0, 0x10($sp)
/* 005FBC 8002ABBC 3122FFFF */  andi  $v0, $t1, 0xffff
/* 005FC0 8002ABC0 AFA20014 */  sw    $v0, 0x14($sp)
/* 005FC4 8002ABC4 3142FFFF */  andi  $v0, $t2, 0xffff
/* 005FC8 8002ABC8 AFA20018 */  sw    $v0, 0x18($sp)
/* 005FCC 8002ABCC 3162FFFF */  andi  $v0, $t3, 0xffff
/* 005FD0 8002ABD0 318400FF */  andi  $a0, $t4, 0xff
/* 005FD4 8002ABD4 31A500FF */  andi  $a1, $t5, 0xff
/* 005FD8 8002ABD8 31C600FF */  andi  $a2, $t6, 0xff
/* 005FDC 8002ABDC 31E700FF */  andi  $a3, $t7, 0xff
/* 005FE0 8002ABE0 0C00AA41 */  jal   func_8002A904
/* 005FE4 8002ABE4 AFA2001C */   sw    $v0, 0x1c($sp)
/* 005FE8 8002ABE8 8FBF0020 */  lw    $ra, 0x20($sp)
/* 005FEC 8002ABEC 03E00008 */  jr    $ra
/* 005FF0 8002ABF0 27BD0028 */   addiu $sp, $sp, 0x28

/* 005FF4 8002ABF4 00000000 */  nop   
/* 005FF8 8002ABF8 00000000 */  nop   
/* 005FFC 8002ABFC 00000000 */  nop   
func_8002AC00:
/* 006000 8002AC00 27BDFFE8 */  addiu $sp, $sp, -0x18
/* 006004 8002AC04 3C050005 */  lui   $a1, 5
/* 006008 8002AC08 3C048030 */  lui   $a0, 0x8030
/* 00600C 8002AC0C 2484B800 */  addiu $a0, $a0, -0x4800
/* 006010 8002AC10 AFBF0010 */  sw    $ra, 0x10($sp)
/* 006014 8002AC14 0C00A40C */  jal   func_80029030
/* 006018 8002AC18 34A54000 */   ori   $a1, $a1, 0x4000
/* 00601C 8002AC1C 8FBF0010 */  lw    $ra, 0x10($sp)
/* 006020 8002AC20 03E00008 */  jr    $ra
/* 006024 8002AC24 27BD0018 */   addiu $sp, $sp, 0x18

func_8002AC28:
/* 006028 8002AC28 27BDFFE8 */  addiu $sp, $sp, -0x18
/* 00602C 8002AC2C 0080282D */  move  $a1, $a0
/* 006030 8002AC30 3C048030 */  lui   $a0, 0x8030
/* 006034 8002AC34 2484B800 */  addiu $a0, $a0, -0x4800
/* 006038 8002AC38 AFBF0010 */  sw    $ra, 0x10($sp)
/* 00603C 8002AC3C 0C00A41B */  jal   func_8002906C
/* 006040 8002AC40 00000000 */   nop   
/* 006044 8002AC44 8FBF0010 */  lw    $ra, 0x10($sp)
/* 006048 8002AC48 03E00008 */  jr    $ra
/* 00604C 8002AC4C 27BD0018 */   addiu $sp, $sp, 0x18

/* 006050 8002AC50 27BDFFE8 */  addiu $sp, $sp, -0x18
/* 006054 8002AC54 0080282D */  move  $a1, $a0
/* 006058 8002AC58 3C048030 */  lui   $a0, 0x8030
/* 00605C 8002AC5C 2484B800 */  addiu $a0, $a0, -0x4800
/* 006060 8002AC60 AFBF0010 */  sw    $ra, 0x10($sp)
/* 006064 8002AC64 0C00A459 */  jal   func_80029164
/* 006068 8002AC68 00000000 */   nop   
/* 00606C 8002AC6C 8FBF0010 */  lw    $ra, 0x10($sp)
/* 006070 8002AC70 03E00008 */  jr    $ra
/* 006074 8002AC74 27BD0018 */   addiu $sp, $sp, 0x18

func_8002AC78:
/* 006078 8002AC78 27BDFFE8 */  addiu $sp, $sp, -0x18
/* 00607C 8002AC7C 0080282D */  move  $a1, $a0
/* 006080 8002AC80 3C048030 */  lui   $a0, 0x8030
/* 006084 8002AC84 2484B800 */  addiu $a0, $a0, -0x4800
/* 006088 8002AC88 AFBF0010 */  sw    $ra, 0x10($sp)
/* 00608C 8002AC8C 0C00A487 */  jal   func_8002921C
/* 006090 8002AC90 00000000 */   nop   
/* 006094 8002AC94 8FBF0010 */  lw    $ra, 0x10($sp)
/* 006098 8002AC98 03E00008 */  jr    $ra
/* 00609C 8002AC9C 27BD0018 */   addiu $sp, $sp, 0x18

func_8002ACA0:
/* 0060A0 8002ACA0 27BDFFE8 */  addiu $sp, $sp, -0x18
/* 0060A4 8002ACA4 AFBF0010 */  sw    $ra, 0x10($sp)
/* 0060A8 8002ACA8 3C050002 */  lui   $a1, 2
/* 0060AC 8002ACAC 3C04803E */  lui   $a0, 0x803e
/* 0060B0 8002ACB0 2484A800 */  addiu $a0, $a0, -0x5800
/* 0060B4 8002ACB4 0C00A40C */  jal   func_80029030
/* 0060B8 8002ACB8 34A55800 */   ori   $a1, $a1, 0x5800
/* 0060BC 8002ACBC 0040202D */  move  $a0, $v0
/* 0060C0 8002ACC0 2402FFFF */  addiu $v0, $zero, -1
/* 0060C4 8002ACC4 0040182D */  move  $v1, $v0
/* 0060C8 8002ACC8 54830001 */  bnel  $a0, $v1, .L8002ACD0
/* 0060CC 8002ACCC 0000102D */   move  $v0, $zero
.L8002ACD0:
/* 0060D0 8002ACD0 8FBF0010 */  lw    $ra, 0x10($sp)
/* 0060D4 8002ACD4 03E00008 */  jr    $ra
/* 0060D8 8002ACD8 27BD0018 */   addiu $sp, $sp, 0x18

func_8002ACDC:
/* 0060DC 8002ACDC 03E00008 */  jr    $ra
/* 0060E0 8002ACE0 0000102D */   move  $v0, $zero

func_8002ACE4:
/* 0060E4 8002ACE4 3C028007 */  lui   $v0, 0x8007
/* 0060E8 8002ACE8 8C42419C */  lw    $v0, 0x419c($v0)
/* 0060EC 8002ACEC 27BDFFE8 */  addiu $sp, $sp, -0x18
/* 0060F0 8002ACF0 AFBF0010 */  sw    $ra, 0x10($sp)
/* 0060F4 8002ACF4 80420070 */  lb    $v0, 0x70($v0)
/* 0060F8 8002ACF8 10400007 */  beqz  $v0, .L8002AD18
/* 0060FC 8002ACFC 0080282D */   move  $a1, $a0
/* 006100 8002AD00 3C04803E */  lui   $a0, 0x803e
/* 006104 8002AD04 2484A800 */  addiu $a0, $a0, -0x5800
/* 006108 8002AD08 0C00A41B */  jal   func_8002906C
/* 00610C 8002AD0C 00000000 */   nop   
/* 006110 8002AD10 0800AB48 */  j     func_8002AD20
/* 006114 8002AD14 00000000 */   nop   

.L8002AD18:
/* 006118 8002AD18 0C00AB0A */  jal   func_8002AC28
/* 00611C 8002AD1C 00A0202D */   move  $a0, $a1
func_8002AD20:
/* 006120 8002AD20 8FBF0010 */  lw    $ra, 0x10($sp)
/* 006124 8002AD24 03E00008 */  jr    $ra
/* 006128 8002AD28 27BD0018 */   addiu $sp, $sp, 0x18

func_8002AD2C:
/* 00612C 8002AD2C 3C028007 */  lui   $v0, 0x8007
/* 006130 8002AD30 8C42419C */  lw    $v0, 0x419c($v0)
/* 006134 8002AD34 27BDFFE8 */  addiu $sp, $sp, -0x18
/* 006138 8002AD38 AFBF0010 */  sw    $ra, 0x10($sp)
/* 00613C 8002AD3C 80420070 */  lb    $v0, 0x70($v0)
/* 006140 8002AD40 14400005 */  bnez  $v0, .L8002AD58
/* 006144 8002AD44 0080282D */   move  $a1, $a0
/* 006148 8002AD48 0C00AB1E */  jal   func_8002AC78
/* 00614C 8002AD4C 00000000 */   nop   
/* 006150 8002AD50 0800AB5A */  j     func_8002AD68
/* 006154 8002AD54 00000000 */   nop   

.L8002AD58:
/* 006158 8002AD58 3C04803E */  lui   $a0, 0x803e
/* 00615C 8002AD5C 2484A800 */  addiu $a0, $a0, -0x5800
/* 006160 8002AD60 0C00A487 */  jal   func_8002921C
/* 006164 8002AD64 00000000 */   nop   
func_8002AD68:
/* 006168 8002AD68 8FBF0010 */  lw    $ra, 0x10($sp)
/* 00616C 8002AD6C 03E00008 */  jr    $ra
/* 006170 8002AD70 27BD0018 */   addiu $sp, $sp, 0x18

func_8002AD74:
/* 006174 8002AD74 27BDFFE8 */  addiu $sp, $sp, -0x18
/* 006178 8002AD78 AFBF0010 */  sw    $ra, 0x10($sp)
/* 00617C 8002AD7C 3C050001 */  lui   $a1, 1
/* 006180 8002AD80 3C048027 */  lui   $a0, 0x8027
/* 006184 8002AD84 24848000 */  addiu $a0, $a0, -0x8000
/* 006188 8002AD88 0C00A40C */  jal   func_80029030
/* 00618C 8002AD8C 34A58000 */   ori   $a1, $a1, 0x8000
/* 006190 8002AD90 0040202D */  move  $a0, $v0
/* 006194 8002AD94 2402FFFF */  addiu $v0, $zero, -1
/* 006198 8002AD98 0040182D */  move  $v1, $v0
/* 00619C 8002AD9C 54830001 */  bnel  $a0, $v1, .L8002ADA4
/* 0061A0 8002ADA0 0000102D */   move  $v0, $zero
.L8002ADA4:
/* 0061A4 8002ADA4 8FBF0010 */  lw    $ra, 0x10($sp)
/* 0061A8 8002ADA8 03E00008 */  jr    $ra
/* 0061AC 8002ADAC 27BD0018 */   addiu $sp, $sp, 0x18

func_8002ADB0:
/* 0061B0 8002ADB0 3C028007 */  lui   $v0, 0x8007
/* 0061B4 8002ADB4 8C42419C */  lw    $v0, 0x419c($v0)
/* 0061B8 8002ADB8 27BDFFE8 */  addiu $sp, $sp, -0x18
/* 0061BC 8002ADBC AFBF0010 */  sw    $ra, 0x10($sp)
/* 0061C0 8002ADC0 80420070 */  lb    $v0, 0x70($v0)
/* 0061C4 8002ADC4 0080282D */  move  $a1, $a0
/* 0061C8 8002ADC8 3C048027 */  lui   $a0, 0x8027
/* 0061CC 8002ADCC 24848000 */  addiu $a0, $a0, -0x8000
/* 0061D0 8002ADD0 10400003 */  beqz  $v0, .L8002ADE0
/* 0061D4 8002ADD4 00000000 */   nop   
/* 0061D8 8002ADD8 3C04803E */  lui   $a0, 0x803e
/* 0061DC 8002ADDC 2484A800 */  addiu $a0, $a0, -0x5800
.L8002ADE0:
/* 0061E0 8002ADE0 0C00A41B */  jal   func_8002906C
/* 0061E4 8002ADE4 00000000 */   nop   
/* 0061E8 8002ADE8 8FBF0010 */  lw    $ra, 0x10($sp)
/* 0061EC 8002ADEC 03E00008 */  jr    $ra
/* 0061F0 8002ADF0 27BD0018 */   addiu $sp, $sp, 0x18

func_8002ADF4:
/* 0061F4 8002ADF4 3C028007 */  lui   $v0, 0x8007
/* 0061F8 8002ADF8 8C42419C */  lw    $v0, 0x419c($v0)
/* 0061FC 8002ADFC 27BDFFE8 */  addiu $sp, $sp, -0x18
/* 006200 8002AE00 AFBF0010 */  sw    $ra, 0x10($sp)
/* 006204 8002AE04 80420070 */  lb    $v0, 0x70($v0)
/* 006208 8002AE08 0080282D */  move  $a1, $a0
/* 00620C 8002AE0C 3C04803E */  lui   $a0, 0x803e
/* 006210 8002AE10 2484A800 */  addiu $a0, $a0, -0x5800
/* 006214 8002AE14 14400003 */  bnez  $v0, .L8002AE24
/* 006218 8002AE18 00000000 */   nop   
/* 00621C 8002AE1C 3C048027 */  lui   $a0, 0x8027
/* 006220 8002AE20 24848000 */  addiu $a0, $a0, -0x8000
.L8002AE24:
/* 006224 8002AE24 0C00A487 */  jal   func_8002921C
/* 006228 8002AE28 00000000 */   nop   
/* 00622C 8002AE2C 8FBF0010 */  lw    $ra, 0x10($sp)
/* 006230 8002AE30 03E00008 */  jr    $ra
/* 006234 8002AE34 27BD0018 */   addiu $sp, $sp, 0x18

/* 006238 8002AE38 00000000 */  nop   
/* 00623C 8002AE3C 00000000 */  nop   
